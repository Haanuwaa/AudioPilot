...
namespace AudioPilot.Coordinators
{
    internal sealed partial class AppSwitchCommandCoordinator
    {
            ...
            Action showOutputSwitchFailureWarning)
        {
            ...
            using var trace = AudioPilot.Logging.OperationTrace.Attach(AudioPilot.Logging.OperationTrace.CurrentId ?? opId);
            ...
            if (!_requestCoordinator.TryBeginOutputSwitchRequest(opId, out AppSwitchRequestRejectionReason rejectionReason))
            {
                if (rejectionReason is AppSwitchRequestRejectionReason.InProgress or AppSwitchRequestRejectionReason.Debounced)
                {
                    ...
                    {
                        string? reconnectDeviceName = _outputIntentTracker.GetReconnectOverlayDeviceName();
                        if (!string.IsNullOrWhiteSpace(reconnectDeviceName))
                        {
                            _overlay.Show(OverlayDeviceKind.Output, "Reconnecting output device", reconnectDeviceName);
                        }
                        using MMDevice? currentDevice = _audio.GetDefaultPlaybackDevice();
                        if (currentDevice != null && _outputIntentTracker.DoesRequestedTargetMatchActiveTarget(configuredCycle, currentDevice.ID, reverse))
                        {
                            return false;
                        }
                    }
            ...
            Action showOutputSwitchFailureWarning)
        {
            return new SwitchExecutionCallbacks(
                ...
                SwitchDirectAsync: (currentDevice, targetDevice, switchOpId) => _audio.SwitchAudioDeviceAsync(targetDevice.Id, muteMic, muteSound, deafen, preserveAudioLevels, opId: switchOpId),
                SwitchFinalAsync: (currentId, targetDevice, switchOpId) => _audio.SwitchAudioDeviceAsync(targetDevice.Id, muteMic, muteSound, deafen, preserveAudioLevels, opId: switchOpId),
