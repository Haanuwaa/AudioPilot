            ...
            BluetoothReconnectOptions reconnectOptions)
        {
            ...
            if (!_requestCoordinator.TryBeginInputSwitchRequest(opId, out AppSwitchRequestRejectionReason rejectionReason))
            {
                if (rejectionReason is AppSwitchRequestRejectionReason.InProgress or AppSwitchRequestRejectionReason.Debounced)
                {
                    ...
                    {
                        string? reconnectDeviceName = _inputIntentTracker.GetReconnectOverlayDeviceName();
                        if (!string.IsNullOrWhiteSpace(reconnectDeviceName))
                        {
                            _overlay.Show(OverlayDeviceKind.Input, "Reconnecting input device", reconnectDeviceName);
                        }
                        using MMDevice? currentDevice = _audio.GetDefaultRecordingDevice();
                        if (currentDevice != null && _inputIntentTracker.DoesRequestedTargetMatchActiveTarget(configuredCycle, currentDevice.ID, reverse))
                        {
                            return false;
                        }
                    }

                    TryQueueInputCoalescedRetry(configuredCycle, reverse, preserveAudioLevels, reconnectOptions, rejectionReason, opId);
                }
        ...
        private SwitchExecutionCallbacks CreateInputSwitchExecutionCallbacks(bool preserveAudioLevels)
        {
            return new SwitchExecutionCallbacks(
                ...
                ScheduleDeferredAutoSwitch: (switchOpId, cycleSnapshot, pendingDeviceId, pendingDeviceName) =>
                    TryScheduleDeferredInputAutoSwitch(switchOpId, cycleSnapshot, pendingDeviceId, pendingDeviceName, preserveAudioLevels),
