...
namespace AudioPilot.Coordinators
{
    ...
    internal sealed class WinRtNetworkMonitor(Logger logger) : INetworkConnectionMonitor
    {
        ...
        private static void AddResolvedNetworkNames(ConnectionProfile? profile, HashSet<string> networkNames)
        {
            ...
            foreach (string networkName in NetworkTriggerCoordinator.ResolveNetworkNames(profile))
            {
        ...
        Func<IReadOnlyList<AudioRoutine>>? routineSnapshotProvider = null) : IDisposable
    {
        ...
        private long _networkStateVersion;
        ...
        internal static IReadOnlyList<string> ResolveNetworkNames(ConnectionProfile profile)
        {
        ...
        private void OnConnectivityChanged(object? sender, EventArgs e)
        {
            ...
            long networkStateVersion;

            lock (_lock)
            {
                if (_disposed || !_started || monitorGeneration != _monitorGeneration || snapshotSequence < _lastAppliedSnapshotSequence)
                {
                    return;
                }

                _lastAppliedSnapshotSequence = snapshotSequence;
                ...
                connectedNetworkNames = new HashSet<string>(currentNetworkNames, StringComparer.OrdinalIgnoreCase);
                connectedNetworkNames.ExceptWith(previousNetworkNames);

                disconnectedNetworkNames = new HashSet<string>(previousNetworkNames, StringComparer.OrdinalIgnoreCase);
                disconnectedNetworkNames.ExceptWith(currentNetworkNames);
                ...
                networkStateVersion = ++_networkStateVersion;
                foreach ((string identity, long timestamp) in _lastTriggerTimestampsByIdentity)
                {
                    if (nowTimestampTicks - timestamp >= _debounceInterval.Ticks)
                    {
                        _lastTriggerTimestampsByIdentity.Remove(identity);
                    }
                }
            ...
            foreach ((AudioRoutine routine, string executionSource) in routineExecutions.DistinctBy(static execution => execution.Routine.Id, StringComparer.OrdinalIgnoreCase))
            {
                try
                {
                    lock (_lock)
                    {
                        if (_disposed || !_started || monitorGeneration != _monitorGeneration || networkStateVersion != _networkStateVersion)
                        {
                            return;
                        }
                    }
                ...
                {
                    logger.Error("NetworkTriggerCoordinator", () => $"network-routine-trigger-failed | routineName={LogPrivacy.Label(routine.Name)} reason={ex.GetType().Name}", exception: ex);
                }
            }
        }
        ...
        {
            string routineIdentity = routine.RuntimeTriggerKey;
            ...
            out string executionSource)
        {
            ...
            switch (routine.NetworkTriggerDirection)
            {
                ...
                case NetworkTriggerDirection.Disconnect:
                    if (string.IsNullOrWhiteSpace(normalizedTargetNetworkName)
                        ? previousConnectedToAnyNetwork && !currentConnectedToAnyNetwork
                        : disconnectedNetworkNames.Contains(normalizedTargetNetworkName))
                    {
        ...
        private List<AudioRoutine> GetRoutineSnapshot()
        {
            try
            {
                return [.. _routineSnapshotProvider().Where(static routine => routine.Enabled && routine.HasExecutionTarget)
                    .SelectMany(static routine => routine.ExpandAutomaticTriggers(static trigger => trigger.Kind == RoutineTriggerKind.Network))];
            }
            catch (Exception ex)
            {
                logger.Warning(
        ...
        public static HashSet<string> GetAvailableNetworkNames()
        {
            ...
            try
            {
                foreach (ConnectionProfile profile in NetworkInformation.GetConnectionProfiles())
                {
                    ...
                    foreach (string networkName in ResolveNetworkNames(profile))
                    {
                        networkNames.Add(networkName);
                    }
                }
