...
namespace AudioPilot.Coordinators
{
    ...
    internal static class AppRoutineRestoreCoordinator
    {
        ...
        /// </remarks>
        internal static Task<RoutineRestoreResult> ExecuteRestoreAsync(
            RoutineStatefulSession session,
            RoutineRestoreDependencies dependencies,
            Logger logger)
        {
            // Both endpoint lookups and nominally async callbacks can complete synchronously after blocking.
            // The caller awaits this task before updating UI or releasing restoration ownership.
            return session.RestoreSnapshot.HasValue
                ? Task.Run(() => ExecuteRestoreCoreAsync(session, dependencies, logger))
                : ExecuteRestoreCoreAsync(session, dependencies, logger);
        }

        private static async Task<RoutineRestoreResult> ExecuteRestoreCoreAsync(
            RoutineStatefulSession session,
            RoutineRestoreDependencies dependencies,
            Logger logger)
        {
            ...
            RoutineAudioRestoreSnapshot snapshot = session.RestoreSnapshot.Value;
