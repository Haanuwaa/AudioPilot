...
namespace AudioPilot.Coordinators
{
    internal sealed partial class AppCliOverlayCoordinator
    {
        ...
        public Task<MediaSeekResult> MediaSeekAsync(bool backward, int? seconds = null, string source = CliSource, CancellationToken cancellationToken = default)
        {
            lock (_mediaLifecycleLock)
            {
                if (Volatile.Read(ref _mediaShutdownStarted) != 0 || cancellationToken.IsCancellationRequested)
                {
                ...
                lock (_mediaSendOrderLock)
                {
                    ...
                    Task<MediaSeekResult> operation = ProcessSeekAsync(predecessor, backward ? -step : step, version, NormalizeMediaCommandSource(source), cancellationToken);
        ...
        private async Task<MediaSeekResult> ProcessSeekAsync(Task predecessor, int offset, int version, string source, CancellationToken cancellationToken)
        {
            long started = Stopwatch.GetTimestamp();
            using var queueDeadline = CancellationTokenSource.CreateLinkedTokenSource(_mediaShutdownCts.Token, cancellationToken);
            ...
            {
                result = _mediaShutdownCts.IsCancellationRequested || cancellationToken.IsCancellationRequested
            ...
            try
            {
                ...
                {
                    if (!cancellationToken.IsCancellationRequested && version == Volatile.Read(ref _latestMediaOverlayRequestVersion))
                    {
                        if (result.PositionChangeText is { } positionChange)
                        {
                            ...
                            overlay.ShowMediaTrack(header, result.Track?.Title ?? "Track information unavailable", result.Track?.Artist, positionChange, () => !cancellationToken.IsCancellationRequested && IsMediaFeedbackCurrent(version), icon: offset > 0 ? OverlayIcon.SeekForward : OverlayIcon.SeekBackward);
                        }
                        else
                        {
                            overlay.Show(result.Message, isCurrent: () => !cancellationToken.IsCancellationRequested && IsMediaFeedbackCurrent(version), icon: OverlayIcon.Information);
                        }
                    }
