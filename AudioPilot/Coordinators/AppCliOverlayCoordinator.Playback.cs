...
namespace AudioPilot.Coordinators
{
    internal sealed partial class AppCliOverlayCoordinator
    {
        ...
        public Task<MediaPlaybackResult> MediaSetPlayingAsync(bool playing, CancellationToken cancellationToken = default)
        {
            lock (_mediaLifecycleLock)
            {
                ...
                lock (_mediaSendOrderLock)
                {
                    ...
                    Task<MediaPlaybackResult> operation = Task.Run(() => ProcessPlaybackChangeAsync(predecessor, playing, cancellationToken));
        ...
        private async Task<MediaPlaybackResult> ProcessPlaybackChangeAsync(Task predecessor, bool playing, CancellationToken cancellationToken)
        {
            long started = Stopwatch.GetTimestamp();
            using var deadline = CancellationTokenSource.CreateLinkedTokenSource(_mediaShutdownCts.Token, cancellationToken);
            ...
            try
            {
                ...
                if (result.Code == "media-playback-canceled" && !_mediaShutdownCts.IsCancellationRequested && !cancellationToken.IsCancellationRequested && deadline.IsCancellationRequested)
                {
            ...
            {
                result = new(false, (_mediaShutdownCts.IsCancellationRequested || cancellationToken.IsCancellationRequested) ? "media-playback-canceled" : "media-playback-timeout",
                    (_mediaShutdownCts.IsCancellationRequested || cancellationToken.IsCancellationRequested) ? "Playback request canceled." : "Playback request timed out in the queue.", playing ? "Playing" : "Paused");
            }
            catch (Exception ex)
            {
