...
namespace AudioPilot.Coordinators
{
        ...
        Action<bool, int>? suppressConnectedHotplugOverlay = null) : IDisposable
    {
        ...
        private readonly AppSwitchIntentTracker _outputIntentTracker = new(BluetoothReconnectDeviceKind.Output);
        private readonly AppSwitchIntentTracker _inputIntentTracker = new(BluetoothReconnectDeviceKind.Input);
        ...
        private readonly SemaphoreSlim _directOutputGate = new(1, 1);
        private readonly SemaphoreSlim _directInputGate = new(1, 1);
        private CancellationTokenSource? _directOutputCts;
        private CancellationTokenSource? _directInputCts;
            ...
            CancellationToken operationCancellationToken)
        {
            ...
            AppSwitchIntentTracker intentTracker = callbacks.ReconnectKind == BluetoothReconnectDeviceKind.Output ? _outputIntentTracker : _inputIntentTracker;
            ...
            try
            {
                ...
                if (connectedCycle.Count <= 1 && skippedDevices.Count > 0)
                {
                    ...
                    intentTracker.SetActiveTarget(callbacks.ReconnectKind, reconnectTargetId, reconnectTargetName);
                    intentTracker.SetReconnectOverlayDeviceName(callbacks.ReconnectKind, reconnectTargetName);
                    ...
                    BluetoothReconnectAttemptResult reconnectResult = await _bluetoothReconnectCoordinator.TryReconnectDetailedAsync(
                        ...
                        onAttemptProgress: (attempt, maxAttempts, attemptDeviceName) =>
                        {
                            ...
                            intentTracker.SetReconnectOverlayDeviceName(callbacks.ReconnectKind, attemptDeviceName);
            ...
            finally
            {
                ...
                {
                    intentTracker.ClearActiveTarget(callbacks.ReconnectKind);
                    intentTracker.ClearReconnectOverlayDeviceName(callbacks.ReconnectKind);
                }
            }
        }
        ...
        public void Dispose()
        {
            ...
            _lifetimeCts.Cancel();
            TryCancelDirectRequest(ref _directOutputCts);
            TryCancelDirectRequest(ref _directInputCts);
            _lifetimeCts.Dispose();
        }

        private static void TryCancelDirectRequest(ref CancellationTokenSource? field)
        {
            CancellationTokenSource? source = Interlocked.Exchange(ref field, null);
            try
            {
                source?.Cancel();
            }
