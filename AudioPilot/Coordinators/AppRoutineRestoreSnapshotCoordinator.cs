...
namespace AudioPilot.Coordinators
{
    ...
    internal static class AppRoutineRestoreSnapshotCoordinator
    {
        ...
        /// </remarks>
        internal static RoutineAudioRestoreSnapshot? CaptureSnapshot(
            ...
            Logger logger)
        {
            ...
            if (!shouldCaptureOutputVolume && !shouldCaptureInputVolume)
            {
                logger.Info(
                    "AppViewModel",
                    () => $"routine-restore-snapshot-skipped | {AppViewModel.BuildRoutineExecutionLogContext(routine, "stateful-capture", showOverlay: false, applicationProcessId: null)} reason=no-device-or-level-actions");
                return null;
            }
            ...
            if (shouldCaptureOutputVolume)
            {
                try
                {
                    ...
                    previousOutputMuted = routine.OutputMuteAction == RoutineMuteAction.Unchanged ? outputDevice.IsMuted : null;
                }
                catch (Exception ex)
                {
            ...
            if (shouldCaptureInputVolume)
            {
                try
                {
                    ...
                    previousInputMuted = routine.InputMuteAction == RoutineMuteAction.Unchanged ? inputDevice.IsMuted : null;
                }
                catch (Exception ex)
                {
            ...
            RoutineAudioRestoreSnapshot snapshot = new(
