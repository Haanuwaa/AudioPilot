...
namespace AudioPilot.Coordinators
{
    ...
    internal readonly record struct RoutineAppOutputLeaseRefreshPreparation(
        ...
        Dictionary<string, RoutineAppOutputLease> ReconciledLeases,
    ...
    internal static class AppRoutineAppStartCoordinator
    {
            ...
            DateTime? nowUtc = null)
        {
            return PrepareLeaseRefresh(
                currentLeases,
                watchedRoutines,
                RoutineApplicationRouting.CreateRoutineAppStartSnapshotSet(processSnapshots),
                nowUtc);
        }
            ...
            DateTime? nowUtc = null)
        {
            ...
            Dictionary<string, RoutineAppOutputLease> reconciledLeases = RoutineApplicationRouting.ReconcileRoutineAppOutputLeases(
                currentLeases,
                watchedRoutines,
                processSnapshotSet,
                nowUtc);
            List<RoutineAppOutputLease> removedLeases =
            [
                .. currentLeases
            ...
            CancellationToken cancellationToken)
        {
            ...
            IReadOnlyList<RoutineAppStartMatch> matches = RoutineApplicationRouting.EvaluateRoutineAppStartMatchesForProcess(
                watchedRoutines,
                snapshot.Value);
            if (matches.Count == 0)
            {
            ...
            return new RoutineAppStartProcessWorkload(
                snapshot.Value,
                matches,
                RoutineApplicationRouting.ShouldCaptureProcessSnapshotsForStartedMatches(matches, activeLeaseCount));
        }
            ...
            RoutineAppStartSnapshotSet processSnapshotSet)
        {
            ...
            foreach (RoutineAppStartMatch match in matches)
            {
                bool shouldSkip = RoutineApplicationRouting.ShouldSkipRoutineAppStartMatchForExistingLease(
                    match,
                    processSnapshot,
                    activeLeases,
                    processSnapshotSet);
        ...
        internal static async Task ExecuteLeaseApplicationsAsync(
            ...
            Func<RoutineAppOutputLease, uint, Task<bool>> tryApplyOutputAsync,
            Func<RoutineAppOutputLease, uint, Task<bool>> tryApplyInputAsync,
            Func<RoutineAppOutputLease, string?, string?, Task> showCompletionOverlayAsync,
            Action<RoutineAppOutputLease, uint, bool> markLeaseProcessApplied,
            Action<RoutineAppOutputLease> markLeaseOverlayShown,
            Action<RoutineAppOutputLease> markLeaseCompleted,
            Logger logger,
            CancellationToken cancellationToken)
        {
        ...
        internal static bool HasPendingLeaseApplications(RoutineAppOutputLease lease)
        {
        ...
        private static bool HasLeaseCompleted(RoutineAppOutputLease lease)
        {
        ...
        internal static bool DoesLiveLeaseMatchExpectedSnapshot(
            RoutineAppOutputLease currentLease,
            RoutineAppOutputLease expectedLease)
        {
            ArgumentNullException.ThrowIfNull(currentLease);
            ArgumentNullException.ThrowIfNull(expectedLease);

            return currentLease.Generation == expectedLease.Generation && string.Equals(currentLease.LeaseKey, expectedLease.LeaseKey, StringComparison.OrdinalIgnoreCase)
