...
namespace AudioPilot.Platform
{
    ...
    public partial class SingleInstanceHelper : IDisposable, IAsyncDisposable
    {
        ...
        private readonly Lock _listenerAcceptLock = new();
        ...
        private TaskCompletionSource? _responseWriteTimedOutSource;
        ...
        private readonly int _responseWriteTimeoutMs;
        private readonly int _commandTimeoutMs;

        public event EventHandler? ActivationRequested;
        internal event Func<Task<bool>>? ActivationRequestedAsync;
        public event Func<string, CancellationToken, Task<SingleInstanceCommandResult>>? CommandRequested;
        ...
        internal Task ResponseWriteTimedOutForTests => _responseWriteTimedOutSource?.Task ?? Task.CompletedTask;
        ...
        public SingleInstanceHelper()
            : this(DiagnosticSession.InstanceName, $"{DiagnosticSession.InstanceName}.Activation")
        {
        }
        ...
        internal SingleInstanceHelper(
            ...
            int responseWriteTimeoutMs = AppConstants.Timing.SingleInstanceResponseWriteTimeoutMs,
            int? commandTimeoutMs = null)
        {
            ...
            _responseWriteTimeoutMs = Math.Max(1, responseWriteTimeoutMs);
            _commandTimeoutMs = Math.Max(1, commandTimeoutMs ?? responseTimeoutMs - 1000);
        }
        ...
        private void StartActivationListener()
        {
            ...
            _responseWriteTimedOutSource = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        private Task StopActivationListener()
        {
            ...
            finally
            {
                ...
                _responseWriteTimedOutSource = null;
            }
        ...
        /// <summary>
        /// Accepts activation requests independently of command execution. Cancellation closes the accepting pipe
        /// before the primary mutex is released; accepted requests retain their streams, gate, and listener token until drained.
        /// </summary>
        private async Task ActivationListenerLoopAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    requests.RemoveWhere(static request => request.IsCompleted);
                    if (requests.Count >= MaxConcurrentRequests)
                    {
                        await Task.WhenAny(requests).WaitAsync(cancellationToken).ConfigureAwait(false);
                        continue;
                    }
                    ...
                    try
                    {
                        CancellationTokenRegistration closeAccept;
                        lock (_listenerAcceptLock)
                        {
                            ...
                            closeAccept = cancellationToken.Register(static state => ((NamedPipeServerStream)state!).Dispose(), server);
                        }
                        using (closeAccept)
                        {
                        ...
                        requests.Add(Task.Run(() => ProcessConnectionAsync(connectedServer, commandGate, cancellationToken), CancellationToken.None));
                        server = null;
                    }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        await Task.Delay(AppConstants.Timing.SingleInstanceListenerRetryDelayMs, cancellationToken).ConfigureAwait(false);
                    }
                    finally
                    {
                        if (server != null)
        ...
        private async Task ProcessConnectionAsync(NamedPipeServerStream server, SemaphoreSlim commandGate, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    readCts.CancelAfter(_requestReadTimeoutMs);
                    try
                    {
                        payload = await ReadFramedMessageAsync(server, MaxRequestBytes, readCts.Token).ConfigureAwait(false);
                    }
                    catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
                    {
                ...
                if (string.Equals(payload, ActivationPayload, StringComparison.Ordinal))
                {
                    try
                    {
                        bool shown = await RaiseActivationRequestedAsync().ConfigureAwait(false);
                        response = shown
                            ? new SingleInstanceCommandResult(0, ProtocolVersion: ResponseProtocolVersion)
                            : new SingleInstanceCommandResult(3, ErrorCode: "activation-failed", ErrorMessage: "The running AudioPilot instance could not show its main window.", ProtocolVersion: ResponseProtocolVersion);
                    }
                    catch (Exception ex)
                    {
                        _logger.Warning("SingleInstanceHelper", "Activation handler failed", nameof(ProcessConnectionAsync), ex);
                        response = new SingleInstanceCommandResult(7, ErrorCode: "activation-handler-failed", ErrorMessage: "The running AudioPilot instance could not process the activation request.", ProtocolVersion: ResponseProtocolVersion);
                    }
                }
                else
                {
                    await ExecuteCommandConnectionAsync(server, payload, commandGate, cancellationToken).ConfigureAwait(false);
                    return;
                }

                using CancellationTokenSource writeCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
                writeCts.CancelAfter(_responseWriteTimeoutMs);
                try
                {
                ...
                catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    _logger.Warning("SingleInstanceHelper", () => $"Response write timed out | timeoutMs={_responseWriteTimeoutMs}");
                }
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
            {
            ...
            {
                _logger.Warning("SingleInstanceHelper", "Activation request failed", nameof(ProcessConnectionAsync), ex);
            }
        }
        ...
        internal static int ResolveCommandTimeoutMs(string payload, int transportBudgetMs)
        {
            if (CliCommand.TryFromPipePayload(payload, out var command) && command.Action == CliAction.WaitForDevice &&
                int.TryParse(command.Value, out int waitMs) && waitMs > 0)
            {
                return (int)Math.Min(int.MaxValue, (long)transportBudgetMs + waitMs);
            }
            return transportBudgetMs;
        }
        ...
        private bool SignalExistingInstance(string payloadText)
        {
            ...
            try
            {
                ...
                using var responseTimeout = new CancellationTokenSource(ResolveCommandTimeoutMs(payloadText, _responseTimeoutMs));
                if (Encoding.UTF8.GetByteCount(payloadText) > MaxRequestBytes)
                {
                    throw new InvalidDataException("The forwarded request exceeds the IPC message limit.");
                }

                submitted = true;
                WriteFramedMessageAsync(client, payloadText, responseTimeout.Token).GetAwaiter().GetResult();

                if (!TryReadResponse(client, responseTimeout.Token, out SingleInstanceCommandResult response, out string normalizationMode))
                {
                ...
                LastSignalOutput = response.Output;
            ...
            {
                LastSignalFailureKind = submitted ? SingleInstanceSignalFailureKind.OutcomeUnknown : SingleInstanceSignalFailureKind.ConnectionFailed;
                if (_logger.IsEnabled(LogLevel.Warning))
                {
        ...
        {
            string? responseText = ReadFramedMessageAsync(client, MaxResponseBytes, cancellationToken).GetAwaiter().GetResult();
        ...
        private static async Task<string?> ReadFramedMessageAsync(Stream stream, int maxPayloadBytes, CancellationToken cancellationToken)
        {
