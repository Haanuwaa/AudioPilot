    ...
    public partial class SingleInstanceHelper : IDisposable, IAsyncDisposable
    {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        response = commandHandler == null
                            ? new SingleInstanceCommandResult(3, ErrorCode: "ui-host-unavailable", ErrorMessage: "No command handler is available.", ProtocolVersion: ResponseProtocolVersion)
                            : await commandHandler(payload).ConfigureAwait(false);
                    }
                    catch (Exception ex)
                    {
                        _logger.Warning("SingleInstanceHelper", "Command forwarding handler failed", nameof(ProcessConnectionAsync), ex);
        ...
        private bool SignalExistingInstance(string payloadText)
        {
            try
            {
                ...
                using var responseTimeout = new CancellationTokenSource(_responseTimeoutMs);
