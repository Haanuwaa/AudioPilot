...
namespace AudioPilot.Platform
{
    ...
    public partial class SingleInstanceHelper : IDisposable, IAsyncDisposable
    {
        ...
        private async Task ActivationListenerLoopAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        // Cancel closes the accepting handle before Dispose releases the mutex.
                        // Otherwise a new primary can inherit clients connected to this stale listener.
                        lock (_listenerAcceptLock)
                        {
                        ...
                        // Transfer stream ownership to the tracked request. Even a synchronous command
                        // handler must not prevent the listener from accepting an activation request.
                        NamedPipeServerStream connectedServer = server;
                        // Always start the owner, including during shutdown, so it disposes the stream.
            ...
            {
                // Keep the gate and listener token alive until every accepted request has drained.
        ...
        private async Task ProcessConnectionAsync(NamedPipeServerStream server, SemaphoreSlim commandGate, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (CancellationTokenSource readCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken))
                {
                    ...
                    try
                    {
                        payload = await ReadFramedMessageAsync(server, readCts.Token).ConfigureAwait(false);
                    }
                    catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
                    {
