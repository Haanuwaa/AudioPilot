...
namespace AudioPilot.Platform
{
    public static partial class MediaKeyHelper
    {
            ...
            int? ErrorCode = null)
        {
            ...
            public bool IsUnconfirmed => !Sent && SuppressFallback && FailureReason == "gsmc-command-timeout";
        }
        ...
        private static async Task<MediaCommandSendOutcome> TrySendSystemMediaCommandAsync(SystemMediaCommand command, string keyName, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                return await TrySendSystemMediaCommandCoreAsync(command, keyName, timeoutCts.Token, cancellationToken).ConfigureAwait(false);
            }
        ...
        private static async Task<MediaCommandSendOutcome> TrySendSystemMediaCommandCoreAsync(SystemMediaCommand command, string keyName, CancellationToken cancellationToken, CancellationToken callerCancellationToken)
        {
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                MediaCommandSendOutcome outcome;
                try
                {
                    outcome = await ResolveSubmittedCommandOutcomeAsync(
                        TrySendSystemMediaCommandAsync(candidate, command, cancellationToken),
                        BuildSystemMediaOutcome(false, candidate, false, null),
                ...
                if (outcome.SuppressFallback)
                {
                    GetLogger()?.Trace(
                        "MediaKeyHelper",
                        () => $"media-command-outcome-unknown:{keyName} candidate={candidate.Kind} source={GetSafeSessionSourceForLog(candidate.Session)} reason={outcome.FailureReason} fallbackSuppressed=true",
                        nameof(TrySendSystemMediaCommand));
                    return outcome;
                }
        ...
        /// <summary>Prevents a timed-out submitted command from being sent twice through another session or synthetic input.</summary>
        internal static async Task<MediaCommandSendOutcome> ResolveSubmittedCommandOutcomeAsync(
            Task<bool> commandTask,
            MediaCommandSendOutcome pendingOutcome,
            CancellationToken callerCancellationToken)
        {
            try
            {
                bool sent = await commandTask.ConfigureAwait(false);
                return pendingOutcome with { Sent = sent };
            }
            catch (OperationCanceledException) when (!callerCancellationToken.IsCancellationRequested)
            {
                return pendingOutcome with
                {
                    Sent = false,
                    SuppressFallback = true,
                    FailureReason = "gsmc-command-timeout",
                };
            }
        }
            ...
            string keyName)
        {
            ...
            try
            {
                ...
                string sessionDetails = logger.IsEnabled(LogLevel.Trace)
                    ? $" sessionSummary={BuildMediaCommandSessionSummary(sessions, command)}"
                    : string.Empty;
                ...
                logger.Debug(
                    "MediaKeyHelper",
                    () => $"media-command-routing-diagnostics | command={keyName} selectionReason={routeSelectionReason} nextRoute={nextRoute} sessions={sessions.Count} commandable={commandableCount} playing={playingCount} currentSource={currentSource} currentSupports={currentSupports} candidateRoutes={candidates.Count}{sessionDetails}",
                    nameof(TrySendSystemMediaCommand));
            }
            catch (Exception ex)
            {
