...
namespace AudioPilot.Platform
{
    ...
    internal partial struct NativePropVariant : IDisposable
    {
        ...
        [FieldOffset(8)] private CountedPointer _countedPointer;

        /// <summary>Sizes the enclosing PROPVARIANT to 16 bytes on x86 and 24 bytes on x64/ARM64.</summary>
        [StructLayout(LayoutKind.Sequential)]
        ...
        {
            if (vt == VT_CLSID && pointerValue != IntPtr.Zero)
            {
                value = Marshal.PtrToStructure<Guid>(pointerValue);
                return true;
            }
