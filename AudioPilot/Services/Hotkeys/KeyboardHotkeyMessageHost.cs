        ...
        Action<IntPtr, int> unregisterHotKeyInvoker) : IKeyboardHotkeyMessageHost
    {
        ...
        public HotkeyOsRegistrationResult RegisterHotkey(int id, uint fsModifiers, uint vk)
        {
            ...
            if (SendMessageTimeout(
                    ...
                    (IntPtr)id,
                    PackRegisterArguments(fsModifiers, vk),
        ...
        private static IntPtr PackRegisterArguments(uint fsModifiers, uint virtualKey)
            => new(unchecked((int)(((uint)checked((ushort)virtualKey) << 16) | checked((ushort)fsModifiers))));

        private static void UnpackRegisterArguments(IntPtr packedArgs, out uint fsModifiers, out uint virtualKey)
        {
            uint raw = unchecked((uint)packedArgs.ToInt64());
            fsModifiers = raw & ushort.MaxValue;
            virtualKey = raw >> 16;
        }
