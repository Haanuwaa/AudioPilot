...
namespace AudioPilot.Services.Hotkeys
{
    ...
    internal readonly record struct MouseHotkeyBindingSnapshot(
        ...
        Action? Callback,
        string Description,
        HotkeyHeldInputState? HoldState = null);
    ...
    internal sealed partial class LowLevelMouseHotkeyThreadHost : IMouseHotkeyCaptureHost
    {
        ...
        private readonly Dictionary<HotkeyMainInput, HotkeyHeldInputState> _heldInputs = [];
        ...
        private int _suppressedSideButtonUps;
        ...
        private void WorkerLoop(ThreadedWin32StartupSignal started, Action<Exception?> reportStartupFailure)
        {
            ...
            try
            {
                ...
                using var desktopMonitor = new HotkeyDesktopSwitchMonitor(() =>
                {
                    foreach (var held in _heldInputs.Values) held.Release();
                });
                lock (_stateLock)
                {
        ...
        private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            try
            {
                ...
                int sideButtonBit = mainInput.Kind == HotkeyMainInputKind.MouseButton
                    && mainInput.MouseButton is MouseButton.XButton1 or MouseButton.XButton2
                    ? 1 << (int)mainInput.MouseButton : 0;
                if (wParam.ToInt32() is 0x0202 or 0x0205 or 0x0208 or WM_XBUTTONUP)
                {
                    if (_heldInputs.Remove(mainInput, out var held))
                    {
                        held.Release();
                        _suppressedSideButtonUps &= ~sideButtonBit;
                        return (IntPtr)1;
                    }
                    if (wParam != (IntPtr)WM_XBUTTONUP) return CallNextHookEx(currentHookId, nCode, wParam, lParam);
                }
                if (wParam == (IntPtr)WM_XBUTTONUP)
                {
                    bool consumedDown = (_suppressedSideButtonUps & sideButtonBit) != 0;
                    _suppressedSideButtonUps &= ~sideButtonBit;
                    return consumedDown ? (IntPtr)1 : CallNextHookEx(currentHookId, nCode, wParam, lParam);
                }

                if (_heldInputs.ContainsKey(mainInput)) return (IntPtr)1;
                ...
                if (binding.HoldState is { } hold)
                {
                    _heldInputs[mainInput] = hold;
                    if (!hold.Press()) return (IntPtr)1;
                    binding = binding with { Callback = hold.CapturePressCallback() };
                }
            ...
            {
                TryLogHookCallbackFailure(ex);
                return TryCallNextHook(currentHookId, nCode, wParam, lParam);
            }
        }

        internal IntPtr InvokeHookCallbackForTests(int nCode, IntPtr wParam, IntPtr lParam)
            => HookCallback(nCode, wParam, lParam);

        private void TryLogHookCallbackFailure(Exception exception)
        {
            try
            {
                _logger.Error(
                    "HotkeyService",
                    () => $"mouse-hook-callback-failed | exceptionType={exception.GetType().Name}");
            }
            catch (Exception)
            {
            }
        ...
        private static IntPtr TryCallNextHook(IntPtr hookId, int nCode, IntPtr wParam, IntPtr lParam)
        {
            try
            {
        ...
        internal static bool TryParseMouseHookInput(IntPtr wParam, IntPtr lParam, out HotkeyMainInput mainInput)
        {
            ...
            {
                WM_LBUTTONDOWN or 0x0202 => HotkeyMainInput.FromMouseButton(MouseButton.Left),
                WM_RBUTTONDOWN or 0x0205 => HotkeyMainInput.FromMouseButton(MouseButton.Right),
                WM_MBUTTONDOWN or 0x0208 => HotkeyMainInput.FromMouseButton(MouseButton.Middle),
            ...
            if (message is not (WM_XBUTTONDOWN or WM_XBUTTONUP or WM_MOUSEWHEEL or WM_MOUSEHWHEEL))
            {
                return false;
            }
            ...
            mainInput = message switch
            {
                WM_XBUTTONDOWN or WM_XBUTTONUP => ((hookData.MouseData >> 16) & 0xFFFF) switch
                {
                ...
                WM_MOUSEWHEEL => ResolveWheelInput(hookData.MouseData, horizontal: false),
                WM_MOUSEHWHEEL => ResolveWheelInput(hookData.MouseData, horizontal: true),
                _ => HotkeyMainInput.None,
            };
        ...
        private static HotkeyMainInput ResolveWheelInput(uint mouseData, bool horizontal)
        {
            short delta = unchecked((short)((mouseData >> 16) & ushort.MaxValue));
            if (delta == 0)
            {
                return HotkeyMainInput.None;
            }

            if (horizontal)
            {
                return delta > 0 ? HotkeyMainInput.WheelRight : HotkeyMainInput.WheelLeft;
            }

            return delta > 0 ? HotkeyMainInput.WheelUp : HotkeyMainInput.WheelDown;
        }
