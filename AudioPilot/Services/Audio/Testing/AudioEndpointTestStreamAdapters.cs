...
internal interface IAudioTestPlayerAdapter : IAsyncDisposable
{
    ...
    string? LowLatencyUnavailableReason { get; }
...
{
    /// <summary>Creates an initialized, stopped player. The caller owns playback startup and all returned resources.</summary>
...
internal sealed class WasapiTestPlayerAdapter(WasapiPlayer player) : IAudioTestPlayerAdapter
{
    ...
    public string? LowLatencyUnavailableReason => player.LowLatencyUnavailableReason;
...
internal sealed class WasapiTestRecorderAdapter(WasapiRecorder recorder) : IAudioTestRecorderAdapter
{
    ...
    public string? LowLatencyUnavailableReason => recorder.LowLatencyUnavailableReason;
        ...
        return await Task.Run(async () =>
        {
            ...
            try
            {
                ...
                if (monitorEndpoint is { Id.Length: > 0 } explicitEndpoint)
                {
                    ...
                    player = builder.WithDevice(device).WithLowLatency().Build();
                }
                else
                {
