...
namespace AudioPilot.Services.Audio
{
    internal sealed class AudioDeviceEndpointQueryHelper(
        ...
        Action<string, Func<string>> logDebugWithCooldown)
    {
        ...
        private readonly Action<string, Func<string>> _logDebugWithCooldown = logDebugWithCooldown;

        internal AudioStatusSnapshot GetAudioStatus()
        {
            _enumeratorLock.EnterReadLock();
            try
            {
                ObjectDisposedException.ThrowIf(_isDisposed(), typeof(AudioDeviceService));
                return CaptureAudioStatus(
                    () => ReadEndpointStatus(DataFlow.Render, AudioDeviceService.ResolveDetectionRole(_getConfiguredOutputRolesSnapshot(), NRole.Multimedia)),
                    () => ReadEndpointStatus(DataFlow.Capture, AudioDeviceService.ResolveDetectionRole(_getConfiguredInputRolesSnapshot(), NRole.Console)),
                    (target, ex) => _logger.Warning("AudioDeviceService", $"audio-status-read-failed | target={target} error={ex.GetType().Name} hresult=0x{ex.HResult:X8}"));
            }
            finally
            {
        ...
        private AudioEndpointStatus ReadEndpointStatus(DataFlow flow, NRole role)
        {
            using MMDevice? device = _enumerator.GetDefaultAudioEndpoint(flow, role);
            if (device == null)
            {
                return new(AudioEndpointStatusKind.NoDevice);
            }

            string name = device.FriendlyName;
            float percent = device.AudioEndpointVolume.MasterVolumeLevelScalar * 100f;
            bool muted = device.AudioEndpointVolume.Mute;
            return new(AudioEndpointStatusKind.Available, name, float.IsFinite(percent) ? Math.Clamp(percent, 0f, 100f) : null, muted);
        }

        /// <summary>Reads endpoints independently so a missing or failing device cannot conceal the other endpoint's state.</summary>
        internal static AudioStatusSnapshot CaptureAudioStatus(
            Func<AudioEndpointStatus> readOutput,
            Func<AudioEndpointStatus> readInput,
            Action<string, Exception> reportFailure)
        {
            return new(Read(readOutput, "output"), Read(readInput, "input"));

            AudioEndpointStatus Read(Func<AudioEndpointStatus> read, string target)
            {
                try
                {
                    return read();
                ...
                {
                    reportFailure(target, ex);
                    return new(AudioEndpointStatusKind.Unavailable);
                }
            }
        }
        ...
        internal MMDeviceCollection GetActivePlaybackDevices()
        {
            ...
            try
            {
                ...
                _logDebugWithCooldown("EnumeratePlaybackDevices", () => $"Enumerated {devices.Count} active playback devices");
                return devices;
            }
        ...
        internal IReadOnlyList<MMDevice> GetPlaybackDevicesById(IReadOnlyCollection<string> deviceIds)
        {
            ...
            try
            {
                ...
                _logDebugWithCooldown("GetPlaybackDevicesById", () => $"Materialized {devices.Count} playback devices by id");
                return devices;
            }
            finally
            {
        ...
        internal MMDeviceCollection GetActiveCaptureDevices()
        {
            ...
            try
            {
                ...
                _logDebugWithCooldown("EnumerateCaptureDevices", () => $"Enumerated {devices.Count} active capture devices");
                return devices;
            }
            catch (COMException ex)
            {
        ...
        internal MMDevice? TryGetDeviceById(string deviceId, DataFlow? expectedFlow = null)
        {
            ...
            try
            {
                ...
                if (expectedFlow.HasValue && device.DataFlow != expectedFlow.Value)
                {
                    return null;
                }
            ...
            {
                try { device?.Dispose(); }
                finally { _enumeratorLock.ExitReadLock(); }
            }
        }
    }
}
