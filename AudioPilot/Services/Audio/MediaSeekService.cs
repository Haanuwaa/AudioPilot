...
namespace AudioPilot.Services.Audio
{
    ...
    {
        Task<bool> IsPresentAsync(CancellationToken cancellationToken);
        object Identity { get; }
        ...
        int timeoutMs = 2000)
    {
        ...
        private IMediaSeekSession? _submittedSession;
        private PendingPosition? _submittedPosition;
        ...
        public async Task<MediaSeekResult> SeekAsync(int offsetSeconds, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    if (_submittedSession == null || await _submittedSession.IsPresentAsync(deadline.Token).WaitAsync(deadline.Token).ConfigureAwait(false))
                    {
                        return new(false, "media-seek-busy", "Previous seek is still pending");
                    }

                    _submittedCommand = null;
                    _submittedPosition = null;
                    InvalidatePosition();
                    logger?.Info("MediaSeekService", "media-seek-pending-retired | reason=session-removed");
                }
                _submittedSession = null;

                if (_submittedCommand is { IsCompletedSuccessfully: true, Result: true } &&
                    _submittedPosition is { } completedPosition && completedPosition.Generation == Volatile.Read(ref _generation))
                {
                    _pending = completedPosition;
                }
                _submittedCommand = null;
                _submittedPosition = null;
                ...
                _submittedSession = session;
                _submittedPosition = new(session.Identity, timeline, target, now, evidenceExpiresAt, generation);
                submitted = true;
                ...
                deadline.Token.ThrowIfCancellationRequested();
                if (accepted) _pending = _submittedPosition;
                _submittedSession = null;
                _submittedCommand = null;
                _submittedPosition = null;
