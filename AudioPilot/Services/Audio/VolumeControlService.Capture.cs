            ...
            try
            {
                ...
                for (int i = 0; i < sessionCount; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (pid != 0)
                        {
                            _cacheStore.UpdateCache(name ?? processName ?? string.Empty, processName, volPercent);
                        }

                        if (!string.IsNullOrWhiteSpace(name))
                        {
                            string normalizedAppName = _snapshotMatcher.NormalizeForMatching(name);
                            if (!byName.ContainsKey(normalizedAppName))
                            {
                                byName[normalizedAppName] = volPercent;
                                VolumeSnapshotMatcher.IndexNormalizedWords(normalizedAppName, wordIndex);
                            }
                        }
                    }
                    catch (COMException ex)
