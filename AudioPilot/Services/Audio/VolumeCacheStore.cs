...
namespace AudioPilot.Services.Audio
{
        ...
        Func<long>? timestampTicksProvider = null)
    {
        ...
        public void UpdateCache(string displayName, string? processName, float volume)
        {
            ...
            var entry = new VolumeCacheEntry(volume, _timestampTicksProvider());
                ...
                displayName != "System Sounds")
            {
                ...
                if (!string.IsNullOrEmpty(normalized))
                {
                    ...
                    _volumeCacheAliases.TryRemove(canonicalKey, out _);
        ...
        public float? TryGetCachedVolume(string normalizedKey)
        {
            ...
            {
                if (_appVolumeCache.TryGetValue(canonicalKey, out var aliasedEntry))
                {
                    if ((nowTicks - aliasedEntry.TimestampTicks) < _volumeCacheTtl.Ticks)
                    {
                        return aliasedEntry.Volume;
                    }

                    _appVolumeCache.TryRemove(new KeyValuePair<string, VolumeCacheEntry>(canonicalKey, aliasedEntry));
                }

                _volumeCacheAliases.TryRemove(new KeyValuePair<string, string>(normalizedKey, canonicalKey));
                return null;
            }
            ...
            if (_appVolumeCache.TryGetValue(normalizedKey, out var entry))
            {
                ...
                _appVolumeCache.TryRemove(new KeyValuePair<string, VolumeCacheEntry>(normalizedKey, entry));
            }
