        ...
        public void ApplySavedVolume(AudioSessionControl session, string processName, string displayName)
        {
            ...
            if (pendingSnapshot != null)
            {
                    ...
                    out string pendingMethod))
                {
                    ...
                    _cacheStore.UpdateCache(lookupName, processName, savedVolume.Value);
                }
            }
            ...
            if (savedVolume == null && !string.IsNullOrWhiteSpace(lookupName))
            {
                ...
                savedVolume = _cacheStore.TryGetCachedVolume(normalizedName);
            ...
            {
                savedVolume = _cacheStore.TryGetCachedVolume(processNameNormalized);
                if (savedVolume.HasValue)
                {
                    if (_logger.IsEnabled(LogLevel.Trace))
                    {
                ...
                else
                {
                    ...
                    {
                        savedVolume = _cacheStore.TryGetCachedVolume(normalizedSanitized);
                        if (savedVolume.HasValue)
                        {
                            if (_logger.IsEnabled(LogLevel.Trace))
                            {
