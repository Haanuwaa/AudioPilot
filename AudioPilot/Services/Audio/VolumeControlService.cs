...
namespace AudioPilot.Services.Audio
{
    ...
    public partial class VolumeControlService : IDisposable
    {
        ...
        private readonly VolumeCacheStore _recordingCacheStore;
            ...
            Func<long, bool> isCacheEntryExpired)
        {
            ...
            _retryStateTracker = new VolumeRetryStateTracker(
                _retryStateTtl,
                TimeSpan.FromSeconds(AppConstants.Timing.CircuitBreakerCooldownSeconds));
            _cacheStore = new VolumeCacheStore(
                _logger,
                _normalizedNameCache,
                _snapshotMatcher.NormalizeForMatching,
                _volumeCacheTtl,
                MaxVolumeCacheEntries,
                MaxVolumeAliasEntries,
                MaxNormalizedNameCacheEntries);
            _recordingCacheStore = new VolumeCacheStore(
                _logger,
                _normalizedNameCache,
                _snapshotMatcher.NormalizeForMatching,
                _volumeCacheTtl,
        ...
        public void ApplySavedVolume(AudioSessionControl session, string processName, string displayName, AudioMixerMode mixerMode)
        {
            if (session == null || _disposed) return;

            VolumeCacheStore cache = mixerMode == AudioMixerMode.Input ? _recordingCacheStore : _cacheStore;
            string lookupName = displayName;
            string processNameNormalized = _snapshotMatcher.NormalizeForMatching(processName);
            ...
            SessionVolumeSnapshot? pendingSnapshot = mixerMode == AudioMixerMode.Output ? GetPendingSnapshotForCurrentPlaybackDevice() : null;
            if (pendingSnapshot != null)
            {
                string normalizedLookupName = _snapshotMatcher.NormalizeForMatching(lookupName);
                    ...
                    out string pendingMethod))
                {
                    ...
                    {
                        _logger.Trace("VolumeControlService", () => $"{AppConstants.Audio.LogEvents.Volume.SavedVolumeMatch} | mode={mixerMode} method=pending-{pendingMethod} key={LogPrivacy.Label(normalizedLookupName)} volume={savedVolume.Value}%");
                    }
                    cache.UpdateCache(lookupName, processName, savedVolume.Value);
                }
            }

            if (savedVolume == null && !string.IsNullOrWhiteSpace(lookupName))
            {
                string normalizedName = _snapshotMatcher.NormalizeForMatching(lookupName);
                savedVolume = cache.TryGetCachedVolume(normalizedName);
                if (savedVolume.HasValue)
                {
                    if (_logger.IsEnabled(LogLevel.Trace))
                    {
                        _logger.Trace("VolumeControlService", () => $"{AppConstants.Audio.LogEvents.Volume.SavedVolumeMatch} | mode={mixerMode} method=display-name key={LogPrivacy.Label(normalizedName)} volume={savedVolume.Value}%");
                    }
                }
            }
            ...
            {
                savedVolume = cache.TryGetCachedVolume(processNameNormalized);
                if (savedVolume.HasValue)
                {
                    if (_logger.IsEnabled(LogLevel.Trace))
                    {
                        _logger.Trace("VolumeControlService", () => $"{AppConstants.Audio.LogEvents.Volume.SavedVolumeMatch} | mode={mixerMode} method=process-name key={LogPrivacy.Label(processNameNormalized)} volume={savedVolume.Value}%");
                    }
                }
                else
                {
                    string sanitized = AudioDeviceHelper.SanitizeProcessName(processName);
                    string normalizedSanitized = _snapshotMatcher.NormalizeForMatching(sanitized);
                    if (normalizedSanitized != processNameNormalized)
                    {
                        savedVolume = cache.TryGetCachedVolume(normalizedSanitized);
                        if (savedVolume.HasValue)
                        {
                            if (_logger.IsEnabled(LogLevel.Trace))
                            {
                                _logger.Trace("VolumeControlService", () => $"{AppConstants.Audio.LogEvents.Volume.SavedVolumeMatch} | mode={mixerMode} method=sanitized-process key={LogPrivacy.Process(sanitized)} volume={savedVolume.Value}%");
                            }
                        }
                    }
                }
                    ...
                    if (AudioDeviceHelper.TryGetSessionVolume(_logger, session, out float currentVol))
                    {
                        ...
                        if (Math.Abs(currentVolPercent - savedVolume.Value) > 1.0f)
                        {
                            if (AudioDeviceHelper.TrySetSessionVolume(_logger, session, savedVolume.Value / 100f))
                            {
                                ...
                                {
                                    _logger.Info("VolumeControlService", () => $"{AppConstants.Audio.LogEvents.Volume.RestoreSavedVolumeSuccess} | mode={mixerMode} displayName={LogPrivacy.Session(lookupName)} processName={LogPrivacy.Process(processName)} targetVolume={savedVolume.Value:F0}%");
                                }
                            }
                        }
                    }
            ...
            SessionVolumeApplicationCandidate candidate)
        {
            ...
            string normalizedDisplayName = string.IsNullOrWhiteSpace(candidate.DisplayName)
                ? string.Empty
                : _snapshotMatcher.NormalizeForMatching(candidate.DisplayName);
            string normalizedProcessName = string.IsNullOrWhiteSpace(candidate.ProcessName)
                ? string.Empty
                : _snapshotMatcher.NormalizeForMatching(candidate.ProcessName);
        ...
        public void UpdateVolume(AudioSessionInfo sessionInfo)
        {
            ...
            {
                _cacheStore.UpdateCache(sessionInfo.DisplayName, sessionInfo.ProcessName, sessionInfo.Volume);
            }
        ...
        public void UpdateSessionVolume(string displayName, string? processName, float volume, AudioMixerMode mixerMode = AudioMixerMode.Output)
        {
            if (_disposed) return;
            (mixerMode == AudioMixerMode.Input ? _recordingCacheStore : _cacheStore).UpdateCache(displayName, processName, volume);
        }
            ...
            bool applyMicVolume = true)
        {
            ...
            if (!shouldContinue() || _retryStateTracker.IsCircuitOpen(targetDeviceId))
            {
                return;
            }
            ...
            try
            {
                playbackDevice = enumerator.GetDevice(targetDeviceId);
                if (playbackDevice == null || playbackDevice.State != DeviceState.Active || playbackDevice.DataFlow != DataFlow.Render)
                {
                    playbackDevice?.Dispose();
                    if (_logger.IsEnabled(LogLevel.Warning))
            ...
            {
                playbackDevice?.Dispose();
                _logger.Debug("VolumeControlService", () => $"{AppConstants.Audio.LogEvents.Volume.ApplySessionVolumesSimpleSkip} | reason=target-unavailable exception={ex.GetType().Name}");
                return;
            }
            ...
            try
            {
                ...
                if (applyMasterVolume && snapshot.MasterVolumePercent.HasValue)
                {
                    if (AudioDeviceHelper.TryGetEndpointVolume(_logger, playbackDevice, out var playbackVolume, "apply-session-volumes-simple:playback-endpoint"))
                    {
                        ...
                        catch (COMException ex)
                        {
                            ...
                            _retryStateTracker.RecordFailure(playbackDevice.ID);
                            throw;
                        }
                    }
                }
                ...
                if (!shouldContinue()) return;
                if (applyMicVolume && snapshot.MicVolumePercent.HasValue && !string.IsNullOrEmpty(recordingDeviceId))
                {
                    try
                    {
                        recordingDevice = enumerator.GetDevice(recordingDeviceId);
                    ...
                    catch (COMException ex)
                    {
                        ...
                        if (recordingDevice != null)
                        {
                            _retryStateTracker.RecordFailure(recordingDevice.ID);
                        }
                        throw;
                    }
                }
                    ...
                    try
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(displayName))
                        {
                            ...
                            if (cachedEntry.HasValue)
                            {
                                ...
                                normalizedProcessName = _snapshotMatcher.NormalizeForMatching(cachedEntry.Value.ProcessName);
                            }
                        }

                        if (!string.IsNullOrWhiteSpace(displayName))
                        {
                            normalizedDisplayName = _snapshotMatcher.NormalizeForMatching(displayName);
                        }
            ...
            finally
            {
                ...
                {
                    _retryStateTracker.Reset(playbackDevice.ID);
                }
                playbackDevice?.Dispose();
        ...
        internal void CleanupExpiredRetryStates()
        {
            ...
            VolumeCacheCleanupResult playback = _cacheStore.CleanupExpiredEntries();
            VolumeCacheCleanupResult recording = _recordingCacheStore.CleanupExpiredEntries();
            var cleanupResult = new VolumeCacheCleanupResult(playback.ExpiredVolumeEntries + recording.ExpiredVolumeEntries,
                playback.ExpiredAliases + recording.ExpiredAliases);
