...
namespace AudioPilot.Services.UI
{
    ...
    public class OverlayService : IDisposable
    {
        ...
        internal interface IOverlayPresenter : IDisposable
        {
            ...
            void SetIcon(OverlayIcon icon);
            void UpdateContent(string message);
            void UpdateContent(OverlayActionStateKind stateKind, string message, string? detail = null);
            ...
            void UpdateAudioStatusContent(AudioStatusSnapshot snapshot);
            ...
            void UpdateContent(string header, string title, string? artist, string? detail = null);
            void ApplyDisplayOptions(OverlayPosition position, double durationSeconds, int stackIndex, int? stackOffsetPixels = null);
            void ShowOverlay();
        }

        /// <summary>Owns a reusable overlay without assigning it as the application's main window; disposal also closes unshown preparations.</summary>
        private sealed class OverlayWindowPresenter : IOverlayPresenter
        {
            private readonly OverlayWindow _window;

            public OverlayWindowPresenter(string initialMessage)
            {
                Application? application = Application.Current;
                Window? mainWindow = application?.MainWindow;
                _window = new OverlayWindow(initialMessage);
                if (application != null && mainWindow == null && ReferenceEquals(application.MainWindow, _window))
                {
            ...
            public void SetIcon(OverlayIcon icon) => _window.SetIcon(icon);
            public void UpdateContent(string message) => _window.UpdateContent(message);
            public void UpdateContent(OverlayActionStateKind stateKind, string message, string? detail = null) => _window.UpdateContent(stateKind, message, detail);
            ...
            public void UpdateAudioStatusContent(AudioStatusSnapshot snapshot) => _window.UpdateAudioStatusContent(snapshot);
            ...
            public void UpdateContent(string header, string title, string? artist, string? detail = null) => _window.UpdateContent(header, title, artist, detail);
        ...
        private long _overlayEnabledState;
        ...
        public OverlayService(bool initiallyEnabled = true)
            : this(DispatchOnCurrentApplication, initialMessage => new OverlayWindowPresenter(initialMessage), initiallyEnabled)
        {
        }

        internal OverlayService(Action<Action> dispatch, Func<string, IOverlayPresenter> presenterFactory, bool initiallyEnabled = true)
        {
            _logger = Logger.Instance;
            _dispatch = dispatch;
            _presenterFactory = presenterFactory;
            _overlayEnabledState = initiallyEnabled ? 1L : 0L;
        }

        internal bool HasPresenterForTests => _overlayPresenters.Count > 0;

        /// <summary>
        /// Prepares a reusable presenter without showing it. Disabled or superseded requests create nothing.
        /// </summary>
        internal void PrepareForFirstUse()
        {
            _ = TryDispatch(() =>
            {
                try
                {
                    _ = EnsurePresenter(0, string.Empty);
                }
                catch (Exception ex)
                {
                    _logger.Debug("OverlayService", () => $"overlay-prepare-failed | error={ex.GetType().Name}", nameof(PrepareForFirstUse));
                }
            });
        }
        ...
        public void Show(string message, Func<bool>? isCurrent = null, OverlayIcon icon = OverlayIcon.None)
        {
            if (!IsOverlayEnabled())
            {
                return;
            ...
            if (!TryDispatch(() =>
            {
                try
                {
                    ...
                    using var trace = OperationTrace.Start("overlay-presentation", _logger);
        ...
        public void Show(OverlayActionStateKind stateKind, string message, string? detail = null, OverlayIcon icon = OverlayIcon.None)
        {
            if (!IsOverlayEnabled())
            {
                return;
            ...
            if (!TryDispatch(() =>
            {
                try
                {
                    ...
                    presenter.UpdateContent(stateKind, message, detail);
                    presenter.SetIcon(icon);
                    using var trace = OperationTrace.Start("overlay-presentation", _logger);
                    presenter.ShowOverlay();
                    trace.Complete();
        ...
        public void Show(OverlayDeviceKind kind, string header, string deviceName, OverlayIcon? icon = null)
        {
            if (!IsOverlayEnabled())
            {
                return;
            ...
            if (!TryDispatch(() =>
            {
                try
                {
                    ...
                    if (icon.HasValue) presenter.SetIcon(icon.Value);
                    using var trace = OperationTrace.Start("overlay-presentation", _logger);
                    presenter.ShowOverlay();
                    trace.Complete();
                    ReleaseUnusedPresenters(1);
        ...
        public void ShowAudioStatus(AudioStatusSnapshot snapshot, Func<bool>? isCurrent = null)
        {
            if (!IsOverlayEnabled())
            {
                return;
            ...
            _ = TryDispatch(() =>
            {
                ...
                {
                    if (isCurrent != null && !isCurrent()) return;
                    IOverlayPresenter presenter = EnsurePresenter(0, "Audio status");
                    presenter.ApplyDisplayOptions(_overlayPosition, _overlayDurationSeconds, 0);
                    presenter.UpdateAudioStatusContent(snapshot);
                    using var trace = OperationTrace.Start("overlay-presentation", _logger);
                    presenter.ShowOverlay();
                    trace.Complete();
                    ReleaseUnusedPresenters(1);
                ...
                {
                    _logger.Error("OverlayService", () => $"overlay-show-failed | mode=audio-status error={ex.GetType().Name}", nameof(ShowAudioStatus), ex);
                }
            });
        }
        ...
        public void ShowMediaTrack(string header, string title, string? artist, string? detail = null, Func<bool>? isCurrent = null, OverlayIcon icon = OverlayIcon.Media)
        {
            if (!IsOverlayEnabled())
            {
                return;
            ...
            if (!TryDispatch(() =>
            {
                try
                {
                    ...
                    presenter.UpdateContent(header, title, artist, detail);
                    presenter.SetIcon(icon);
                    using var trace = OperationTrace.Start("overlay-presentation", _logger);
                    presenter.ShowOverlay();
                    trace.Complete();
        ...
        private bool TryDispatch(Action action)
        {
            ...
            try
            {
                _dispatch(() =>
                {
                    using var context = OperationTrace.Attach(traceId);
                    if (Volatile.Read(ref _disposed) == 0 &&
        ...
        internal bool IsOverlayEnabled() => Volatile.Read(ref _disposed) == 0 && IsOverlayEnabled(Volatile.Read(ref _overlayEnabledState));
