    ...
    internal static class MediaOverlayMessageFormatter
    {
        ...
        internal static MediaOverlayResult BuildOverlayMessage(MediaOverlayCommand command, SessionSnapshot baseline, SnapshotCaptureResult capture)
        {
            ...
            return command switch
            {
                ...
                MediaOverlayCommand.NextTrack => BuildTrackMessage(snapshot, baseline, capture.RecoveryDisposition, "Next track", "Next track unchanged", "Next track loading", "Next track metadata loading", OverlayIcon.Next),
                MediaOverlayCommand.PreviousTrack => BuildTrackMessage(snapshot, baseline, capture.RecoveryDisposition, "Previous track", "Previous track unchanged", "Previous track loading", "Previous track metadata loading", OverlayIcon.Previous),
        ...
        internal static MediaOverlayResult BuildPlayPauseMessage(SessionSnapshot snapshot, SessionSnapshot baseline, bool hasOtherSessionContext = false)
        {
            ...
            OverlayIcon icon = (statusChanged ? snapshot.PlaybackStatus : null) switch
            {
                GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing => OverlayIcon.Play,
                GlobalSystemMediaTransportControlsSessionPlaybackStatus.Paused => OverlayIcon.Pause,
            ...
            if (MediaOverlayEngine.HasTrackData(effectiveSnapshot) && hasEvidence)
            {
                ...
                return MediaOverlayResult.Track(header, title, effectiveSnapshot.Artist, icon);
            }

            return MediaOverlayResult.Plain(header, icon);
        }
        ...
        private static MediaOverlayResult BuildTrackMessage(
            ...
            string metadataPendingFallback, OverlayIcon icon)
        {
            if (MediaOverlayEngine.IsSessionMissing(snapshot) && MediaOverlayEngine.IsSessionMissing(baseline))
            {
                Logger.Instance?.Debug(
            ...
            return MediaOverlayResult.Track(confirmedHeader, title, snapshot.Artist, icon);
        }
    }
}
