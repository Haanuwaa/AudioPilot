...
namespace AudioPilot.Services.UI.MediaOverlay
{
    ...
    public readonly record struct MediaOverlayResult(
        ...
        string? Artist)
    {
        public OverlayIcon Icon { get; init; }
        ...
        public static MediaOverlayResult Plain(string message, OverlayIcon icon = OverlayIcon.Information) => new(MediaOverlayResultKind.PlainMessage, string.Empty, message, null, null) { Icon = icon };

        public static MediaOverlayResult Track(string header, string title, string? artist, OverlayIcon icon = OverlayIcon.Media) => new(MediaOverlayResultKind.TrackMessage, header, null, title, artist) { Icon = icon };
    }
        ...
        BrowserSameSourceWinnerElectionResult WinnerElection = default)
    {
        ...
        public bool HasUncontestedPendingCandidate => DistinctCandidateCount == 1
            && HasPendingCandidateEvidence
            && !HasPendingNonWinnerRivalEvidence
            && !HasBlockedRivalEvidence
            && !ConflictObserved
            && !ConflictActive
            && ActiveRivalCount == 0
            && ReinforcedRivalCount == 0
            && !WinnerElection.HasWinner;
    }
