...
namespace AudioPilot.Services.UI.MediaOverlay
{
        ...
        Func<DateTimeOffset>? utcNow = null)
    {
        ...
        private readonly Func<DateTimeOffset> _utcNow = utcNow ?? (static () => DateTimeOffset.UtcNow);
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                stateMachine.Complete(TrackNavigationRecoveryOutcome.Changed);
            ...
            if (sawSessionDrop)
            {
                stateMachine.AdvanceTo(TrackNavigationRecoveryPhase.SessionDropRecovery);
            ...
            if (!MediaOverlayEngine.IsSessionMissing(fallback) && MediaOverlayEngine.HasTrackData(fallback))
            {
                ...
                if (hasObservedTransition)
                {
                    ...
                    string finalPhase = stateMachine.CurrentPhase switch
                    {
                        TrackNavigationRecoveryPhase.SessionDropRecovery => "session-drop-recovery",
                        TrackNavigationRecoveryPhase.UnchangedRecovery => "unchanged-track-recovery",
                        _ => "initial-preferred-source-sampling",
                    };
                    stateMachine.Complete(TrackNavigationRecoveryOutcome.Changed);
                ...
                && sameSourceCommandSummary.ConflictObserved)
            {
                Logger.Instance?.Debug(
                    "MediaOverlayHelper",
                    () => BuildUnresolvedSameSourceConflictLog(recoverySource, sameSourceCommandSummary),
                    nameof(CaptureSnapshotWithRetryAsync));
            }
            RecordDiagnostics(
                "final-fallback",
                ...
                context.CommandSequence,
                Stopwatch.GetElapsedTime(recoveryStartTimestamp).TotalMilliseconds);
            stateMachine.Complete(recoveryDisposition.Outcome);
            ...
            CancellationToken cancellationToken)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts; attempt++)
            {
                ...
                if (IsConfirmedOrCommittedTrackNavigationTransition(context.Command, context.Baseline, latest))
                {
                    LogTrackNavigationRecoveryPhase(
                        "initial-preferred-source-sampling",
                        context,
            ...
            double elapsedMs)
        {
            _recordDiagnostics?.Invoke(new MediaOverlayTrackNavigationDiagnostics(
                finalPhase,
                outcome,
                outcome == "changed" ? DescribeChangeKind(finalChangeKind) : "none",
                sawSessionDrop,
                usedSessionDropRecovery,
                usedLateTrackLoadRecovery,
                usedRecoveredAlternateSource,
        ...
        /// <summary>
        /// Samples delayed track metadata and reserves one bounded confirmation for a browser candidate first seen
        /// on the final sample. All waits, including confirmation, remain subject to the command deadline and cancellation.
        /// </summary>
        private async Task<SessionSnapshot> TryRecoverAfterTrackLoadAsync(
            ...
            CancellationToken cancellationToken)
        {
            MediaOverlayDelayAssistResult trackLoadDelayResult = await _delayWithEventAssistIfWithinBudgetAsync(
                _timingProfile.TrackLoadRecoveryInitialDelayMs, preferredSourceAppUserModelId, deadlineUtc, commandSequence, cancellationToken);
            ...
            SessionSnapshot latest = SessionSnapshot.Empty;
            SessionSnapshot lastNonMissing = SessionSnapshot.Empty;
            int attempts = _timingProfile.TrackLoadRecoveryAttempts;
            bool confirmLateCandidate = false;
            for (int attempt = 0; attempt < attempts || (attempt == attempts && confirmLateCandidate); attempt++)
            {
                if (attempt > 0)
                {
                    int retryDelayMs = MediaOverlayBrowserPendingCorroborationStrategy.ResolveRetryDelayMs(
                        _timingProfile.TrackLoadRecoveryRetryDelayMs, confirmLateCandidate);
                ...
                confirmLateCandidate = MediaOverlayEngine.IsSessionMissing(latest)
                    && _browserPendingCorroborationStrategy.ShouldExpediteLatePendingRetry(
                        preferredSourceAppUserModelId, baseline, commandSequence);
            }

            return !MediaOverlayEngine.IsSessionMissing(lastNonMissing) ? lastNonMissing : latest;
        }
        ...
        internal static string BuildUnresolvedSameSourceConflictLog(
            string? recoverySource,
            BrowserSameSourceCommandSummary summary)
        {
            return $"same-source conflict unresolved: source={LogPrivacy.Id(recoverySource)} rivals={summary.RivalReasonClasses} activeRivals={summary.ActiveRivalCount} reinforcedRivals={summary.ReinforcedRivalCount} staleRivals={summary.StaleRivalCount} winnerPresent={summary.WinnerElection.HasWinner}";
        }
