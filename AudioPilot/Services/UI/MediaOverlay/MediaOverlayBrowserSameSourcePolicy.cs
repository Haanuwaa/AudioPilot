...
namespace AudioPilot.Services.UI.MediaOverlay
{
    internal static class MediaOverlayBrowserSameSourcePolicy
    {
        ...
        {
            return AudioPilot.Platform.BrowserMediaSourceResolver.Installed.IsBrowserSource(sourceAppUserModelId);
        }
        ...
        /// <summary>Prefixes field lengths so punctuation in media metadata cannot merge distinct candidates.</summary>
        internal static string BuildPendingCandidateFingerprint(SessionSnapshot snapshot)
        {
            ...
            return string.Create(CultureInfo.InvariantCulture,
                $"{source.Length}:{source}{title.Length}:{title}{artist.Length}:{artist}{status.Length}:{status}{bucket}");
        }
        ...
        internal static string BuildPendingCandidateTrackFingerprint(SessionSnapshot snapshot)
        {
            ...
            return string.Create(CultureInfo.InvariantCulture,
                $"{source.Length}:{source}{title.Length}:{title}{artist.Length}:{artist}{album.Length}:{album}");
        }
