namespace AudioPilot.Services.UI.MediaOverlay
{
    internal sealed class MediaOverlayCommandEventWaiter : IDisposable
    {
        ...
        {
            if (maxWaitMs <= 0 || cancellationToken.IsCancellationRequested)
            {
            ...
            try
            {
                return await registration.Completion.Task.WaitAsync(
                    TimeSpan.FromMilliseconds(maxWaitMs), cancellationToken).ConfigureAwait(false);
            }
        ...
        /// <summary>Consumes one source's queued burst together so stale notifications cannot exhaust later sampling retries.</summary>
        private MediaEventAssistOutcome? TryTakeObservedEventUnderLock(string? preferredSourceAppUserModelId)
        {
            MediaEventAssistOutcome? newest = null;
            for (int index = _observedEvents.Count - 1; index >= 0; index--)
            {
                ...
                if (IsRelevantForPreferredSource(observed, preferredSourceAppUserModelId)
                    && (!newest.HasValue || string.Equals(observed.SignaledSourceAppUserModelId,
                        newest.Value.SignaledSourceAppUserModelId, StringComparison.OrdinalIgnoreCase)))
                {
                    newest ??= observed;
