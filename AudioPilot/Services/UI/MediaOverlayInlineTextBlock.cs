...
namespace AudioPilot.Services.UI
{
    public sealed class MediaOverlayInlineTextBlock : FrameworkElement
    {
        ...
        private IOverlayEmojiImageSourceFactory _emojiImageSourceFactory;
        ...
        internal void SetEmojiImageSourceFactory(IOverlayEmojiImageSourceFactory emojiImageSourceFactory)
        {
            _emojiImageSourceFactory = emojiImageSourceFactory ?? throw new ArgumentNullException(nameof(emojiImageSourceFactory));
            InvalidateMeasure();
            InvalidateVisual();
        ...
        internal IOverlayEmojiImageSourceFactory EmojiImageSourceFactoryForTests => _emojiImageSourceFactory;
        ...
        private List<InlineLayoutLine> BuildLines(double availableWidth)
        {
            ...
            double lineHeight = ResolveLineHeight();
            double pixelsPerDip = VisualTreeHelper.GetDpi(this).PixelsPerDip;
            IEnumerable<InlineLayoutToken> tokens = ExpandOversizedTokens(
        ...
        private IEnumerable<InlineLayoutToken> ExpandOversizedTokens(IEnumerable<InlineLayoutToken> tokens, double availableWidth, double pixelsPerDip)
        {
            foreach (InlineLayoutToken token in tokens)
            {
                if (availableWidth <= 0d || token.IsLineBreak ||
                ...
                {
                    yield return CreateTextToken(textElement, pixelsPerDip);
                }
            }
        }

        /// <summary>
        /// Defers native emoji rendering and text measurement until layout needs a token, so reaching
        /// the line limit stops work on the hidden remainder of a media title.
        /// </summary>
        private IEnumerable<InlineLayoutToken> CreateLayoutTokens(double pixelsPerDip, double lineHeight)
        {
            foreach (OverlayInlineToken token in OverlayInlineBuilder.Tokenize(Text ?? string.Empty))
            {
                switch (token.Kind)
                {
                    case OverlayInlineTokenKind.LineBreak:
                        yield return InlineLayoutToken.LineBreak;
                        break;
                    case OverlayInlineTokenKind.Emoji:
                        yield return CreateEmojiToken(token.Text, pixelsPerDip, lineHeight) ?? CreateTextToken(token.Text, pixelsPerDip);
                        break;
                    default:
