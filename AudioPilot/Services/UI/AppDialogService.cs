...
namespace AudioPilot.Services.UI
{
    internal sealed class AppDialogService : IAppDialogService
    {
        ...
        public bool SoundsEnabled => Volatile.Read(ref _soundsEnabled) != 0;
                ...
                while (true)
                {
                    ...
                    try
                    {
                        result = await PresentAsync(
                            item,
                            kind => TryPlaySound(item, kind)).ConfigureAwait(false);
                    }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        result = ShowFallback(item, ex.GetType().Name);
                    }
            ...
            Action<AppDialogKind> onPresented)
        {
            ...
            {
                return await PresentCurrentAsync().ConfigureAwait(false);
            }

            Dispatcher? dispatcher = _applicationDispatcherProvider();
            if (dispatcher == null || dispatcher.HasShutdownStarted || dispatcher.HasShutdownFinished)
            {
                return ShowFallback(item, "dispatcher-unavailable");
            }

            if (dispatcher.CheckAccess())
            {
                return await PresentCurrentAsync();
            }

            return await dispatcher.InvokeAsync(PresentCurrentAsync).Task.Unwrap();

            Task<AppDialogResult> PresentCurrentAsync()
            {
                ...
                return _presenter.PresentAsync(request, cancellationToken, onPresented);
            }
        }
        ...
        private AppDialogResult ShowFallback(DialogWorkItem item, string reason)
        {
            AppDialogRequest request;
            lock (_stateLock)
            {
                if (_disposed || item.Abandoned)
                {
                    return AppDialogResult.Cancelled;
                }
            ...
            try
            {
                while (true)
                {
                    ...
                    await InvokePresenterSafelyAsync(() =>
                    {
                        lock (_stateLock)
                        {
                            if (_disposed || !ReferenceEquals(_active, operation.Item)
                                || (operation.Kind == PresenterOperationKind.UpdateAcknowledgement
                                    && (operation.Item.Abandoned || operation.Item.UpdateRevision != operation.Revision)))
                            {
                                return;
                            }
                        }

                        if (operation.Kind == PresenterOperationKind.UpdateAcknowledgement)
                        {
                            _presenter.TryUpdateAcknowledgement(operation.Request!, operation.RepetitionCount);
                        }
                        else if (operation.Item.Abandoned)
                        {
                            _presenter.CloseActive(AppDialogResult.Cancelled);
                        }
                    }, operation.Kind == PresenterOperationKind.UpdateAcknowledgement
