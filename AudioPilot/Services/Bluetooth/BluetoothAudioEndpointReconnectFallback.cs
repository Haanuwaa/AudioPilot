...
namespace AudioPilot.Services.Bluetooth
{
    public interface IBluetoothAudioEndpointReconnectFallback
    {
        ...
        Task<bool> TryReconnectAsync(string expectedDeviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
            => TryReconnectAsync(expectedDeviceName, opId, kind, cancellationToken);
    }
    ...
    public sealed partial class BluetoothAudioEndpointReconnectFallback(Logger logger) : IBluetoothAudioEndpointReconnectFallback
    {
        ...
        public Task<bool> TryReconnectAsync(string expectedDeviceName, string opId, string kind, CancellationToken cancellationToken)
            => TryReconnectAsync(expectedDeviceName, string.Empty, opId, kind, cancellationToken);

        public Task<bool> TryReconnectAsync(string expectedDeviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
        {
            ...
            return ComThreadingHelper.RunOnCoreAudioThreadAsync(
                () => TryReconnectCore(expectedDeviceName, audioEndpointId, opId, kind, cancellationToken),
                cancellationToken);
        }

        private bool TryReconnectCore(string expectedDeviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
        {
            ...
            try
            {
                return TryReconnectEndpoints(devices, expectedDeviceName, opId, kind, cancellationToken, audioEndpointId);
            }
            finally
            {
        ...
        internal bool TryReconnectEndpoints(
            IReadOnlyList<MMDevice> endpoints, string expectedDeviceName, string opId, string kind, CancellationToken cancellationToken, string audioEndpointId = "")
        {
            string normalizedExpected = BluetoothReconnectService.NormalizeForMatch(expectedDeviceName);
            string cacheKey = string.IsNullOrEmpty(audioEndpointId) ? normalizedExpected : audioEndpointId;
            _rememberedAudioEndpoints.TryGetEndpointId(cacheKey, out string cachedId);
            Guid containerId = Guid.Empty;
            bool hasContainer = !string.IsNullOrEmpty(audioEndpointId) && TryGetContainerId(audioEndpointId, out containerId, out _);
            bool hasExactEndpoint = false;
            foreach (MMDevice endpoint in endpoints)
            {
                try
                {
                    if (string.Equals(endpoint.ID, audioEndpointId, StringComparison.OrdinalIgnoreCase)) { hasExactEndpoint = true; break; }
                }
                catch (Exception ex) { LogEndpointFailure(opId, kind, "identity", ex); }
            }
            ...
            foreach (MMDevice endpoint in endpoints)
            {
                ...
                try
                {
                    ...
                    bool exactId = string.Equals(endpoint.ID, audioEndpointId, StringComparison.OrdinalIgnoreCase);
                    bool sameContainer = hasContainer && TryGetContainerId(endpoint.ID, out Guid candidateContainer, out _) && candidateContainer == containerId;
                    if (!exactId && !sameContainer && (hasContainer || hasExactEndpoint)) continue;
                    string reason = exactId ? "endpoint-id" : sameContainer ? "container-id" : BluetoothReconnectService.ResolveMatchReason(endpoint.FriendlyName, expectedDeviceName, normalizedExpected);
            ...
            if (cachedCandidate == null) _rememberedAudioEndpoints.ForgetEndpointId(cacheKey);
            List<(MMDevice Candidate, string MatchReason)> ordered = BluetoothReconnectService.OrderCandidatesByMatchPriority(candidates);
            if (!hasContainer && !hasExactEndpoint && ordered.Count > 0)
            {
                int rank = BluetoothReconnectService.GetMatchRank(ordered[0].MatchReason);
                ordered.RemoveAll(item => BluetoothReconnectService.GetMatchRank(item.MatchReason) != rank);
            }
            if (ordered.Count > 1 && !hasContainer && !hasExactEndpoint)
            {
                string firstId = ordered[0].Candidate.ID;
                if (!TryGetContainerId(firstId, out Guid firstContainer, out _)
                    || ordered.Any(item => !TryGetContainerId(item.Candidate.ID, out Guid otherContainer, out _) || otherContainer != firstContainer))
                {
                    _logger.Warning("BluetoothReconnect", () => $"{AppConstants.Audio.LogEvents.BluetoothReconnect.FallbackResult} | opId={opId} kind={kind} result=skipped reason=ambiguous-device candidates={ordered.Count}");
                    return false;
                }
            }
            ...
            foreach ((MMDevice candidate, string reason) in ordered)
            {
                ...
                try
                {
                    ...
                    {
                        _rememberedAudioEndpoints.RememberEndpointId(cacheKey, endpointId);
            ...
            _rememberedAudioEndpoints.ForgetEndpointId(cacheKey);
