...
namespace AudioPilot.Services.Bluetooth
{
    public interface IBluetoothReconnectService
    {
        ...
        Task<bool> TryReconnectPairedAudioDeviceAsync(string deviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
            => TryReconnectPairedAudioDeviceAsync(deviceName, opId, kind, cancellationToken);
        ...
        Task<bool> TryReconnectUsingAudioEndpointControlAsync(string deviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
            => TryReconnectUsingAudioEndpointControlAsync(deviceName, opId, kind, cancellationToken);
    }
    ...
    public sealed partial class BluetoothReconnectService : IBluetoothReconnectService
    {
        ...
        private readonly Func<string, Guid?> _getContainerId;
        ...
        internal BluetoothReconnectService(
            ...
            Func<string, Guid?>? getContainerId = null)
        {
            _logger = logger;
            ...
            _getContainerId = getContainerId ?? AudioEndpointIdentity.TryGetContainerId;
        }
        ...
        {
            return await TryReconnectPairedAudioDeviceAsync(deviceName, string.Empty, opId, kind, cancellationToken);
        }

        public async Task<bool> TryReconnectPairedAudioDeviceAsync(string deviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
        {
            ...
            Guid? containerId = _getContainerId(audioEndpointId);
            string rememberedDeviceKey = string.IsNullOrWhiteSpace(audioEndpointId)
                ? ResolveRememberedDeviceKey(deviceName)
                : $"endpoint:{audioEndpointId.Trim()}";
            string rememberedEndpointId = TryGetRememberedAssociationEndpointId(rememberedDeviceKey, out string cachedEndpointId)
                ? cachedEndpointId
                : string.Empty;

            if (containerId.HasValue && !string.IsNullOrWhiteSpace(rememberedEndpointId)
                && await TryReconnectLastSuccessfulAssociationEndpointAsync(deviceName, normalizedExpectedName, rememberedDeviceKey, rememberedEndpointId, opId, kind, containerId, cancellationToken))
            {
                return true;
            }
            ...
            var pairedDevices = new List<BluetoothAssociationEndpointCandidate>(devices.Count);
            ...
            var candidates = new List<(BluetoothAssociationEndpointCandidate Device, string MatchReason)>(pairedDevices.Count);
            for (int index = 0; index < pairedDevices.Count; index++)
            {
                BluetoothAssociationEndpointCandidate device = pairedDevices[index];
                if (containerId.HasValue && device.ContainerId.HasValue && device.ContainerId != containerId) continue;
                string reason = containerId.HasValue && device.ContainerId == containerId
                    ? "container-id"
                    : ResolveMatchReason(device.Name, deviceName, normalizedExpectedName);
            ...
            if (_logger.IsEnabled(LogLevel.Debug))
            {
                _logger.Debug(
                    "BluetoothReconnect",
                    () => $"{AppConstants.Audio.LogEvents.BluetoothReconnect.Candidates} | opId={opId} kind={kind} expected={LogPrivacy.Device(deviceName)} normalizedExpected={LogPrivacy.Label(normalizedExpectedName)} pairedDevices={pairedDevices.Count} matched={candidates.Count}");
            }
            ...
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                _logger.Trace(
                    "BluetoothReconnect",
                    () => $"{AppConstants.Audio.LogEvents.BluetoothReconnect.Candidates} | opId={opId} kind={kind} pairedNames={BuildCandidateSummary(pairedDevices)} matchedNames={BuildMatchedSummary(candidates)}");
            }
            ...
            List<(BluetoothAssociationEndpointCandidate Candidate, string MatchReason)> prioritizedCandidates = OrderCandidatesByMatchPriority(candidates);
            ...
            (BluetoothAssociationEndpointCandidate selected, string selectedReason) = prioritizedCandidates[0];
            if (selected.IsConnected) return false;
            _logger.Debug("BluetoothReconnect", () => $"{AppConstants.Audio.LogEvents.BluetoothReconnect.Match} | opId={opId} kind={kind} candidate={LogPrivacy.Device(selected.Name)} reason={selectedReason}");
            return await TryPairAssociationEndpointCandidateAsync(selected, rememberedDeviceKey, opId, kind, allowRememberedIdUpdate: true, cancellationToken);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!cachedCandidate.IsPaired || cachedCandidate.IsConnected || cachedCandidate.ContainerId != containerId)
            {
        ...
        public async Task<bool> TryReconnectUsingAudioEndpointControlAsync(string deviceName, string opId, string kind, CancellationToken cancellationToken)
            => await TryReconnectUsingAudioEndpointControlAsync(deviceName, string.Empty, opId, kind, cancellationToken);

        public async Task<bool> TryReconnectUsingAudioEndpointControlAsync(string deviceName, string audioEndpointId, string opId, string kind, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(deviceName))
            {
                return false;
            ...
            try
            {
                return await _audioEndpointFallback.TryReconnectAsync(deviceName, audioEndpointId, opId, kind, cancellationToken);
            }
            catch (OperationCanceledException)
            {
                throw;
            ...
            out (T Candidate, string MatchReason)? selected)
        {
            ...
            {
                string matchReason = candidates[index].MatchReason;
            ...
            if (bestCandidateIndex < 0 || topRankCount != 1)
            {
                return false;
            }
