...
namespace AudioPilot.Services.Internal
{
    internal sealed class HotkeyDispatchCoordinator(Logger logger)
    {
        ...
        public void ExecuteCallback(int hotkeyId, string description, Action? callback, long? enqueueTimestamp = null)
        {
            ...
            if (hotkeyId is not (AppConstants.Hotkeys.PushToTalkHotkeyId or AppConstants.Hotkeys.HoldToMuteHotkeyId) &&
                !TryReserveDebounceSlot(hotkeyId, now))
            {
                return;
            }
            ...
            _queueWorkItem(() =>
            {
                ...
                using var trace = OperationTrace.Start($"hotkey-{hotkeyId}-dispatch", _logger);
                try
                {
