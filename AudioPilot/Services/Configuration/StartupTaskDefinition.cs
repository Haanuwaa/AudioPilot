...
internal static class StartupTaskDefinition
{
    ...
    internal static string Create(string executablePath, string userSid, bool enabled = true)
    {
        ...
        return new XElement(ns + "Task", new XAttribute("version", "1.2"),
            ...
            new XElement(ns + "Triggers", new XElement(ns + "LogonTrigger",
                new XElement(ns + "Enabled", true), new XElement(ns + "UserId", userSid), new XElement(ns + "Delay", "PT0S"))),
    ...
    private static bool IsEnabled(XElement task) =>
        ReadEnabled(task.Element(TaskNamespace + "Settings")?.Element(TaskNamespace + "Enabled"))
        && ReadEnabled(task.Element(TaskNamespace + "Triggers")?.Element(TaskNamespace + "LogonTrigger")?.Element(TaskNamespace + "Enabled"));

    private static bool ReadEnabled(XElement? element) => element == null || XmlConvert.ToBoolean(element.Value.Trim());
    ...
    internal static bool Matches(string? xml, string executablePath, string userSid, bool includeDisabled)
    {
        ...
        foreach (string section in new[] { "Triggers", "Principals", "Actions" })
        {
            ...
            foreach (XElement value in expectedEntry.Elements())
            {
                ...
                if (!ValuesMatch(actualValue, value.Value, comparison))
                {
                    return false;
                }
            }
        ...
        return expected.Element(ns + "Settings")!.Elements()
            ...
            .All(element => ValuesMatch(actualSettings?.Element(element.Name)?.Value ?? DefaultSettingValue(element.Name.LocalName), element.Value));
    }

    private static bool ValuesMatch(string? actual, string expected, StringComparison comparison = StringComparison.Ordinal) =>
        expected is "true" or "false"
            ? actual != null && XmlConvert.ToBoolean(actual.Trim()) == XmlConvert.ToBoolean(expected)
