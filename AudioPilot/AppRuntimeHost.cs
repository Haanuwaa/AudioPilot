...
namespace AudioPilot
{
    ...
    internal sealed partial class AppRuntimeHost : IAsyncDisposable
    {
        ...
        private async Task InitializeAsync()
        {
            ...
            if (_appVm.CurrentSettings?.Miscellaneous.UseScheduledStartup == true)
            {
                await WaitForStartupTaskbarAsync();
            }
        ...
        /// <summary>
        /// An early logon task can beat Explorer to the desktop. Keep hotkeys active while waiting briefly
        /// for its notification area, before treating tray registration as a failure and showing the main window.
        /// </summary>
        private async Task WaitForStartupTaskbarAsync()
        {
            if (FindWindow("Shell_TrayWnd", null) != IntPtr.Zero) return;
            _logger.Debug("AppRuntimeHost", "startup-waiting-for-taskbar");
            long started = Stopwatch.GetTimestamp();
            while (FindWindow("Shell_TrayWnd", null) == IntPtr.Zero && Stopwatch.GetElapsedTime(started) < TimeSpan.FromSeconds(10))
            {
                await Task.Delay(250, _appVm.ShutdownToken);
            }
            _logger.Debug("AppRuntimeHost", () => $"startup-taskbar-wait-completed | elapsedMs={Stopwatch.GetElapsedTime(started).TotalMilliseconds:F0}");
        }
