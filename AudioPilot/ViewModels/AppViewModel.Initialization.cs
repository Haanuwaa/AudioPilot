    ...
    public partial class AppViewModel
    {
            ...
            IAppDialogService? dialogService = null)
        {
            ...
            _applicationTriggerCoordinator = new Lazy<ApplicationTriggerCoordinator>(
                () => new ApplicationTriggerCoordinator(
                    ...
                    _logger,
                    deactivateRoutine: (routine, processId) => DeactivateRoutineFromApplicationTrigger(routine, processId),
                    routineSnapshotProvider: GetPersistedRoutineSnapshot),
                System.Threading.LazyThreadSafetyMode.ExecutionAndPublication);
            ...
            SettingsShowAudioStatusHotkeyDraftCapture = new HotkeyViewModel();
            ...
            SettingsHoldToMuteHotkeyDraftCapture = new HotkeyViewModel { RequiresHeldInput = true };
            SettingsPushToTalkHotkeyDraftCapture = new HotkeyViewModel { RequiresHeldInput = true };
            SettingsForegroundMuteHotkeyDraftCapture = new HotkeyViewModel();
            SettingsForegroundVolumeDownHotkeyDraftCapture = new HotkeyViewModel();
            SettingsForegroundVolumeUpHotkeyDraftCapture = new HotkeyViewModel();
            ...
            SettingsShowAudioStatusHotkeyDraftCapture.BaseHoverText = "Shows the current output and microphone devices, volumes, and mute states without changing them. Requires overlays to be enabled. Press Delete to clear.";
            ...
            SettingsHoldToMuteHotkeyDraftCapture.BaseHoverText = "Temporarily mutes the default microphones while held, then restores their prior states. Use a keyboard key or mouse button; not the wheel, Pause, or Print Screen. Press Delete to clear.";
            SettingsPushToTalkHotkeyDraftCapture.BaseHoverText = "Temporarily unmutes the default microphones while held, then restores their prior states. Use a keyboard key or mouse button; not the wheel, Pause, or Print Screen. Press Delete to clear.";
            SettingsForegroundMuteHotkeyDraftCapture.BaseHoverText = "Foreground app mute. Press Delete to clear.";
            SettingsForegroundVolumeDownHotkeyDraftCapture.BaseHoverText = "Foreground app volume down. Press Delete to clear.";
            SettingsForegroundVolumeUpHotkeyDraftCapture.BaseHoverText = "Foreground app volume up. Press Delete to clear.";
            ...
            WireHotkeyDraft(SettingsShowAudioStatusHotkeyDraftCapture, nameof(SettingsShowAudioStatusHotkeyDraft));
            ...
            WireHotkeyDraft(SettingsHoldToMuteHotkeyDraftCapture, nameof(SettingsHoldToMuteHotkeyDraft));
            WireHotkeyDraft(SettingsPushToTalkHotkeyDraftCapture, nameof(SettingsPushToTalkHotkeyDraft));
            WireHotkeyDraft(SettingsForegroundMuteHotkeyDraftCapture, nameof(SettingsForegroundMuteHotkeyDraft));
            WireHotkeyDraft(SettingsForegroundVolumeDownHotkeyDraftCapture, nameof(SettingsForegroundVolumeDownHotkeyDraft));
            WireHotkeyDraft(SettingsForegroundVolumeUpHotkeyDraftCapture, nameof(SettingsForegroundVolumeUpHotkeyDraft));

            SettingsToggleAppVisibilityHotkeyDraft = "Ctrl+Alt+H";
            SettingsShowAudioStatusHotkeyDraft = string.Empty;
            ...
            SettingsHoldToMuteHotkeyDraft = string.Empty;
            SettingsPushToTalkHotkeyDraft = string.Empty;
            SettingsForegroundMuteHotkeyDraft = string.Empty;
            SettingsForegroundVolumeDownHotkeyDraft = string.Empty;
            SettingsForegroundVolumeUpHotkeyDraft = string.Empty;
            ...
            var refreshCommand = TrackCommand(new RelayCommand(() => RefreshDevicesAsync(), () => !IsResettingSettings && !IsRefreshing, ex => HandleAsyncCommandException("refresh-devices", ex)), observeViewModel: true);
            var saveCommand = TrackCommand(new RelayCommand(() => SaveSettingsAsync(), () => !IsResettingSettings && !_isSaving && !_isApplyingSettings && !IsSavingRoutines, ex => HandleAsyncCommandException("save-settings", ex)), observeViewModel: true);
            var saveCurrentContextCommand = TrackCommand(new RelayCommand(SaveCurrentContextAsync, () => !IsResettingSettings && !_isSaving && !_isApplyingSettings && !IsSavingRoutines, ex => HandleAsyncCommandException("save-current-context", ex)), observeViewModel: true);
            var applySettingsCommand = TrackCommand(new RelayCommand(() => ApplySettingsAsync(), () => !IsResettingSettings && !_isApplyingSettings && !_isSaving && !IsSavingRoutines, ex => HandleAsyncCommandException("apply-settings", ex)), observeViewModel: true);
            var importSettingsCommand = TrackCommand(new RelayCommand(ImportSettingsAsync, () => !IsResettingSettings && !_isApplyingSettings && !_isSaving && !IsSavingRoutines, ex => HandleAsyncCommandException("import-settings", ex)), observeViewModel: true);
            var exportSettingsCommand = TrackCommand(new RelayCommand(ExportSettingsAsync, () => !IsResettingSettings && !_isApplyingSettings && !_isSaving && !IsSavingRoutines, ex => HandleAsyncCommandException("export-settings", ex)), observeViewModel: true);
            var resetPerAppAudioRoutingCommand = TrackCommand(new RelayCommand(ResetPerAppAudioRoutingAsync, () => !IsResettingSettings && !_isApplyingSettings && !_isSaving && !IsSavingRoutines, ex => HandleAsyncCommandException("reset-per-app-audio", ex)), observeViewModel: true);
            var resetCommand = TrackCommand(new RelayCommand(ResetToDefaultsAsync, () => !IsResettingSettings && !_isApplyingSettings && !_isSaving && !IsSavingRoutines && !_isCleaningUp, ex => HandleAsyncCommandException("reset-settings", ex)), observeViewModel: true);
            ...
            var addRoutineCommand = TrackCommand(new RelayCommand(AddRoutine, () => !IsResettingSettings), observeViewModel: true);
            ...
            var saveRoutinesCommand = TrackCommand(new RelayCommand(SaveRoutinesFromButtonAsync, () => !IsResettingSettings && !_isApplyingSettings && !_isSaving && !IsSavingRoutines && (HasRoutines || HasUnsavedRoutineChanges), ex => HandleAsyncCommandException("save-routines", ex)), observeViewModel: true);
            ...
            ReorderOutputCycleDevicesCommand = TrackCommand(new RelayCommand(parameter => ReorderCycleDevices(parameter, output: true)));
            ...
            ReorderInputCycleDevicesCommand = TrackCommand(new RelayCommand(parameter => ReorderCycleDevices(parameter, output: false)));
            ...
            ReorderRoutinesCommand = TrackCommand(new RelayCommand(ReorderRoutines, _ => !IsSavingRoutines && !_isCleaningUp));
        ...
        public async Task InitializeAsync(bool noSettingsFileExists)
        {
            ...
            try
            {
                ...
                StartupInitPlan startupPlan = await Task.Run(() =>
                {
                    ...
                    var plan = AppStartupRegistryCoordinator.BuildInitPlan(
        ...
        private async Task<bool> HasSettingsFileChangedAsync()
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    bool changed = currentWriteTime != lastTime;
                ...
                try
                {
                    AppRefreshExecutionResult refreshResult = await AppRefreshCycleCoordinator.ExecuteAsync(
                        ...
                        new AppRefreshExecutionDependencies(
                            ...
                            settings => ApplyExternallyReloadedSettings(settings),
                            ...
                            interactive =>
                            {
                                _audio.InvalidateRecentMixerSnapshotState(refreshProcessMetadata: true);
                                return RefreshMixerAsync(interactive);
                            }),
                        refreshOpId,
                        _logger);
        ...
        private void NotifyUpcomingScheduledRoutines(IReadOnlyList<ScheduledRoutineReminder> reminders)
        {
            ...
            _dispatcher.BeginInvoke(() =>
            {
                ...
                _shell.NotifyUpcomingScheduledRoutines([.. current.DistinctBy(static routine => routine.Id, StringComparer.OrdinalIgnoreCase).Select(static routine => routine.DisplayName)]);
            });
        }
        ...
        private void ExecuteRoutineFromScheduler(AudioRoutine routine, string executionSource)
        {
            _dispatcher.BeginInvoke(async () =>
            {
                try
                {
                    await ExecuteRoutineForResolvedProcessAsync(routine, 0, showOverlay: true, executionSource: executionSource, trackLifetime: false, cancellationToken: ShutdownToken);
                }
                catch (Exception ex)
                {
        ...
        {
            List<AudioRoutine> automaticRoutines = [.. Routines.SelectMany(static routine => routine.ExpandAutomaticTriggers())];
            bool hasScheduledRoutines = automaticRoutines.Any(r => r.Enabled && r.TriggerKind == RoutineTriggerKind.Scheduled);
            bool hasNetworkTriggeredRoutines = automaticRoutines.Any(r => r.Enabled && r.TriggerKind == RoutineTriggerKind.Network);
            bool hasProcessFocusRoutines = automaticRoutines.Any(r =>
                r.Enabled &&
