...
namespace AudioPilot.ViewModels
{
    public partial class AppViewModel
    {
        ...
        private bool HasSettingsDraftDivergedFromCachedSettings()
        {
            ...
            return !AreHotkeyStringsEquivalent(SettingsToggleAppVisibilityHotkeyDraft, cachedCopy.Hotkeys.App.ToggleAppVisibility) ||
        ...
        private void ApplyExternallyReloadedSettings(Settings newSettings)
        {
            ...
            {
                _dispatcher.Invoke(() => ApplyExternallyReloadedSettings(newSettings));
                return;
            }
                ...
                await ExecuteSettingsWriteAsync(async () =>
                {
                    ...
                    await InvokeOnDispatcherAsync(() =>
                    {
                        ApplySettingsSideEffectResult applyEffects = AppSettingsEffectsCoordinator.RunApplySideEffects(
                            ...
                            syncSettingsDrafts: () => SyncSettingsDraftFromCurrentState(resetHotkeyExpansion: false),
        ...
        private void SaveSettingsWithStartupRegistration(Settings settings, bool forceStartup = false)
        {
            ...
            bool startupChanged = forceStartup || previous.RunAtStartup != settings.RunAtStartup
                || (settings.RunAtStartup && previous.Miscellaneous.UseScheduledStartup != settings.Miscellaneous.UseScheduledStartup);
            if (!startupChanged)
            {
            ...
            {
                if (!apply(settings.RunAtStartup, opId)) throw new InvalidOperationException("Failed to update startup registration.");
                _settings.SaveSettings(settings);
                return;
            }
            _startup.ApplyRegistration(settings.RunAtStartup, settings.Miscellaneous.UseScheduledStartup,
                () => _settings.SaveSettings(settings), opId);
        }
        ...
        private async Task ResetToDefaultsAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                await _dialogs.ShowErrorAsync("Failed to reset settings.");
            }
        }
        ...
        {
            if (_isInitializing || _isCleaningUp || Volatile.Read(ref _autoSaveSuppressionCount) > 0 || !IsPersistedAutoSaveEnabled())
            {
                return;
            }
