...
namespace AudioPilot.ViewModels
{
    public partial class AppViewModel
    {
        ...
        {
            if (_isApplyingRoutines || IsRoutineRuntimeOnlyProperty(e.PropertyName))
            {
                return;
            }
        ...
        private void UpdateRoutineLastRunState(AudioRoutine routine, RoutineExecutionResult result)
        {
            if (result.Skipped)
            {
                if (result.SkipReason != null) SetRoutineLastRunState(routine, RoutineLastRunState.Skipped, result.SkipReason);
                return;
            }
