...
namespace AudioPilot.ViewModels
{
    public partial class AppViewModel : IResumeRecoveryHandler
    {
        ...
        public async Task RecoverAfterSystemResumeAsync(string? resumeOpId = null)
        {
            ...
            long? routineRecoveryVersion = _routineSystemEvents?.RecoveryVersion;
            ...
            _routineSystemEvents?.CompleteRecovery(routineRecoveryVersion);
            if (!result.Succeeded && !_isCleaningUp && !_backgroundWorkCts.IsCancellationRequested)
            {
                _shell.NotifyBackgroundFailure(BackgroundFailureKind.ResumeRecovery);
            }
        }
        ...
        public async Task CleanupAsync()
        {
            ...
            CancellationTokenSource? autoSaveDebounceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _autoSaveDebounceCts);
            ...
            await ExecuteCleanupTaskAsync(() => Updates.DisposeAsync().AsTask(), "stop-update-checks", cleanupOpId);
            ...
            await ExecuteCleanupTaskAsync(() => AudioTesting.DisposeAsync().AsTask(), "dispose-audio-endpoint-testing", cleanupOpId);
            ...
            CancellationTokenSource? sessionDebounceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _sessionRefreshDebounceCts);
            CancellationTokenSource? visibleMixerActivationDebounceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _visibleMixerActivationRefreshDebounceCts);
            CancellationTokenSource? steamBigPictureDebounceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _steamBigPictureDebounceCts);
            CancellationTokenSource? steamBigPictureConfirmationDebounceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _steamBigPictureConfirmationDebounceCts);
            CancellationTokenSource? steamBigPictureFallbackMaintenanceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _steamBigPictureFallbackMaintenanceCts);
            CancellationTokenSource? routineLeaseDebounceToDispose = AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref _routineAppOutputLeaseRefreshDebounceCts);
        ...
        private void GenerateDeviceReferenceFile()
        {
            ...
            {
                _logger.Warning("AppViewModel", "device-reference-file-generate-failed", nameof(GenerateDeviceReferenceFile), ex);
            }
        }
