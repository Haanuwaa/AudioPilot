...
namespace AudioPilot.ViewModels
{
    internal sealed class MixerSharedSessionCoordinator(AudioMixerMode preferredOwnerMode)
    {
        ...
        public bool HasPendingVolumeChange(string sessionId)
        {
            if (!_subscriptionStates.TryGetValue(sessionId, out SharedSessionSubscriptionState? state)) { return false; }
            lock (state.Lock) { return state.PresentMixers.Any(mixer => mixer.HasPendingVolumeChange(sessionId)); }
        }
        ...
        public void DetachVolumeChangedHandler(string sessionId, AudioSessionItem item, MixerViewModel mixer)
        {
            ...
            lock (subscriptionState.Lock)
            {
                ...
                if (subscriptionState.PresentMixers.Count == 0)
                {
                    ...
                    _sharedItems.TryRemove(sessionId, out _);
                }
            }
        }
