...
namespace AudioPilot.ViewModels
{
    ...
    internal sealed class RoutineEditorViewModel : INotifyPropertyChanged, IDisposable
    {
        ...
        {
            _selectedOutputIndex = SyncDeviceCollection(OutputDevices, _sourceOutputDevices, "No output device", _selectedOutputIndex);
        ...
        {
            _selectedInputIndex = SyncDeviceCollection(InputDevices, _sourceInputDevices, "No input device", _selectedInputIndex);
        ...
        private static int SyncDeviceCollection(ObservableCollection<CycleDevice> target, ObservableCollection<CycleDevice> source, string placeholderName, int selectedIndex)
        {
            ...
            return ResolveSelectedIndex(target, selectedDevice?.Id, selectedDevice?.Name);
        }
            ...
            CultureInfo? displayCulture = null)
        {
            ...
            SelectedOutputIndex = ResolveSelectedIndex(OutputDevices, existingRoutine.OutputDeviceId, existingRoutine.OutputDeviceName);
            SelectedInputIndex = ResolveSelectedIndex(InputDevices, existingRoutine.InputDeviceId, existingRoutine.InputDeviceName);
        ...
        private static int ResolveSelectedIndex(ObservableCollection<CycleDevice> devices, string? deviceId, string? deviceName)
        {
            ...
            devices.Add(new CycleDevice { Id = deviceId, Name = string.IsNullOrWhiteSpace(deviceName) ? "Unavailable device" : deviceName });
