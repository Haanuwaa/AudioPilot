    ...
    public partial class AppViewModel
    {
        ...
        public async Task<CliExecutionResult> SetRoutineEnabledFromCliAsync(string selector, bool enabled, bool jsonOutput, bool redactOutput = false)
        {
                    ...
                    settings =>
                    {
                        ...
                        if (resolution.Status != CliRoutineResolutionStatus.Success || resolution.Routine == null)
                        {
                            return new CliSettingsMutationDecision<CliExecutionResult>(
                                BuildRoutineErrorResult(5, resolution.ErrorCode, resolution.Message, jsonOutput, redactOutput: redactOutput),
                                Persist: false);
                        }
            ...
            catch (Exception ex)
            {
                ...
                return BuildRoutineErrorResult(3, "routine-update-failed", $"Failed to update routine '{selectedRoutine?.Name ?? selector}'.", jsonOutput, selectedRoutine, redactOutput: redactOutput);
            }
        }
