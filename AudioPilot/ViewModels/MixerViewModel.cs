        ...
        DeviceCacheHelper? deviceCache = null)
    {
        ...
        public async Task RefreshAsync(bool interactive = true)
        {
            ...
            try
            {
                ...
                foreach (var session in serviceSessions)
                {
                    ...
                    existingItem.UpdateRoutingMetadataFromSystem(session.ProcessName, session.SessionInstanceId, session.EndpointId, session.SessionCount);
        ...
        private MixerMutationResult ApplyVolumeChange(AudioSessionItem item, float requestedVolume)
        {
            ...
            try
            {
                ...
                {
                    _audio.UpdateSessionVolumeCache(item.DisplayName, processName, scalar * 100f);
                }
                else
                {
                    _audio.UpdateSessionVolumeCache(item.DisplayName, null, scalar * 100f);
                }
