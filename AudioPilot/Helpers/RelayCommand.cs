...
namespace AudioPilot.Helpers
{
    public class RelayCommand : ICommand, IDisposable
    {
        ...
        {
            if (_executeAsync == null && _executeAsyncWithParameter == null)
            {
                _execute!(parameter);
                return;
            }
            ...
            LastExecutionTaskForTests = ExecuteAsyncCore(parameter);
        }
        ...
        private void ReportAsyncFailure(Exception exception)
        {
            try
            {
                ...
                {
                    Logger.Instance.Error("RelayCommand", "Asynchronous command failed", nameof(ExecuteAsyncCore), exception);
                }
            }
            catch (Exception reportingException)
            {
                LifecycleFallbackDiagnostics.Write("RelayCommand", "Failed to report asynchronous command failure", nameof(ExecuteAsyncCore), exception, reportingException);
            }
        }
