...
namespace AudioPilot.Cli
{
    public sealed partial class CliCommand
    {
        private static bool TryParseMediaCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            {
                error = CliSuggestionHelper.BuildUnknownValueError("media command", args[1], ["play-pause", "next", "previous", "status", "seek"], CliCommandHelpMetadata.GetParserUsage(CliCommandHelpMetadata.ParserUsageId.Media));
                return false;
            }
