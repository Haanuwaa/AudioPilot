...
namespace AudioPilot.Cli
{
    public sealed partial class CliCommand
    {
        private static bool TryParseMediaCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            if (tokens[1] is "play" or "pause")
            {
                if (!CliOutputFlagParser.TryParse(tokens, args, 2, out bool jsonOutput, out bool redactOutput, out error)) return false;
                if (redactOutput)
                {
                    error = "Play and pause commands do not support --redact; their output contains no media metadata.";
                    return false;
                }

                command = new CliCommand { Action = tokens[1] == "play" ? CliAction.MediaPlay : CliAction.MediaPause, JsonOutput = jsonOutput };
                return true;
            }

            if (tokens[1] == "seek")
            {
                if (tokens.Length < 3 || tokens[2] is not ("forward" or "backward"))
                {
                    error = "Use: media seek forward|backward [duration] [--json]";
                    return false;
                }

                string? seconds = null;
                int flagStart = 3;
                if (tokens.Length > 3 && !tokens[3].StartsWith("--", StringComparison.Ordinal))
                {
                    if (!MediaSeekStep.TryParse(tokens[3], out int parsed))
                    {
                        error = MediaSeekStep.InputHelp;
                        return false;
                    }

                    seconds = parsed.ToString(CultureInfo.InvariantCulture);
                    flagStart++;
                }

                if (!CliOutputFlagParser.TryParse(tokens, args, flagStart, out bool jsonOutput, out bool redactOutput, out error)) return false;
                if (redactOutput)
                {
                    error = "Seek commands do not support --redact; their output contains no media metadata.";
                    return false;
                }

                command = new CliCommand { Action = CliAction.MediaSeek, Reverse = tokens[2] == "backward", Value = seconds, JsonOutput = jsonOutput };
                return true;
            }
            ...
            {
                error = CliSuggestionHelper.BuildUnknownValueError("media command", args[1], ["play-pause", "play", "pause", "next", "previous", "status", "seek"], CliCommandHelpMetadata.GetParserUsage(CliCommandHelpMetadata.ParserUsageId.Media));
                return false;
            }
        ...
        private static bool TryParseVolumeCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            if (tokens[1] is "set" or "adjust")
            {
                bool relative = tokens[1] == "adjust";
                var usageId = relative ? CliCommandHelpMetadata.ParserUsageId.VolumeAdjust : CliCommandHelpMetadata.ParserUsageId.VolumeSet;
                if (args.Length < 4)
                {
                    error = $"Missing volume {(relative ? "adjustment" : "percent")}. Use: {CliCommandHelpMetadata.GetParserUsage(usageId)}";
                    return false;
                }

                if (!float.TryParse(args[3], NumberStyles.Float, CultureInfo.InvariantCulture, out float percent) || !float.IsFinite(percent) || percent < (relative ? -100f : 0f) || percent > 100f)
                {
                    error = relative ? "Invalid volume adjustment. Use a number between -100 and 100." : "Invalid volume percent. Use a number between 0 and 100.";
                    return false;
                }
                ...
                command = tokens[2] switch
                {
                    "master" => new CliCommand { Action = relative ? CliAction.VolumeAdjustMaster : CliAction.VolumeSetMaster, Key = deviceId, Value = args[3], JsonOutput = jsonOutput, RedactOutput = redactOutput },
                    "mic" => new CliCommand { Action = relative ? CliAction.VolumeAdjustMic : CliAction.VolumeSetMic, Key = deviceId, Value = args[3], JsonOutput = jsonOutput, RedactOutput = redactOutput },
                    _ => new CliCommand(),
                };

                if (command.Action == CliAction.None)
                {
                    error = CliSuggestionHelper.BuildUnknownValueError("volume target", args[2], ["master", "mic"], CliCommandHelpMetadata.GetParserUsage(usageId));
                    return false;
                }
            ...
            error = CliSuggestionHelper.BuildUnknownValueError("volume command", args[1], ["get", "set", "adjust"], "volume get|set|adjust master|mic ...");
            return false;
        }
