namespace AudioPilot.Cli
{
    public sealed partial class CliCommand
    {
        private static bool TryParseRoutineCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            if (tokens[1] is not ("get" or "next" or "run" or "enable" or "disable" or "delete"))
            {
                error = CliSuggestionHelper.BuildUnknownValueError("routine command", args[1], ["list", "get", "next", "run", "enable", "disable", "create", "update", "delete", "import", "export"], "routine list|get|next|run|enable|disable|create|update|delete|import|export ...");
                return false;
            }
            ...
            command = new CliCommand
            {
                ...
                {
                    "get" => CliAction.RoutineGet,
                    "next" => CliAction.RoutineNext,
