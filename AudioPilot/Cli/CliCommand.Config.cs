    ...
    public sealed partial class CliCommand
    {
        private static bool TryParseConfigCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            if (tokens[1] == "set")
            {
                ...
                if (!TryParseSettingValue(tokens, args, out string value, out bool jsonOutput, out bool redactOutput, out error))
                {
                    return false;
                }
        ...
        private static bool TryParseSettingValue(string[] tokens, string[] args, out string value, out bool jsonOutput, out bool redactOutput, out string? error)
        {
            int flagIndex = Array.FindIndex(tokens, 3, static token => token.StartsWith("--", StringComparison.Ordinal));
            if (flagIndex < 0) flagIndex = args.Length;
            value = string.Join(" ", args[3..flagIndex]);
            jsonOutput = false;
            redactOutput = false;
            if (string.IsNullOrWhiteSpace(value))
            {
                error = "Setting value cannot be empty. Place the value before any output flags.";
                return false;
            }

            return CliOutputFlagParser.TryParse(tokens, args, flagIndex, out jsonOutput, out redactOutput, out error);
        }
