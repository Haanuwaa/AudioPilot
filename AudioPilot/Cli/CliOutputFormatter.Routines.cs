...
namespace AudioPilot.Cli
{
    ...
    {
        public static string FormatRoutineNextOccurrence(AudioRoutine routine, TimeZoneInfo timeZone, DateTime occurrenceUtc, bool jsonOutput, bool redactOutput = false)
        {
            var snapshot = new
            {
                routine.Id,
                TriggerId = routine.TriggerIdentity,
                Name = FormatRoutineName(routine.Name, redactOutput),
                OccurrenceUtc = occurrenceUtc.ToString("O", CultureInfo.InvariantCulture),
                OccurrenceLocal = TimeZoneInfo.ConvertTime(new DateTimeOffset(occurrenceUtc), timeZone).ToString("O", CultureInfo.InvariantCulture),
                TimeZoneId = timeZone.Id,
                ReminderUtc = routine.NotifyBeforeScheduledRun ? occurrenceUtc.AddMinutes(-1).ToString("O", CultureInfo.InvariantCulture) : null,
                DiagCode = "routine-next-occurrence",
            };
            ...
            return $"{snapshot.Name} (Id: {snapshot.Id}){Environment.NewLine}Next scheduled occurrence: {snapshot.OccurrenceLocal} [{snapshot.TimeZoneId}]{Environment.NewLine}UTC: {snapshot.OccurrenceUtc}"
                + (snapshot.ReminderUtc == null ? string.Empty : $"{Environment.NewLine}Reminder UTC: {snapshot.ReminderUtc}");
        }

        public static string FormatRoutineDetails(AudioRoutine routine, bool jsonOutput, bool redactOutput = false)
        {
            var snapshot = new
            {
                routine.Id,
                ...
                routine.MasterVolumePercent,
                routine.MicVolumePercent,
                CommunicationsOutput = FormatRoutineDevice(routine.CommunicationsOutput, redactOutput),
                CommunicationsInput = FormatRoutineDevice(routine.CommunicationsInput, redactOutput),
                routine.OutputMuteAction,
                routine.InputMuteAction,
                routine.Hotkey,
                HotkeyCycleGroup = redactOutput ? RedactLabel(routine.HotkeyCycleGroup, "routine-group") : routine.HotkeyCycleGroup,
                Conditions = routine.Conditions with
                {
                    Device = FormatRoutineDevice(routine.Conditions.Device, redactOutput),
                    RunningAppPath = FormatOptionalPath(routine.Conditions.RunningAppPath, redactOutput) ?? string.Empty,
                    ConnectedNetwork = FormatNetworkName(routine.Conditions.ConnectedNetwork, redactOutput),
                },
                Triggers = routine.Triggers.Select(trigger => trigger == null ? null : trigger with
                {
                    Device = FormatRoutineDevice(trigger.Device, redactOutput),
                    Days = [.. (trigger.Days ?? []).Order()],
                    AppPath = FormatOptionalPath(trigger.AppPath, redactOutput) ?? string.Empty,
                    TitlePattern = redactOutput ? RedactLabel(trigger.TitlePattern, "window-title") : trigger.TitlePattern,
                    NetworkName = FormatNetworkName(trigger.NetworkName, redactOutput),
                }).ToArray(),
                TargetAppPath = FormatOptionalPath(routine.TargetAppPath, redactOutput) ?? string.Empty,
            ...
            return string.Join(Environment.NewLine,
                ...
                $"Output device: {snapshot.OutputDeviceName} ({snapshot.OutputDeviceId})",
                $"Input device: {snapshot.InputDeviceName} ({snapshot.InputDeviceId})",
                $"Master volume: {snapshot.MasterVolumePercent?.ToString(CultureInfo.InvariantCulture) ?? "unchanged"}",
                $"Microphone volume: {snapshot.MicVolumePercent?.ToString(CultureInfo.InvariantCulture) ?? "unchanged"}",
                $"Communications playback: {snapshot.CommunicationsOutput?.Name ?? "unchanged"}",
                $"Communications microphone: {snapshot.CommunicationsInput?.Name ?? "unchanged"}",
                $"Output mute: {snapshot.OutputMuteAction}",
                $"Microphone mute: {snapshot.InputMuteAction}",
                $"Hotkey: {snapshot.Hotkey}",
                $"Hotkey cycle group: {snapshot.HotkeyCycleGroup}",
                $"Triggers (any): {FormatRoutineTriggerSummary(routine, redactOutput)}",
                $"Conditions (all): {snapshot.Conditions.Summary}",
                $"Target application: {snapshot.TargetAppPath}",
                $"App-only audio: {snapshot.SwitchOutputPerApp}",
                $"Restore previous audio: {snapshot.RestorePreviousAudioOnDeactivate}",
                $"Show in tray menu: {snapshot.ShowInTrayMenu}");
        }
        ...
        public static string FormatRoutineList(IReadOnlyList<AudioRoutine> routines, bool jsonOutput, bool redactOutput = false)
        {
            ...
            for (int index = 0; index < routines.Count; index++)
            {
                ...
                items.Add(new CliRoutineItem(
                    ...
                    FormatOptionalPath(routine.TargetAppPath, redactOutput) ?? string.Empty,
                    ...
                    routine.HasScheduledTrigger && routine.NotifyBeforeScheduledRun,
                    routine.RestorePreviousAudioOnDeactivate,
                    routine.SwitchOutputPerApp,
                    routine.ShowInTrayMenu,
            ...
            foreach (CliRoutineItem item in items)
            {
                ...
                if (!string.IsNullOrWhiteSpace(item.ScheduleTime))
                {
                    ...
                    if (item.NotifyBeforeScheduledRun)
                    {
                        builder.Append(" | Reminder: 1 minute before");
                    }
                }
        ...
        private static RoutineDeviceReference? FormatRoutineDevice(RoutineDeviceReference? device, bool redactOutput) => device == null || !redactOutput ? device : device with
        {
            Id = FormatDeviceId(device.Id, true),
            StableId = string.IsNullOrWhiteSpace(device.StableId) ? device.StableId : FormatDeviceId(device.StableId, true),
            Name = FormatOptionalDeviceName(device.Name, true) ?? string.Empty,
        };
        ...
        private static string GetRoutineTriggerMode(AudioRoutine routine)
        {
            return routine.TriggerKind switch
            {
                ...
                RoutineTriggerKind.DeviceAvailability => "Device availability",
                RoutineTriggerKind.SessionUnlock => "Windows unlock",
                RoutineTriggerKind.SystemResume => "System resume",
        ...
        private static string FormatRoutineTriggerSummary(AudioRoutine routine, bool redactOutput)
        {
            ...
            summary = ReplaceSensitiveValue(summary, routine.HotkeyCycleGroup, RedactLabel(routine.HotkeyCycleGroup, "routine-group"));
            ...
            {
                string appName = RoutineTriggerPathHelper.GetTriggerDisplayName(routine.TargetAppPath);
                summary = ReplaceSensitiveValue(summary, appName, FormatProcessName(appName, redactOutput: true));
            }
            ...
            foreach (RoutineTrigger trigger in routine.Triggers.OfType<RoutineTrigger>())
            {
                if (trigger.Device != null) summary = ReplaceSensitiveValue(summary, trigger.Device.Name, FormatOptionalDeviceName(trigger.Device.Name, true) ?? string.Empty);
                summary = ReplaceSensitiveValue(summary, trigger.TitlePattern, RedactLabel(trigger.TitlePattern, "window-title"));
                summary = ReplaceSensitiveValue(summary, trigger.NetworkName, RedactLabel(trigger.NetworkName, "network"));
                string displayName = RoutineTriggerPathHelper.GetTriggerDisplayName(trigger.AppPath);
                summary = ReplaceSensitiveValue(summary, displayName, FormatProcessName(displayName, redactOutput: true));
            }
        ...
        private static string FormatRoutineTargetSummary(AudioRoutine routine, bool redactOutput)
        {
            ...
            if (routine.SwitchOutputPerApp) parts.Add($"App: {FormatProcessName(RoutineTriggerPathHelper.GetTriggerDisplayName(routine.TargetAppPath), redactOutput: true)}");
            ...
            if (routine.CommunicationsOutput != null) parts.Add($"Communications output: {FormatDeviceName(routine.CommunicationsOutput.Name, redactOutput)}");
            if (routine.CommunicationsInput != null) parts.Add($"Communications microphone: {FormatDeviceName(routine.CommunicationsInput.Name, redactOutput)}");
            if (routine.OutputMuteAction != RoutineMuteAction.Unchanged) parts.Add($"Output: {routine.OutputMuteAction}");
            if (routine.InputMuteAction != RoutineMuteAction.Unchanged) parts.Add($"Microphone: {routine.InputMuteAction}");
        ...
        public static string FormatRoutineRunResult(AudioRoutine routine, string? appliedOutputDeviceName, string? appliedInputDeviceName, bool jsonOutput, bool redactOutput = false)
        {
            ...
            var snapshot = new CliRoutineRunSnapshot(
                ...
                diagCode, routine.OutputMuteAction, routine.InputMuteAction, FormatRoutineDevice(routine.CommunicationsOutput, redactOutput), FormatRoutineDevice(routine.CommunicationsInput, redactOutput));
            ...
            if (routine.CommunicationsOutput != null) targets.Add($"communications output '{FormatDeviceName(routine.CommunicationsOutput.Name, redactOutput)}'");
            if (routine.CommunicationsInput != null) targets.Add($"communications microphone '{FormatDeviceName(routine.CommunicationsInput.Name, redactOutput)}'");
            if (routine.OutputMuteAction != RoutineMuteAction.Unchanged) targets.Add($"Output: {routine.OutputMuteAction}");
            if (routine.InputMuteAction != RoutineMuteAction.Unchanged) targets.Add($"Microphone: {routine.InputMuteAction}");
            ...
            bool redactOutput = false)
        {
            string displayMessage = redactOutput
                ? RedactRoutineMessage(message, routine, targetApplicationName)
                : message;
            ...
            if (jsonOutput)
            {
                var snapshot = new CliRoutineErrorSnapshot(
                    ...
                    FormatOptionalPath(routine?.TargetAppPath, redactOutput),
                    ...
                    FormatOptionalProcessName(targetApplicationName, redactOutput),
                    requiresRunningTargetApplication,
                    routine?.RestorePreviousAudioOnDeactivate,
                    routine?.SwitchOutputPerApp,
