    ...
    public static partial class CliOutputFormatter
    {
        ...
        public static string FormatRoutineDetails(AudioRoutine routine, bool jsonOutput, bool redactOutput = false)
        {
            var snapshot = new
            {
                ...
                routine.Hotkey,
                TriggerKind = routine.TriggerKind.ToString(),
                TriggerAppPath = FormatOptionalPath(routine.TriggerAppPath, redactOutput) ?? string.Empty,
                routine.SwitchOutputPerApp,
                ApplicationTriggerMode = routine.ApplicationTriggerMode.ToString(),
                ApplicationTriggerTitlePattern = redactOutput ? RedactLabel(routine.ApplicationTriggerTitlePattern, "window-title") : routine.ApplicationTriggerTitlePattern,
                ApplicationTriggerTitleMatchMode = routine.ApplicationTriggerTitleMatchMode.ToString(),
                routine.RestorePreviousAudioOnDeactivate,
                routine.EnforceTargetsOnDeviceChange,
                routine.ShowInTrayMenu,
                routine.NotifyBeforeScheduledRun,
                ScheduleTime = routine.ScheduleTime.ToString("HH:mm", CultureInfo.InvariantCulture),
                ScheduleDays = routine.ScheduleDays.Order().Select(static day => day.ToString()).ToArray(),
                routine.ScheduleTimeZoneId,
                TriggerNetworkName = FormatNetworkName(routine.TriggerNetworkName, redactOutput),
                NetworkTriggerDirection = routine.NetworkTriggerDirection.ToString(),
            };
            ...
            return string.Join(Environment.NewLine,
                ...
                $"Trigger: {snapshot.TriggerKind}",
                $"Application: {snapshot.TriggerAppPath}",
                $"Application mode: {snapshot.ApplicationTriggerMode}",
                $"Title filter: {snapshot.ApplicationTriggerTitlePattern}",
                $"Title match mode: {snapshot.ApplicationTriggerTitleMatchMode}",
                $"App-only audio: {snapshot.SwitchOutputPerApp}",
                $"Restore previous audio: {snapshot.RestorePreviousAudioOnDeactivate}",
                $"Enforce targets on device change: {snapshot.EnforceTargetsOnDeviceChange}",
                $"Show in tray menu: {snapshot.ShowInTrayMenu}",
                $"Scheduled reminder: {snapshot.NotifyBeforeScheduledRun}",
                $"Schedule time: {snapshot.ScheduleTime}",
                $"Schedule days: {(snapshot.ScheduleDays.Length == 0 ? "Daily" : string.Join(", ", snapshot.ScheduleDays))}",
                $"Schedule time zone: {snapshot.ScheduleTimeZoneId}",
                $"Network: {snapshot.TriggerNetworkName}",
                $"Network direction: {snapshot.NetworkTriggerDirection}");
        }
