...
namespace AudioPilot.Cli
{
    public sealed partial class CliCommand
    {
        ...
        private static bool TryParseStartupCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            CliAction action = tokens[1] switch
            {
                "enable" => CliAction.StartupEnable,
                "disable" => CliAction.StartupDisable,
                _ => CliAction.None,
            };
            ...
            if (!CliOutputFlagParser.TryParse(tokens, args, 2, out bool startupJsonOutput, out bool startupRedactOutput, out error))
            {
                return false;
            }
            command = new CliCommand { Action = action, JsonOutput = startupJsonOutput, RedactOutput = startupRedactOutput };
            return true;
        }
        ...
        private static bool TryParseSwitchCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            for (int i = 2; i < tokens.Length; i++)
            {
                switch (tokens[i])
                {
                    ...
                    case "--device-id":
                        ...
                        {
                            error = "Use only one device selector: --device or --device-id.";
                            return false;
                        }
                        if (i + 1 >= args.Length || string.IsNullOrWhiteSpace(args[i + 1]) || tokens[i + 1].StartsWith("--", StringComparison.Ordinal))
                        {
                            error = $"Missing value for {args[i]}.";
                            return false;
                        }
                        deviceSelector = tokens[i] == "--device"
                            ? CliDeviceSelectorResolver.EncodeExactName(args[++i])
                            : CliDeviceSelectorResolver.EncodeExactId(args[++i]);
                        break;
                    case "--reverse":
                        reverse = true;
                        break;
            ...
            if (reverse && deviceSelector != null)
            {
                error = "--reverse cannot be combined with a direct device selector.";
                return false;
            }
            ...
            switch (tokens[1])
            {
                case "output":
                    command = new CliCommand
                    {
                        Action = deviceSelector == null ? CliAction.SwitchOutput : CliAction.SwitchOutputToDevice,
                        Reverse = reverse,
                        MuteMic = muteMic,
                        MuteSound = muteSound,
                        Deafen = deafen,
                ...
                case "input":
                    ...
                    command = new CliCommand
                    {
                        Action = deviceSelector == null ? CliAction.SwitchInput : CliAction.SwitchInputToDevice,
                        Reverse = reverse,
                        DryRun = dryRun,
                        JsonOutput = jsonOutput,
                        RedactOutput = redactOutput,
