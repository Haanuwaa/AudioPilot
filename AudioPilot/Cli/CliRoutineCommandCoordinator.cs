        ...
        internal async Task<CliExecutionResult> RunAsync(IReadOnlyList<AudioRoutine> routines, string selector, bool jsonOutput, bool redactOutput = false)
        {
            ...
            if (!CliRoutineExecutionPolicy.TryResolveManualRunProcessId(routine, processSnapshots, out int? processId, out string? errorCode, out string? errorMessage))
            {
                recordHistory(new ExecutionHistoryEntry(
                    ...
                    RoutineId: routine.Id,
                    RoutineName: routine.Name,
                    Target: CliRoutineExecutionPolicy.GetApplicationDisplayName(routine.TargetAppPath),
                    DiagCode: errorCode ?? "routine-target-app-not-running",
                    Details: new Dictionary<string, string> { ["trigger"] = routine.TriggerKind.ToString(), ["executionSource"] = "cli" }));
            ...
            if (result.SkipCode != null)
                return BuildErrorResult(5, result.SkipCode, result.SkipReason ?? "Routine conditions were not met.", jsonOutput, routine, redactOutput: redactOutput);
            ...
            if (!success)
            {
                ...
                return BuildErrorResult(
                    3,
                    "routine-run-failed",
                    $"Failed to run routine '{routine.Name}'." + ((result.CommunicationsFailureDetail ?? result.MuteFailureDetail) is { } detail ? $" {detail}" : string.Empty),
                    jsonOutput,
                    routine,
