...
namespace AudioPilot.Cli
{
    ...
    {
        private static async Task<CliExecutionResult> ExecuteMediaCommandAsync(CliCommand command, ICliCommandRuntime runtime)
        {
            if (command.Action is CliAction.MediaPlay or CliAction.MediaPause)
            {
                MediaPlaybackResult result = await runtime.MediaSetPlayingAsync(command.Action == CliAction.MediaPlay);
                return new CliExecutionResult(result.Success ? 0 : 1,
                    command.JsonOutput ? CliOutputFormatter.SerializeCliJson(result) : result.Message);
            }

            if (command.Action == CliAction.MediaSeek)
            {
                int? seconds = null;
                if (command.Value != null)
                {
                    if (!MediaSeekStep.TryParse(command.Value, out int parsed))
                    {
                        return BuildErrorResult(2, "media-seek-invalid-step", "Use a seek step between 1 and 3600 seconds.", command.JsonOutput);
                    }

                    seconds = parsed;
                }

                MediaSeekResult result = await runtime.MediaSeekAsync(command.Reverse, seconds);
                return new CliExecutionResult(result.Success ? 0 : 1, CliOutputFormatter.FormatMediaSeek(result, command.JsonOutput));
            }

            if (command.Action == CliAction.MediaStatus)
            {
                return new CliExecutionResult(
                    0,
                    await runtime.GetMediaStatusAsync(command.JsonOutput, command.RedactOutput));
            }
        ...
        private static async Task<CliExecutionResult> ExecuteMediaAndVolumeCommandAsync(CliCommand command, ICliCommandRuntime runtime)
        {
            if (command.Action is CliAction.SwitchOutputToDevice or CliAction.SwitchInputToDevice)
            {
                return await runtime.SwitchToDeviceAsync(command);
            }

            if (command.Action is CliAction.SwitchOutput or CliAction.SwitchInput)
            {
                return await ExecuteSwitchAsync(command, runtime, output: command.Action == CliAction.SwitchOutput);
            }
            ...
            return command.Action switch
            {
                ...
                CliAction.VolumeAdjustMaster => ExecuteVolumeWrite(command, runtime, playback: true, relative: true),
                CliAction.VolumeAdjustMic => ExecuteVolumeWrite(command, runtime, playback: false, relative: true),
        ...
        {
            var (success, output) = runtime.GetVolume(playback, command.Key, command.JsonOutput, command.RedactOutput);
            return new CliExecutionResult(success ? 0 : 3, output);
        }

        private static CliExecutionResult ExecuteVolumeWrite(CliCommand command, ICliCommandRuntime runtime, bool playback, bool relative = false)
        {
            ...
            if (!float.TryParse(command.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out float percent) || !float.IsFinite(percent) || percent < (relative ? -100f : 0f) || percent > 100f)
            {
                return BuildErrorResult(2, "invalid-volume-percent", relative ? "Invalid volume adjustment. Use a number between -100 and 100." : "Invalid volume percent. Use a number between 0 and 100.", command.JsonOutput);
            }

            var (success, output) = runtime.SetVolume(playback, command.Key, percent, command.JsonOutput, command.RedactOutput, relative);
            return new CliExecutionResult(success ? 0 : 3, output);
        }
