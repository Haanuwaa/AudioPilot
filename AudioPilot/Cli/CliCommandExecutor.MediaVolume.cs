...
namespace AudioPilot.Cli
{
    public static partial class CliCommandExecutor
    {
        ...
        private static CliExecutionResult ExecuteVolumeWrite(CliCommand command, ICliCommandRuntime runtime, bool playback)
        {
            ...
            if (!float.TryParse(command.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out float percent) || !float.IsFinite(percent) || percent < 0f || percent > 100f)
            {
            ...
            var (success, output) = runtime.SetVolume(playback, command.Key, percent, command.JsonOutput, command.RedactOutput);
            return new CliExecutionResult(success ? 0 : 3, output);
        }
