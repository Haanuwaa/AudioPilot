...
namespace AudioPilot.Cli
{
    ...
    public interface ICliCommandRuntime
    {
        ...
        Task<MediaSeekResult> MediaSeekAsync(bool backward, int? seconds);
        Task<MediaPlaybackResult> MediaSetPlayingAsync(bool playing);
        ...
        (bool Success, string Output) SetVolume(bool playback, string? deviceId, float percent, bool jsonOutput, bool redactOutput = false, bool relative = false);
        /// <summary>Returns detached routines for read-only CLI queries without exposing live settings.</summary>
        IReadOnlyList<AudioRoutine> GetRoutinesSnapshot();
        ...
        Task<CliExecutionResult> SwitchToDeviceAsync(CliCommand command);
        Task RefreshAsync();
    ...
    public static partial class CliCommandExecutor
    {
        ...
        {
            using var trace = OperationTrace.Start($"cli-{command.Action}");
            string? previousDirectory = CliPathPolicy.WorkingDirectory;
            CliPathPolicy.WorkingDirectory = command.WorkingDirectory;
            try
            {
                CliExecutionResult result = command.Action switch
                {
                ...
                trace.Complete(result.ExitCode == 0 ? "success" : $"exit-{result.ExitCode}");
                return result;
            }
            ...
            {
                CliPathPolicy.WorkingDirectory = previousDirectory;
            }
        }
        ...
        private static CliExecutionResult BuildErrorResult(int exitCode, string errorCode, string message, bool jsonOutput)
        {
            if (!jsonOutput)
            {
                return new CliExecutionResult(exitCode, CliHostUtilities.FormatTextError(errorCode, message));
            }
        ...
        private static async Task<CliExecutionResult> ExecuteNetworkCommandAsync(CliCommand command, ICliCommandRuntime runtime)
        {
            ...
            {
                Logging.Logger.Instance.Error("CliCommandExecutor", "cli-network-list-failed", nameof(ExecuteNetworkCommandAsync), ex);
                return BuildErrorResult(7, "network-list-failed", "Failed to scan networks.", command.JsonOutput);
            }
        }
    }
}
