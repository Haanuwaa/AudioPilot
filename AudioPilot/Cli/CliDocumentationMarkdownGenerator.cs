    ...
    internal static class CliDocumentationMarkdownGenerator
    {
        ...
        internal static string GenerateHelpTopicsSection()
        {
            ...
            foreach (string topic in CliCommandHelpMetadata.Topics)
            {
                ...
                {
                    builder.AppendLine();
                    AppendWrappedNote(builder, note);
                }
        ...
        internal static string GenerateCommandsSection()
        {
            ...
            builder.AppendLine();
            builder.AppendLine("Grouped help is also available as `<topic> help`, for example `AudioPilot.Cli.exe diagnostics help` or");
            builder.AppendLine("`AudioPilot.Cli.exe volume help`.");
            builder.AppendLine("When a help topic or grouped subcommand is close to a known value, text-mode parse errors also include a");
            builder.AppendLine("`Did you mean ...?` suggestion.");
            builder.AppendLine();
        ...
        private static void AppendWrappedNote(StringBuilder builder, string note)
        {
            const int lineLength = 120;
            while (note.Length > lineLength)
            {
                int boundary = note.LastIndexOf(' ', lineLength);
                if (boundary <= 0) break;
                builder.AppendLine(note[..boundary]);
                note = note[(boundary + 1)..];
            }
