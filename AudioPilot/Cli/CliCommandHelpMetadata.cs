namespace AudioPilot.Cli
{
    internal static class CliCommandHelpMetadata
    {
        ...
        private static readonly Dictionary<ParserUsageId, string> ParserUsages = new()
        {
            ...
            [ParserUsageId.Media] = "media play-pause|next|previous; media status [--json] [--redact]; media seek forward|backward [duration] [--json]",
            [ParserUsageId.Mute] = "mute mic|sound|deafen [toggle|on|off] [--json]",
            [ParserUsageId.Listen] = "listen toggle|on|off [--json] [--redact]",
            [ParserUsageId.Volume] = "volume get master|mic [--device <name>|--device-id <id>] [--json] [--redact] | volume set master|mic <percent> [--device <name>|--device-id <id>] [--json] [--redact]",
            ...
            [ParserUsageId.Config] = "config list [--json] | config get <key> [--json] [--redact] | config set <key> <value> | config export <path.json|path.zip> [--json] [--redact] [--allow-any-path] | config import <path.json|path.zip> [--merge(default)|--replace] [--json] [--redact] [--allow-any-path] | config validate [--json] [--redact]",
        ...
        [
            ...
            "audio-pilot diagnostics export-bundle <path.zip> [--json] [--detail summary|manifest] [--allow-any-path] [--include-sensitive]",
        ...
        [
            ...
            "audio-pilot media seek forward|backward [duration] [--json]",
        ];
        ...
        [
            "audio-pilot volume get master|mic [--device <name>|--device-id <deviceId>] [--json] [--redact]",
            "audio-pilot volume set master|mic <percent> [--device <name>|--device-id <deviceId>] [--json] [--redact]",
        ];

        private static readonly string[] RoutineUsageLines =
        [
            "audio-pilot routine list [--json] [--redact]",
            "audio-pilot routine get <id|name> [--json] [--redact]",
            "audio-pilot routine next <id|name> [--json] [--redact]",
        ...
        private static readonly string[] NetworkUsageLines = ["audio-pilot network list [--json] [--redact]"];
        ...
        [
            new("core", CoreUsageLines, ["Use a command's --help option or audio-pilot help core for these one-shot commands."]),
            ...
            new("media", MediaUsageLines, ["Use media status when you need the current media snapshot in text or JSON form. Play/pause and track navigation remain fire-and-forget. Seeking reports the result; durations accept seconds or minutes (90, 90s, 1.5m, 1m30s, or 1:30). Omit the duration to use media-seek-step-seconds (default 10 seconds, range 1 second to 60 minutes). Seeking requires the selected player to expose a seekable timeline; live streams and some browser sessions may not support it."]),
            ...
            new("runtime", RuntimeUsageLines, ["Runtime changes affect only the current AudioPilot process and are reset when that process exits. Use config set for the subset of tuning keys that can be persisted."]),
