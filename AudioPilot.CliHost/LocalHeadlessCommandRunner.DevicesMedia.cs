...
namespace AudioPilot.CliHost
{
    internal sealed partial class LocalHeadlessCommandRunner
    {
            ...
            bool usePollingFallbackWhenOverridesPresent)
        {
            ...
            if (CanSubscribeToDeviceStateChanged())
            {
                ...
                while (!condition())
                {
                    ...
                    try
                    {
                        if (!await signal.WaitAsync(TimeSpan.FromMilliseconds(remainingMs), cancellationToken))
                        {
                            return false;
                        }
                    }
        ...
        public (bool Success, string Output) GetVolume(bool playback, string? deviceId, bool jsonOutput, bool redactOutput = false)
        {
            ...
            {
                return (false, CliOutputFormatter.FormatVolumeError(kind, "volume-get-failed", selectorError ?? "Failed to read volume.", jsonOutput, normalizedDeviceId, redactOutput));
            }
            ...
            {
                return (false, CliOutputFormatter.FormatVolumeError(kind, "volume-get-failed", HasDefaultEndpoint(playback, resolvedDeviceId) ? readFailureMessage : unavailableMessage, jsonOutput, resolvedDeviceId, redactOutput));
            }
        ...
        public (bool Success, string Output) SetVolume(bool playback, string? deviceId, float percent, bool jsonOutput, bool redactOutput = false)
        {
            ...
            if (!TryResolveCliVolumeDevice(playback, normalizedDeviceId, out string? resolvedDeviceId, out string? selectorError))
            {
                return (false, CliOutputFormatter.FormatVolumeError(kind, "volume-set-failed", selectorError ?? "Failed to set volume.", jsonOutput, normalizedDeviceId, redactOutput));
            }
            ...
            {
                return (false, CliOutputFormatter.FormatVolumeError(kind, "volume-set-failed", HasDefaultEndpoint(playback, resolvedDeviceId) ? writeFailureMessage : unavailableMessage, jsonOutput, resolvedDeviceId, redactOutput));
            }

            return (true, CliOutputFormatter.FormatVolumeResult(kind, appliedPercent, muted, jsonOutput, "volume-set-success", resolvedDeviceId, redactOutput));
        }
