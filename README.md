...
It is built for people who switch output or input devices often, want reliable global hotkeys, and do not want to dig
through Windows audio panels every time their setup changes.
...
- Test any configured output or microphone directly from its switch-order context menu without changing Windows
  defaults.
- Run routines triggered by hotkeys, application launch or focus, scheduled times, network connections, Steam Big
  Picture, device changes, or tray actions.
...
- Download: [GitHub Releases](https://github.com/Haanuwaa/AudioPilot/releases)
...
- App target: `.NET 10` WPF on `net10.0-windows10.0.19041.0`
- Installer note: `x64` and `arm64` releases include per-user MSI installers as an alternate install path; portable ZIP
  packages remain available for all supported architectures
- Runtime note: framework-dependent ZIP packages require the .NET Desktop Runtime 10; self-contained ZIP packages
  include it
...
| --- | --- | --- |
| Most modern Windows PCs | `SelfContained-win-x64.zip` | Includes the runtime; extract the whole folder and run AudioPilot |
| Windows on ARM | `SelfContained-win-arm64.zip` | Native ARM64 build with the runtime included |
| Windows installations that require 32-bit binaries | `SelfContained-win-x86.zip` | Use only when you specifically need 32-bit support; MSI is not provided for x86 |
...
4. Add the devices you want in the **Output Switch Order** and **Input Switch Order** lists.
...
| Check a configured speaker or microphone | Right-click it in the Output or Input switch-order list | Tests that endpoint directly; microphone samples stay in memory and are never recorded to a file |
| Auto-switch when a game or app starts | Routines | Use app-start routines, with optional app-only routing |
| Keep one app on another device | Routines | Use app-only output/input routing instead of moving system defaults |
...
AudioPilot.Cli.exe volume adjust mic -10
...
- UI-only commands such as `show`, `hide`, `startup open`, and `runtime set` require a running UI host.
- CLI `show` and `hide` are deterministic commands; the global window hotkey toggles visibility.
...
Combine automatic triggers with “any trigger” behavior, alongside optional hotkey and tray access. Overlapping active
...
On Windows, install the .NET 10 SDK and PowerShell 7, then run:
