...
{
    private static readonly HashSet<string> ExcludedSourceDirectories = new(StringComparer.OrdinalIgnoreCase)
    {
        "bin", "obj", "artifacts", ".artifacts", ".git", ".vs", "TestResults", "node_modules",
    };
    ...
    public void ApplicationSources_DoNotRepeatProjectGlobalUsings()
    {
        ...
        foreach (string sourcePath in EnumerateRepositorySourceFiles(applicationRoot, "*.cs"))
        {
    ...
    {
        string repositoryRoot = ResolveRepositoryRoot();
        string[] violations = [.. EnumerateRepositorySourceFiles(
    ...
    public void SupportedWindowsFloor_IsAlignedAcrossBuildInstallerAndReleaseMetadata()
    {
        ...
        AssertFileContains(repositoryRoot, "scripts/release-body.ps1", "Windows 10 version 2004 (build 19041) or later");
    ...
    {
        string repositoryRoot = ResolveRepositoryRoot();
        IEnumerable<string> xamlFiles = EnumerateRepositorySourceFiles(
            Path.Combine(repositoryRoot, "AudioPilot"),
            "*.xaml");
    ...
    public void RepositoryText_UsesNeutralProfileNamesInWindowsPathFixtures()
    {
        ...
        foreach (string sourceRoot in sourceRoots)
        {
            ...
            foreach (string path in EnumerateRepositorySourceFiles(absoluteSourceRoot, "*"))
            {
    ...
    public void AnalyzerPolicy_DoesNotUsePragmasOrProjectWideNoWarnSuppressions()
    {
        ...
        string repositoryRoot = ResolveRepositoryRoot();
        foreach (string projectFile in EnumerateRepositorySourceFiles(repositoryRoot, "*.csproj"))
        {
            Assert.DoesNotContain("<NoWarn>", File.ReadAllText(projectFile), StringComparison.Ordinal);
        }

        foreach (string sourceFile in EnumerateRepositorySourceFiles(repositoryRoot, "*.cs"))
        {
    ...
    public void DialogPolicy_ConfinesWpfMessageBoxesToEmergencyFallback()
    {
        ...
        foreach (string path in EnumerateRepositorySourceFiles(sourceRoot, "*.cs"))
        {
    ...
    public void InstallerPolicy_PersistsOptionsAndReleaseWorkflowUsesOneVersion()
    {
        ...
        XDocument installerDefinition = XDocument.Parse(package);
        foreach (string option in new[] { "INSTALLDESKTOPSHORTCUT", "INSTALLSTARTMENUSHORTCUT", "ADD_CLI_TO_PATH", "AUDIOPILOT_CLEAN_UNINSTALL" })
        {
            var declaration = installerDefinition.Descendants().Single(element => element.Name.LocalName == "Property" && (string?)element.Attribute("Id") == option);
            Assert.Null(declaration.Attribute("Value"));
            Assert.Equal("yes", (string?)declaration.Attribute("Secure"));
            var assignments = installerDefinition.Descendants().Where(element => element.Name.LocalName == "SetProperty" && (string?)element.Attribute("Id") == option).ToArray();
            Assert.NotEmpty(assignments);
            Assert.All(assignments, element => Assert.Equal("first", (string?)element.Attribute("Sequence")));
        }
        ...
        var runCleanup = installerDefinition.Descendants().Single(element => element.Name.LocalName == "CustomAction" && (string?)element.Attribute("Id") == "CleanupStartupRunValue");
        Assert.Equal("immediate", (string?)runCleanup.Attribute("Execute"));
        Assert.Equal("WixQuietExec", (string?)runCleanup.Attribute("DllEntry"));
        var runCleanupSequence = installerDefinition.Descendants().Single(element => element.Name.LocalName == "Custom" && (string?)element.Attribute("Action") == "CleanupStartupRunValue");
        Assert.Equal("CleanupStartupScheduledTask", (string?)runCleanupSequence.Attribute("After"));
    ...
    [Theory]
    [InlineData("MainWindow.xaml", "SettingsHotkeyExpanderStyle")]
    ...
    /// <summary>Checks semantic contrast; supplemental borders use a lower threshold because fills, labels, and focus outlines identify controls.</summary>
    [Theory]
    ...
    /// <summary>Scans source without descending into mutable build outputs, local artifacts, or directory links.</summary>
    private static FileSystemEnumerable<string> EnumerateRepositorySourceFiles(string root, string pattern)
    {
        ...
        {
            ShouldRecursePredicate = static (ref entry) => !ExcludedSourceDirectories.Contains(entry.FileName.ToString()),
            ShouldIncludePredicate = (ref entry) => !entry.IsDirectory
                && !entry.FileName.EndsWith("_wpftmp.csproj", StringComparison.OrdinalIgnoreCase)
                && FileSystemName.MatchesSimpleExpression(pattern, entry.FileName, ignoreCase: true),
        };
    }
