...
public sealed partial class ProjectConfigurationPolicyTests
{
    ...
    public void FullTestRun_IsolatesNativeIntegrationAndStressCategories()
    {
        ...
        Assert.Contains("& pwsh @childArguments", testRunner, StringComparison.Ordinal);
    ...
    [Fact]
    public void HighChurnSources_StayBehindExtractedDomainBoundaries()
    {
        string repositoryRoot = ResolveRepositoryRoot();
        var maximumLineCounts = new Dictionary<string, int>(StringComparer.Ordinal)
        {
            ["AudioPilot/MainWindow.xaml"] = 1650,
            ["AudioPilot/ViewModels/AppViewModel.cs"] = 700,
            ["AudioPilot/ViewModels/MixerViewModel.cs"] = 1600,
            ["AudioPilot/Services/UI/MediaOverlay/MediaOverlayTrackNavigationRecoveryCoordinator.cs"] = 1500,
            ["AudioPilot/Services/Audio/VolumeControlService.cs"] = 1000,
            ["AudioPilot/Services/Audio/AudioDeviceService.cs"] = 700,
        };

        foreach ((string relativePath, int maximumLines) in maximumLineCounts)
        {
            string path = Path.Combine(repositoryRoot, relativePath);
            int lineCount = File.ReadLines(path).Count();
            Assert.True(lineCount <= maximumLines, $"{relativePath} grew to {lineCount} lines; its policy ceiling is {maximumLines}. Extract the new domain instead of expanding the host file.");
        }

        AssertFileContains(repositoryRoot, "AudioPilot/MainWindow.xaml", "<views:SettingsDeviceSwitchingPanel />");
        AssertFileContains(repositoryRoot, "AudioPilot/MainWindow.xaml", "<views:SettingsGeneralPanel />");
        AssertFileContains(repositoryRoot, "AudioPilot/MainWindow.xaml", "<views:OutputDevicePanel x:Name=\"OutputDevicePanel\" />");
        AssertFileContains(repositoryRoot, "AudioPilot/MainWindow.xaml", "<views:InputDevicePanel Loaded=\"InputDevicePanel_Loaded\" />");
        AssertFileContains(repositoryRoot, "AudioPilot/ViewModels/MixerViewModel.cs", "MixerSharedSessionCoordinator");
        AssertFileDoesNotContain(repositoryRoot, "AudioPilot/ViewModels/MixerViewModel.cs", "class SharedSessionBridge");
        AssertFileContains(repositoryRoot, "AudioPilot/Services/UI/MediaOverlay/MediaOverlayTrackNavigationRecoveryCoordinator.cs", "MediaOverlayTrackNavigationRecoveryStateMachine");

        foreach (string requiredDomainFile in new[]
        {
            "AudioPilot/ViewModels/AppViewModel.State.cs",
            "AudioPilot/ViewModels/MixerSharedSessionCoordinator.cs",
            "AudioPilot/Services/UI/MediaOverlay/MediaOverlayTrackNavigationRecoveryPolicy.cs",
            "AudioPilot/Services/UI/MediaOverlay/MediaOverlayTrackNavigationRecoveryStateMachine.cs",
            "AudioPilot/Services/Audio/VolumeControlService.Capture.cs",
            "AudioPilot/Services/Audio/VolumeControlService.Mute.cs",
            "AudioPilot/Services/Audio/AudioDeviceService.Switching.cs",
            "AudioPilot/Services/Audio/Testing/AudioEndpointTestService.cs",
            "AudioPilot/Views/OutputDevicePanel.xaml",
            "AudioPilot/Views/InputDevicePanel.xaml",
        })
        {
            Assert.True(File.Exists(Path.Combine(repositoryRoot, requiredDomainFile)), $"Required extracted domain file is missing: {requiredDomainFile}");
        }
    }
    ...
    [Fact]
    public void CliCommandParsing_RemainsSplitByCommandDomain()
    {
        string repositoryRoot = ResolveRepositoryRoot();
        string cliDirectory = Path.Combine(repositoryRoot, "AudioPilot", "Cli");
        string[] requiredDomains = ["Config", "Control", "Devices", "Diagnostics", "Media", "Routines"];

        foreach (string domain in requiredDomains)
        {
            string path = Path.Combine(cliDirectory, $"CliCommand.{domain}.cs");
            Assert.True(File.Exists(path), $"CLI parser domain is missing: {domain}");
            Assert.True(File.ReadLines(path).Count() <= 500, $"CLI parser domain {domain} exceeded 500 lines and should be decomposed further.");
        }
    }
    ...
    public void TestPlatform_UsesXunitV3WithMicrosoftTestingPlatformV2Conventions()
    {
        ...
        Assert.Contains("Include=\"xunit.v3\" Version=\"4.0.0\"", packages, StringComparison.Ordinal);
        Assert.Contains("Include=\"xunit.runner.visualstudio\" Version=\"4.0.0\"", packages, StringComparison.Ordinal);
