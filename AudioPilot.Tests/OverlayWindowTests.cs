...
{
    [Theory]
    [InlineData(AppTheme.Dark, OverlayActionStateKind.Enabled, "OverlaySuccessTextBrush")]
    [InlineData(AppTheme.Dark, OverlayActionStateKind.Disabled, "OverlayErrorDeviceBrush")]
    [InlineData(AppTheme.Dark, OverlayActionStateKind.Neutral, "OverlayPrimaryTextBrush")]
    [InlineData(AppTheme.Light, OverlayActionStateKind.Enabled, "OverlaySuccessTextBrush")]
    [InlineData(AppTheme.Light, OverlayActionStateKind.Disabled, "OverlayErrorDeviceBrush")]
    [InlineData(AppTheme.Light, OverlayActionStateKind.Neutral, "OverlayPrimaryTextBrush")]
    public void ActionDetail_UsesStateColorAndNeutralHeadingInBothThemes(AppTheme theme, OverlayActionStateKind kind, string brushKey)
    {
        TestExecutionGuards.RunSta(() =>
        {
            ...
            try
            {
                ...
                window.UpdateContent(kind, "Push-to-talk", "Microphone state");
                var state = window.GetStructuredOverlayTextStateForTests();
                Assert.Equal("Push-to-talk", state.Header);
                Assert.Equal("Microphone state", Assert.Single(state.Rows, row => row.Visibility == Visibility.Visible).Value);
                var value = (TextBlock)window.FindName("StructuredRow1ValueText");
                var run = Assert.IsType<Run>(Assert.Single(value.Inlines));
                Assert.Equal(((SolidColorBrush)window.FindResource(brushKey)).Color, ((SolidColorBrush)run.Foreground).Color);
                var header = (TextBlock)window.FindName("StructuredHeaderText");
                var headingRun = Assert.IsType<Run>(Assert.Single(header.Inlines));
                Assert.Equal(((SolidColorBrush)window.FindResource("OverlayPrimaryTextBrush")).Color, ((SolidColorBrush)headingRun.Foreground).Color);
            }
            finally { window.Cleanup(); }
        });
    }
    ...
    [InlineData(1.5, 540)]
    [InlineData(3, 1080)]
    public void LayoutConstraints_PreserveLogicalWidthAcrossDisplayScales(double scale, int expectedWidthPx)
    {
        TestExecutionGuards.RunSta(() =>
        {
            OverlayWindow window = CreateOverlayWindow();
            try
            {
                window.UpdateContent("Current track", "CHALLENGER ADC IS BACK", "humzh");
                int width = window.ApplyLayoutConstraints(3792, 1968, scale, scale);
                Assert.Equal(expectedWidthPx, width);
                Assert.Equal(360, window.Width);

                window.ApplyLayoutConstraints(2544, 1376, 1, 1);
                Assert.Equal(360, window.Width);
                Assert.Equal(100, window.MinHeight);
            }
            finally { window.Cleanup(); }
        });
    }
    ...
    [Fact]
    public void LayoutConstraints_ConstrainSmallWorkAreaAndRestoreNormalMinimumHeight()
    {
        TestExecutionGuards.RunSta(() =>
        {
            OverlayWindow window = CreateOverlayWindow();
            try
            {
                Assert.Equal(240, window.ApplyLayoutConstraints(240, 150, 3, 3));
                Assert.Equal(80, window.Width);
                Assert.Equal(50, window.MinHeight);
                Assert.Equal(50, window.MaxHeight);
                window.ApplyLayoutConstraints(3792, 1968, 3, 3);
                Assert.Equal(360, window.Width);
                Assert.Equal(100, window.MinHeight);
            }
            finally { window.Cleanup(); }
    ...
    [Fact]
    public void UpdateAudioStatusContent_ShowsDeviceLevelsAndIndependentMuteStates()
    {
        TestExecutionGuards.RunSta(() =>
        {
            OverlayWindow window = CreateOverlayWindow();
            try
            {
                window.UpdateAudioStatusContent(new(
                    new(AudioEndpointStatusKind.Available, "Desk speakers", 0, false),
                    new(AudioEndpointStatusKind.Available, "USB microphone", 75.5f, true)));

                var state = window.GetStructuredOverlayTextStateForTests();
                Assert.Equal("Audio status", state.Header);
                ...
                Assert.Equal(["Desk speakers", "0% · Unmuted", "USB microphone", "76% · Muted"], state.Rows.Select(row => row.Value));
                Assert.All(state.Rows, row => Assert.Equal(Visibility.Visible, row.Visibility));

                window.UpdateAudioStatusContent(new(new(AudioEndpointStatusKind.NoDevice), new(AudioEndpointStatusKind.Unavailable)));
                state = window.GetStructuredOverlayTextStateForTests();
                Assert.Equal(["No default device", "Status unavailable"], state.Rows.Where(row => row.Visibility == Visibility.Visible).Select(row => row.Value));
            }
            finally
            {
                window.Cleanup();
    ...
    [Fact]
    public void MediaSeekDetailIsShownAndClearedWhenOverlayIsReused()
    {
        TestExecutionGuards.RunSta(() =>
        {
            OverlayWindow window = CreateOverlayWindow();
            try
            {
                window.UpdateContent("Seek forward", "Song", "Artist", "1:24 → 1:34");
                ...
                Assert.Equal("1:24 → 1:34", state.Detail);
                Assert.Equal(Visibility.Visible, state.DetailVisibility);

                window.UpdateContent("Next track", "Other song", null);
                state = window.GetMediaOverlayTextStateForTests();
                Assert.Equal(string.Empty, state.Detail);
                Assert.Equal(Visibility.Collapsed, state.DetailVisibility);
                Assert.Equal(Visibility.Collapsed, state.ArtistVisibility);

                window.UpdateContent("Seek backward", "Other song", null, "1:34 → 1:24");
                window.UpdateContent("Player timeline unavailable");
                state = window.GetMediaOverlayTextStateForTests();
                Assert.Equal(Visibility.Collapsed, state.MediaPanelVisibility);
    ...
    [Fact]
    public void ReusedOverlay_ClearsIconWhenNextMessageHasNoIcon()
    {
        TestExecutionGuards.RunSta(() =>
        {
            ...
            try
            {
                window.UpdateContent(OverlayDeviceKind.Input, "Input device", "Microphone");
                var icon = (System.Windows.Shapes.Path)window.FindName("StructuredGlyph");
                Assert.Equal(Visibility.Visible, icon.Visibility);
                Assert.Same(AudioPilot.Controls.AppMenuGlyphs.Input, icon.Data);
                window.UpdateContent("Plain message");
                Assert.Equal(Visibility.Collapsed, icon.Visibility);
                Assert.Null(icon.Data);
                window.UpdateContent("Current track", "Title", "Artist");
                Assert.Same(AudioPilot.Controls.AppMenuGlyphs.Media, ((System.Windows.Shapes.Path)window.FindName("MediaGlyph")).Data);
                window.UpdateAudioStatusContent(new(new(AudioEndpointStatusKind.NoDevice), new(AudioEndpointStatusKind.NoDevice)));
                Assert.Equal(Visibility.Collapsed, icon.Visibility);
            }
            finally { window.Cleanup(); }
        });
