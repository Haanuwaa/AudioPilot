...
{
    [Fact]
    public void MultipleNetworkTriggers_CoalesceTheSameConnectivityEvent()
    {
        var monitor = new FakeNetworkMonitor(["Home", "VPN"]);
        var routine = new AudioRoutine
        {
            Id = "multi",
            Enabled = true,
            MasterVolumePercent = 30,
            Triggers = [new RoutineTrigger { Id = "first", Kind = RoutineTriggerKind.Network, NetworkName = "Home", NetworkDirection = NetworkTriggerDirection.Disconnect }, new() { Id = "vpn", Kind = RoutineTriggerKind.Network, NetworkName = "VPN", NetworkDirection = NetworkTriggerDirection.Disconnect }]
        };
        var executed = new List<string>();
        using var coordinator = new NetworkTriggerCoordinator(new([routine]),
            (trigger, _) => executed.Add(trigger.Id), Logger.Instance, monitor);
        coordinator.Start();
        monitor.SimulateConnectivityChange([]);
        Assert.Equal("multi", Assert.Single(executed));
    }

    [Fact]
    public void NamedDisconnect_FiresWhenTargetDisappearsWhileOtherNetworksRemain()
    {
        var monitor = new FakeNetworkMonitor(["HomeWiFi", "Ethernet", "VPN"]);
        var named = new AudioRoutine
        {
            Id = "named",
            Enabled = true,
        ...
        var any = named.Clone();
        any.Id = "all";
        any.TriggerNetworkName = string.Empty;
        var executed = new List<string>();
        using var coordinator = new NetworkTriggerCoordinator(new([named, any]),
            (routine, _) => executed.Add(routine.Id), Logger.Instance, monitor);
        coordinator.Start();

        monitor.SimulateConnectivityChange(["HomeWiFi", "Ethernet"]);
        Assert.Empty(executed);
        monitor.SimulateConnectivityChange(["Ethernet"]);
        Assert.Equal(["named"], executed);
        monitor.SimulateConnectivityChange([]);
        Assert.Equal(["named", "all"], executed);
    }
