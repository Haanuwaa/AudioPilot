...
{
    [Fact]
    public void DisablingAndReenablingSchedule_PreservesAlreadyExecutedOccurrence()
    {
        DateTime now = new(2026, 9, 28, 9, 0, 0, DateTimeKind.Utc);
        var routine = new AudioRoutine
        {
            Name = "Morning",
            MasterVolumePercent = 40,
            Triggers = [new() { Kind = RoutineTriggerKind.Scheduled, Time = new(9, 0), TimeZoneId = "UTC" },
                new() { Kind = RoutineTriggerKind.Network, NetworkName = "Office" }]
        };
        int executions = 0;
        using var coordinator = new ScheduleTriggerCoordinator([routine], (_, _) => executions++, Logger.Instance, nowProvider: () => now);
        coordinator.Start();
    ...
    [Fact]
    public void MultipleSchedules_KeepIndependentHistoryAndCoalesceTheSameOccurrence()
    {
        DateTime now = new(2026, 9, 28, 8, 58, 0, DateTimeKind.Utc);
        var routine = new AudioRoutine
        {
            Id = "multi",
            Enabled = true,
            MasterVolumePercent = 35,
            Triggers = [new RoutineTrigger { Id = "first", Kind = RoutineTriggerKind.Scheduled, Time = new(9, 0), TimeZoneId = "UTC" }, new() { Id = "monday", Kind = RoutineTriggerKind.Scheduled, Time = new(9, 0), TimeZoneId = "UTC", Days = [DayOfWeek.Monday] }, new() { Id = "later", Kind = RoutineTriggerKind.Scheduled, Time = new(9, 30), TimeZoneId = "UTC" }]
        };
        var fired = new List<string>();
        using var coordinator = new ScheduleTriggerCoordinator([routine], (trigger, _) => fired.Add(trigger.RuntimeTriggerKey), Logger.Instance, nowProvider: () => now);
        coordinator.Start();
        ...
        coordinator.CheckScheduledRoutinesForTests();
        Assert.Equal(["multi/first", "multi/later"], fired);
    }
    ...
    public void QueuedReminders_RevalidateSavedStateBeforeShowing(string change)
    {
        ...
        var queued = new ScheduledRoutineReminder[] { new(Assert.Single(original.ExpandAutomaticTriggers()), now.AddMinutes(1)) };
