...
public sealed partial class AppRuntimeStartupResumeCoordinatorTests
{
    ...
    public async Task HandlePowerModeChanged_SkipsDuplicateResumeSignals_WhenRecoveryIsAlreadyInProgress()
    {
        ...
        long tick = 0;
        var coordinator = CreateCoordinator(loggerScope.Logger, recoveryHandler, getTickCount: () => tick);
    ...
    [Fact]
    public async Task HandlePowerModeChanged_RecoversNewSleepCycle_AfterActiveRecoveryCompletes()
    {
        using var loggerScope = new TestLoggerScope(nameof(AppRuntimeStartupResumeCoordinatorTests), "resume-trailing.log");
        var handler = new FakeResumeRecoveryHandler { BlockUntilReleased = true };
        Task worker = Task.CompletedTask;
        using var coordinator = CreateCoordinator(loggerScope.Logger, handler, work => worker = work());

        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "first");
        await handler.WaitForBlockEntryAsync().WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Suspend), "sleep");
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "second");
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "duplicate");
        Assert.Equal(1, handler.InvocationCount);
        handler.Release();
        await worker.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);

        Assert.Equal(2, handler.InvocationCount);
    }

    [Fact]
    public async Task HandlePowerModeChanged_DoesNotStartQueuedRecoveryDuringSuspend_AndRecoversOnResume()
    {
        using var loggerScope = new TestLoggerScope(nameof(AppRuntimeStartupResumeCoordinatorTests), "resume-queued-suspend.log");
        var handler = new FakeResumeRecoveryHandler();
        Func<Task>? queued = null;
        using var coordinator = CreateCoordinator(loggerScope.Logger, handler, work =>
        {
            queued = work;
    ...
    [Fact]
    public void HandlePowerModeChanged_UsesElapsedTimeForCooldown_AndNewSleepBypassesIt()
    {
        using var loggerScope = new TestLoggerScope(nameof(AppRuntimeStartupResumeCoordinatorTests), "resume-clock.log");
        var handler = new FakeResumeRecoveryHandler();
        long tick = 0;
        using var coordinator = CreateCoordinator(loggerScope.Logger, handler, work => work(), () => tick);
        ...
        Assert.Equal(1, handler.InvocationCount);
        tick = 1000;
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "later");
        Assert.Equal(2, handler.InvocationCount);
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Suspend), "sleep");
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "new-cycle");
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void HandlePowerModeChanged_ContainsQueueFailure_AndAllowsRetry(bool synchronousFailure)
    {
        using var loggerScope = new TestLoggerScope(nameof(AppRuntimeStartupResumeCoordinatorTests), "resume-queue-failure.log");
        var handler = new FakeResumeRecoveryHandler();
        bool reject = true;
        using var coordinator = CreateCoordinator(loggerScope.Logger, handler, work =>
        {
        ...
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "rejected");
        Assert.Equal(0, handler.InvocationCount);
        reject = false;
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), "retry");

        Assert.Equal(1, handler.InvocationCount);
        Assert.Contains("power-resume-queue-failed", loggerScope.DisposeAndReadLogText());
    }
    ...
    private static AppRuntimeStartupResumeCoordinator CreateCoordinator(
        Logger logger, IResumeRecoveryHandler recoveryHandler,
        Func<Func<Task>, Task>? queueResumeRecoveryWork = null, Func<long>? getTickCount = null)
    {
        return CreateCoordinator(
            ...
            new AppRuntimeStartupResumeDependencies(
                ...
                InitializeStartupAsync: static _ => Task.CompletedTask,
                CaptureInitialHotplugSnapshot: static () => { }),
            queueResumeRecoveryWork: queueResumeRecoveryWork,
    ...
    private static AppRuntimeStartupResumeCoordinator CreateCoordinator(
        ...
        Func<Func<Task>, Task>? queueResumeRecoveryWork = null,
