...
public sealed class AppResumeRecoveryCoordinatorTests
{
    ...
    {
        var success = new ResumeHotkeyRegistrationResult(
            true, true, true, true, true, true, true, true, true);

        var failed = new ResumeHotkeyRegistrationResult(
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task RegisterHotkeysAsync_RetriesOnce_WhenInitialAttemptFails(bool audioStatusFails)
    {
        int attempts = 0;

        (ResumeHotkeyRegistrationResult Result, int Attempts) = await AppResumeRecoveryCoordinator.RegisterHotkeysAsync(
            () =>
            {
                attempts++;
                return Task.FromResult(new ResumeHotkeyRegistrationResult(
                    ToggleAppVisibilityRegistered: audioStatusFails || attempts > 1,
                    ...
                    ShowAudioStatusRegistered: !audioStatusFails || attempts > 1));
            },
            retryDelayMs: 0,
            Logger.Instance,
            ...
            new ResumeRecoveryExecutionDependencies(
                ...
                RegisterHotkeysAsync: _ =>
                {
                    ...
                    return Task.FromResult((
                        new ResumeHotkeyRegistrationResult(true, true, true, true, true, true, true, true, true),
                        Attempts: 1));
                },
                RefreshDevicesAsync: () =>
                {
    ...
    [InlineData("hotkeys")]
    [InlineData("devices")]
    public async Task ExecuteAsync_RecoversRemainingPhases_WhenOnePhaseThrows(string failedPhase)
    {
        int hotkeyCalls = 0;
        int refreshCalls = 0;
        ...
        ResumeRecoveryExecutionResult result = await AppResumeRecoveryCoordinator.ExecuteAsync(
            "resume:test",
            new ResumeRecoveryExecutionDependencies(
                RecoverAudioAsync: () => failedPhase == "audio" ? throw new InvalidOperationException("boom") : Task.CompletedTask,
                RegisterHotkeysAsync: _ =>
                {
                    hotkeyCalls++;
                    if (failedPhase == "hotkeys")
                    {
                ...
                {
                    refreshCalls++;
                    return failedPhase == "devices" ? throw new InvalidOperationException("boom") : Task.CompletedTask;
                }),
            Logger.Instance,
            failureMethodName: "test",
            cancellationToken: TestContext.Current.CancellationToken);
        ...
        Assert.Equal(failedPhase == "hotkeys" ? 0 : 1, result.HotkeyAttempts);
        Assert.Equal(0, result.HotkeyFailedCount);
        Assert.Equal(1, hotkeyCalls);
        Assert.Equal(1, refreshCalls);
    }
    ...
    [Fact]
    public async Task ExecuteAsync_ReturnsPartialFailure_WhenHotkeysStillFailAfterRetry()
    {
        int refreshCalls = 0;
        ...
        ResumeRecoveryExecutionResult result = await AppResumeRecoveryCoordinator.ExecuteAsync(
            "resume:partial",
            new ResumeRecoveryExecutionDependencies(
                RecoverAudioAsync: () => Task.CompletedTask,
                RegisterHotkeysAsync: _ => Task.FromResult((
                    new ResumeHotkeyRegistrationResult(
                        true, true, true, true, true, true, true, true, true,
                        RoutineHotkeysRegistered: false),
                    Attempts: 2)),
                RefreshDevicesAsync: () =>
                {
                    refreshCalls++;
        ...
        Assert.False(result.Succeeded);
        Assert.Equal(2, result.HotkeyAttempts);
        Assert.Equal(1, result.HotkeyFailedCount);
        Assert.Equal(1, refreshCalls);
    }
