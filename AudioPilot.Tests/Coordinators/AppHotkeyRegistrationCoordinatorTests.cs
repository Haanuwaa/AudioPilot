...
public sealed class AppHotkeyRegistrationCoordinatorTests : IDisposable
{
    ...
    [Fact]
    public void SharedGroup_MapsRegistrationFailureToEveryMember()
    {
        Assert.True(_hotkeys.RegisterToggleAppVisibilityHotkey("Ctrl+Alt+R"));
        var coordinator = new AppHotkeyRegistrationCoordinator(_hotkeys, Logger.Instance);
        var first = new AudioRoutine { Id = "first", Hotkey = "Ctrl+Alt+R", HotkeyCycleGroup = "Desk", MasterVolumePercent = 30 };
        var second = first.Clone();
        second.Id = "second";
        var result = coordinator.RegisterRoutineHotkeys([first, second], _ => { });
        Assert.True(result.HasFailures);
        Assert.Equal(0, result.RegisteredGroupCount);
        Assert.Equal(result.AttemptedHotkeyIdsByRoutineId[first.Id], result.AttemptedHotkeyIdsByRoutineId[second.Id]);
    }

    [Fact]
    public async Task SharedGroup_RegistersOnceMapsEveryMemberAndRefreshesAfterRepresentativeRemoval()
    {
        var coordinator = new AppHotkeyRegistrationCoordinator(_hotkeys, Logger.Instance);
        var activations = Channel.CreateUnbounded<string>();
        var first = new AudioRoutine { Id = "first", Hotkey = "Ctrl+Alt+R", HotkeyCycleGroup = "Desk", MasterVolumePercent = 30, DisplayOrder = 1 };
        var second = new AudioRoutine { Id = "second", Hotkey = "Ctrl+Alt+R", HotkeyCycleGroup = "desk", MasterVolumePercent = 50, DisplayOrder = 2 };
        var disabled = second.Clone();
        disabled.Id = "disabled";
        disabled.Enabled = false;
        var result = coordinator.RegisterRoutineHotkeys([first, second, disabled], routine => activations.Writer.TryWrite(routine.HotkeyCycleGroup));
        Assert.Equal(1, result.RegisteredGroupCount);
        Assert.Equal(0, result.FailedGroupCount);
        Assert.Equal(1, _messageHost.RegisterCount);
        Assert.Equal(result.AttemptedHotkeyIdsByRoutineId[first.Id], result.AttemptedHotkeyIdsByRoutineId[second.Id]);
        Assert.False(result.AttemptedHotkeyIdsByRoutineId.ContainsKey(disabled.Id));
        InvokeHotkeyById(result.AttemptedHotkeyIdsByRoutineId[first.Id]);
        Assert.Equal("Desk", await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken));
        _messageHost.ResetCounts();
        coordinator.RegisterRoutineHotkeys([first, second], _ => { });
        Assert.Equal(0, _messageHost.RegisterCount);
        result = coordinator.RegisterRoutineHotkeys([second], routine => activations.Writer.TryWrite(routine.Id));
        _utcNowTicks += TimeSpan.TicksPerSecond;
        InvokeHotkeyById(result.AttemptedHotkeyIdsByRoutineId[second.Id]);
        Assert.Equal("second", await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken));
        second.Hotkey = "Ctrl+Alt+T";
        result = coordinator.RegisterRoutineHotkeys([first, second], _ => { });
        Assert.True(result.HasFailures);
        Assert.Equal(0, result.RegisteredGroupCount);
        Assert.False(_messageHost.Trigger(AppConstants.Hotkeys.RoutineHotkeyIdBase));
    }
    ...
    [Fact]
    public void RegisterChangedGlobalHotkeys_AudioStatusBindingIsIndependentAndCanBeCleared()
    {
        var coordinator = new AppHotkeyRegistrationCoordinator(_hotkeys, Logger.Instance);
        var initial = new Settings();
        coordinator.RegisterAll(initial);
        _messageHost.ResetCounts();
        Settings updated = initial.Clone();
        updated.Hotkeys.App.ShowAudioStatus = "Ctrl+Alt+A";

        HotkeyRegistrationResult result = coordinator.RegisterChangedGlobalHotkeys(initial, updated);

        Assert.True(result.ShowAudioStatusAttempted);
        Assert.True(result.ShowAudioStatusRegistered);
        Assert.False(result.ToggleAppVisibilityAttempted);
        Assert.False(result.MediaHotkeysAttempted);
        Assert.Equal(1, _messageHost.RegisterCount);
        Assert.Equal(0, _messageHost.UnregisterCount);

        _messageHost.ResetCounts();
        result = coordinator.RegisterChangedGlobalHotkeys(updated, updated.Clone());
        Assert.False(result.ShowAudioStatusAttempted);
        Assert.True(result.ShowAudioStatusRegistered);
        Assert.Equal(0, _messageHost.RegisterCount);

        Settings cleared = updated.Clone();
        cleared.Hotkeys.App.ShowAudioStatus = "";
        result = coordinator.RegisterChangedGlobalHotkeys(updated, cleared);
        Assert.True(result.ShowAudioStatusAttempted);
        Assert.True(result.ShowAudioStatusRegistered);
        Assert.Equal(1, _messageHost.UnregisterCount);
        Assert.False(_messageHost.Trigger(AppConstants.Hotkeys.ShowAudioStatusHotkeyId));
    }
    ...
    public async Task RegisterRoutineHotkeys_SkipsDuplicateRoutineHotkeys_AndRegistersUniqueHotkeys()
    {
        ...
        string firstActivation = await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken);

        InvokeHotkeyById(10000);
        _utcNowTicks += AppConstants.Timing.HotkeyDebounceTicks + TimeSpan.TicksPerMillisecond;
        InvokeHotkeyById(10000);
        string secondActivation = await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken);
    ...
    public async Task RegisterRoutineHotkeys_ReplacesPreviousRegistrations_WhenCalledAgain()
    {
        ...
        InvokeHotkeyById(10000);
        string activation = await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public void RegisterAll_WithFullReset_RecreatesUnchangedRoutineHotkeys()
    {
        var coordinator = new AppHotkeyRegistrationCoordinator(_hotkeys, Logger.Instance);
        ...
        RoutineHotkeyRegistrationResult initial = coordinator.RegisterRoutineHotkeys([routine], static _ => { });
        int hotkeyId = initial.AttemptedHotkeyIdsByRoutineId[routine.Id];
        Assert.Equal(HotkeyRegistrationOutcomeKind.Registered, _hotkeys.GetLastRegistrationOutcome(hotkeyId).Kind);

        coordinator.RegisterAll(new Settings(), unregisterAllFirst: true);
        Assert.Equal(HotkeyRegistrationOutcomeKind.None, _hotkeys.GetLastRegistrationOutcome(hotkeyId).Kind);

        RoutineHotkeyRegistrationResult restored = coordinator.RegisterRoutineHotkeys([routine], static _ => { });

        Assert.Equal(1, restored.RegisteredGroupCount);
        Assert.Equal(0, restored.FailedGroupCount);
        Assert.Equal(HotkeyRegistrationOutcomeKind.Registered, _hotkeys.GetLastRegistrationOutcome(hotkeyId).Kind);
    }
    ...
    public void RegisterChangedGlobalHotkeys_ReRegistersOnlyChangedGroup()
    {
        ...
        updated.Hotkeys.App.ToggleAppVisibility = "Ctrl+Alt+Y";

        HotkeyRegistrationResult result = coordinator.RegisterChangedGlobalHotkeys(initial, updated);

        Assert.Equal(1, messageHost.RegisterCount);
        Assert.Equal(1, messageHost.UnregisterCount);
        Assert.True(result.ToggleAppVisibilityAttempted);
        Assert.False(result.MediaHotkeysAttempted);
    ...
    public void RegisterChangedSwitchHotkeys_ReRegistersOnlyChangedSwitchBinding()
    {
        ...
        SwitchHotkeyRegistrationResult initialResult = coordinator.RegisterChangedSwitchHotkeys(null, initial);
    ...
    public async Task RegisterRoutineHotkeys_RetainsUnchangedRegistrations_AndRefreshesChangedRoutinePayload()
    {
        ...
        string alphaActivation = await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken);
        Assert.Equal("Alpha", alphaActivation);

        _utcNowTicks += AppConstants.Timing.HotkeyDebounceTicks + TimeSpan.TicksPerMillisecond;
        Assert.True(messageHost.Trigger(10001));
        string betaActivation = await activations.Reader.ReadAsync(TestContext.Current.CancellationToken).AsTask().WaitAsync(HotkeyActivationTimeout, TestContext.Current.CancellationToken);
        Assert.Equal("Beta Updated", betaActivation);
    }

    [Fact]
    public async Task RegisterRoutineHotkeys_AllowsStandaloneSpecialKeys()
    {
        var coordinator = new AppHotkeyRegistrationCoordinator(_hotkeys, Logger.Instance);
        var activations = Channel.CreateUnbounded<string>();
