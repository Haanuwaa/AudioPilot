...
public sealed class AppDebouncedBackgroundWorkCoordinatorTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void BeginDebounce_WhenPreviousOwnerHasDisposedSource_PreservesReplacement(bool useSwapCallback)
    {
        var previous = new CancellationTokenSource();
        CancellationTokenSource? current = previous;
        previous.Dispose();

        using CancellationTokenSource next = useSwapCallback
            ? AppDebouncedBackgroundWorkCoordinator.BeginDebounce(replacement => Interlocked.Exchange(ref current, replacement))
            : AppDebouncedBackgroundWorkCoordinator.BeginDebounce(ref current);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void Cancel_WhenPreviousOwnerHasDisposedSource_ClearsState(bool dispose)
    {
        var previous = new CancellationTokenSource();
        CancellationTokenSource? current = previous;
        previous.Dispose();
        ...
        {
            Assert.Same(previous, AppDebouncedBackgroundWorkCoordinator.CancelAndDetach(ref current));
        }
    ...
    public void ReleaseOwned_DoesNotClearNewerDebounceState()
    {
        ...
        CancellationTokenSource? current = replacement;
    ...
    [Fact]
    public async Task RapidReplacement_SkipsSupersededWorkAndRunsOnlyLatestRequest()
    {
        CancellationTokenSource? current = null;
        CancellationTokenSource first = AppDebouncedBackgroundWorkCoordinator.BeginDebounce(ref current);
        CancellationTokenSource second = AppDebouncedBackgroundWorkCoordinator.BeginDebounce(ref current);
        int firstInvocations = 0;
        int secondInvocations = 0;

        await AppDebouncedBackgroundWorkCoordinator.ExecuteAsync(
            first,
            owned => AppDebouncedBackgroundWorkCoordinator.ReleaseOwned(ref current, owned),
            _ =>
            {
                firstInvocations++;
                return Task.CompletedTask;
