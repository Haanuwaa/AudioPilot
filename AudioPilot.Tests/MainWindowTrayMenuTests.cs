...
public sealed partial class MainWindowTrayMenuTests(ITestOutputHelper output)
{
    [Trait(TestCategories.Name, TestCategories.Integration)]
    [Trait(TestCategories.Name, TestCategories.VisualWpf)]
    [VisualIntegrationFact]
    public void BalloonClick_RestoresSettingsThroughTrayShowPathAndHonorsShutdown()
    {
        if (!TestExecutionGuards.RequireVisualWpfIntegrationEnabled(nameof(BalloonClick_RestoresSettingsThroughTrayShowPathAndHonorsShutdown)))
        {
            return;
        }
        ...
        TestExecutionGuards.RunOnSharedSta(() =>
        {
            ...
            TestPrivateAccess.SetField(service, "_balloonTarget", MainWindowOpenTarget.Settings);

            icon.RaiseEvent(new RoutedEventArgs(TaskbarIcon.TrayBalloonTipClickedEvent));
            ...
            TestPrivateAccess.RunTaskOnDispatcher(Dispatcher.CurrentDispatcher.InvokeAsync(
                static () => { }, DispatcherPriority.ContextIdle).Task);
            Assert.Equal([MainWindowOpenTarget.Settings, MainWindowOpenTarget.Routines], manager.ShowTargets);

            service.BeginShutdown();
            icon.RaiseEvent(new RoutedEventArgs(TaskbarIcon.TrayBalloonTipClickedEvent));
            TestPrivateAccess.RunTaskOnDispatcher(Dispatcher.CurrentDispatcher.InvokeAsync(
                static () => { }, DispatcherPriority.ContextIdle).Task);
            Assert.Equal(2, manager.ShowTargets.Count);
        });
    }
    ...
    private sealed class RecordingTrayRuntimeActions : IAppTrayRuntimeActions
    {
        ...
        public bool ThrowOnReadRoutines { get; set; }
        public IReadOnlyList<AudioRoutine> Routines => ThrowOnReadRoutines
            ? throw new InvalidOperationException("Simulated routine enumeration failure.")
            : [];
