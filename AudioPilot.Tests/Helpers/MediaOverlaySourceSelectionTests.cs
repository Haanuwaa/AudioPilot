...
{
    [Theory]
    [InlineData(false, 0, "no-session")]
    [InlineData(true, null, "session-unavailable")]
    [InlineData(false, 2, "session-unavailable")]
    public void CurrentSnapshotLog_DistinguishesAbsentAndUnreadableSessions(bool hasCurrentSession, int? fallbackCandidateCount, string outcome)
    {
        string message = MediaOverlayEngine.BuildCurrentMediaSnapshotLog(
            MediaOverlaySessionSnapshot.Empty, hasCurrentSession, fallbackCandidateCount, 12);

        Assert.Contains($"outcome={outcome}", message, StringComparison.Ordinal);
        Assert.Contains($"currentSession={hasCurrentSession}", message, StringComparison.Ordinal);
        Assert.Contains("elapsedMs=12", message, StringComparison.Ordinal);
    }

    [Fact]
    public void CurrentSnapshotLog_ReportsMissingMetadataWithoutClaimingTheSessionIsAbsent()
    {
        var snapshot = new MediaOverlaySessionSnapshot(
            GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing, null, null, null, "browser", null);

        string message = MediaOverlayEngine.BuildCurrentMediaSnapshotLog(snapshot, true, null, 3);

        Assert.Contains("outcome=metadata-unavailable", message, StringComparison.Ordinal);
        Assert.Contains("playbackStatus=Playing", message, StringComparison.Ordinal);
        Assert.Contains("hasTitle=False", message, StringComparison.Ordinal);
    }
    ...
    public async Task GetCurrentMediaSnapshotAsync_WithoutCurrentSession_FallsBackToBestMaterializedSession()
    {
        ...
        MediaOverlaySessionSnapshot snapshot = await engine.GetCurrentMediaSnapshotAsync(TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task GetCurrentMediaSnapshotAsync_DoesNotReturnDataFromAProviderThatIgnoresCancellation()
    {
        using var cancellation = CancellationTokenSource.CreateLinkedTokenSource(TestContext.Current.CancellationToken);
        var snapshot = new MediaOverlaySessionSnapshot(
            GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing, "Late title", null, null, "browser", 1);
        var engine = new MediaOverlayEngine(
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => engine.GetCurrentMediaSnapshotAsync(cancellation.Token));
    }

    [Fact]
    public async Task GetCurrentMediaSnapshotAsync_CancelsProviderAtCaptureDeadline()
    {
        var engine = new MediaOverlayEngine(
            ...
            snapshotsBySourceOverride: (_, _) => Task.FromResult(new Dictionary<string, MediaOverlaySessionSnapshot>()),
            sessionSnapshotsOverride: (_, _) => Task.FromResult(new List<MediaOverlaySessionSnapshot>()),
            timingProfile: MediaOverlayTestHarness.CreateTimingProfile(maxCaptureDurationMs: 100));

        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => engine.GetCurrentMediaSnapshotAsync(TestContext.Current.CancellationToken));
    }
