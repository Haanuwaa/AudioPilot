...
public class RelayCommandTests
{
    ...
    public async Task AsyncExecute_DisablesReentryUntilCompletion()
    {
        ...
        await command.LastExecutionTaskForTests.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task ParameterizedAsyncExecute_PassesClickedRowAndHonorsCanExecute()
    {
        object expected = new();
        ...
        var command = new RelayCommand(
            ...
            parameter => ReferenceEquals(parameter, expected));
    ...
    [InlineData(1)]
    [InlineData(2)]
    public async Task AsyncExecute_WhenStateNotificationThrows_ReportsFailureAndAllowsNextExecution(int failingNotification)
    {
        int notifications = 0;
        int executions = 0;
        var failures = new List<Exception>();
        var command = new RelayCommand(
            () => { executions++; return Task.CompletedTask; },
            failures.Add);
        ...
        Assert.True(command.CanExecute(null));
        int beforeRetry = executions;

        command.Execute(null);
        await command.LastExecutionTaskForTests.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        Assert.Equal(beforeRetry + 1, executions);
    }
