...
{
    [Fact]
    public void CandidateFingerprints_DistinguishMetadataSeparators_WhileNormalizingCaseAndWhitespace()
    {
        var first = new MediaOverlaySessionSnapshot(
            GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing, "A|B", "C", "Album", "brave", 0);
        var other = first with { Title = "A", Artist = "B|C" };
        var normalized = first with { Title = " a|b ", Artist = " c ", AlbumTitle = " album ", SourceAppUserModelId = " BRAVE " };

        Assert.NotEqual(MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateFingerprint(first),
            MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateFingerprint(other));
        Assert.NotEqual(MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateTrackFingerprint(first),
            MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateTrackFingerprint(other));
        Assert.Equal(MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateFingerprint(first),
            MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateFingerprint(normalized));
        Assert.Equal(MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateTrackFingerprint(first),
            MediaOverlayBrowserSameSourcePolicy.BuildPendingCandidateTrackFingerprint(normalized));
    }
