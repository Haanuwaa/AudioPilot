...
internal static class TestExecutionGuards
{
    ...
    private const string AudioHardwareEnvVar = "AUDIOPILOT_RUN_AUDIO_HARDWARE";
    ...
    internal static bool ShouldRunAudioHardware() =>
        ShouldRunIntegration() && IsTruthyEnvironmentVariable(AudioHardwareEnvVar);
    ...
    internal static bool ShouldRunHardwareSoak() =>
        ShouldRunAudioHardware() && ShouldRunStress() && IsTruthyEnvironmentVariable(HardwareSoakEnvVar);

    internal static bool ShouldRunVisualWpfIntegration() =>
        ShouldRunIntegration() && IsTruthyEnvironmentVariable(VisualWpfEnvVar) && ShouldShowTestWindows();
    ...
    internal static string GetAudioHardwareSkipReason() =>
        $"Real audio test disabled. These tests can change mute, devices, playback, or recording. Use -Category integration -AllowHardwareAudio, or explicitly set {IntegrationEnvVar}=1 and {AudioHardwareEnvVar}=1.";
    ...
    internal static string GetHardwareSoakSkipReason() =>
        $"Hardware soak disabled. Use -Category hardware-soak -AllowHardwareAudio, or explicitly set {IntegrationEnvVar}=1, {AudioHardwareEnvVar}=1, {StressEnvVar}=1, and {HardwareSoakEnvVar}=1.";

    internal static string GetVisualWpfSkipReason() =>
        $"Visual WPF integration test disabled. Use -Category visual -AllowDesktopInteraction, or explicitly set {IntegrationEnvVar}=1, {VisualWpfEnvVar}=1, and {ShowWindowsEnvVar}=1.";
    ...
    internal static bool RequireAudioHardwareEnabled(string scopeName)
    {
        if (ShouldRunAudioHardware())
        {
            return true;
        }

        Console.WriteLine($"[{scopeName}] {GetAudioHardwareSkipReason()}");
        return false;
    }
    ...
    /// <summary>Runs isolated WPF work and shuts down its dispatcher before the COM apartment exits, releasing native input and composition resources.</summary>
    internal static void RunIsolatedSta(Action action, TimeSpan? timeout = null)
    {
            ...
            finally
            {
                ...
                catch (Exception ex)
                {
                    capturedException = capturedException == null
                        ? ex
                        : new AggregateException("STA test action and dispatcher cleanup both failed.", capturedException, ex);
                }
                finally
                {
        ...
        if (!completed.Task.Wait(waitTimeout))
        {
