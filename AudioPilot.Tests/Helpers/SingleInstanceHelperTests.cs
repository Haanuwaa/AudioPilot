...
public sealed class SingleInstanceHelperTests
{
    [Theory]
    [InlineData("{}")]
    [InlineData("{\"output\":\"done\"}")]
    [InlineData("{\"protocolVersion\":1}")]
    [InlineData("{\"exitCode\":null}")]
    [InlineData("{\"exitCode\":\"0\"}")]
    [InlineData("null")]
    [InlineData("[]")]
    public void ResponseParser_RejectsMissingOrInvalidExitCode(string payload)
    {
        Assert.False(SingleInstanceCommandResultParser.TryParse(payload, out _));
    }
    ...
    [InlineData(0, 1)]
    [InlineData(7, 1)]
    public void ResponseParser_PreservesExplicitExitCodeWithOptionalProtocolVersion(int exitCode, int? protocolVersion)
    {
        var expected = new SingleInstanceCommandResult(exitCode, "result", ProtocolVersion: protocolVersion);

        Assert.True(SingleInstanceCommandResultParser.TryParse(SingleInstanceCommandResultParser.Serialize(expected), out var parsed));

        Assert.Equal(expected, parsed.Response);
    }
    ...
    public async Task TryAcquire_CommandForwarding_ReceivesExitCodeAndOutput()
    {
        ...
        first.CommandRequested += (payload, cancellationToken) =>
        {
    ...
    public async Task TryAcquire_StartupStatusForwarding_ReceivesOutput()
    {
        ...
        first.CommandRequested += (_, _) => Task.FromResult(new SingleInstanceCommandResult(0, "enabled", ProtocolVersion: 1));
    ...
    public async Task TryAcquireDetailed_WhenExistingInstanceStallsAfterConnect_ReturnsConnectionFailure()
    {
        ...
        Assert.False(result.Acquired);
        Assert.True(result.ExistingUnresponsive);
        Assert.Equal(SingleInstanceSignalFailureKind.OutcomeUnknown, result.FailureKind);
        Assert.Equal(SingleInstanceSignalFailureKind.OutcomeUnknown, helper.LastSignalFailureKind);
        await serverTask;
    }

    [Fact]
    public async Task TryAcquire_CommandForwarding_MalformedPayload_ReceivesProtocolMismatch()
    {
        string scope = Guid.NewGuid().ToString("N");
        string mutexName = $"AudioPilot.Tests.Mutex.{scope}";
        string pipeName = $"AudioPilot.Tests.Pipe.{scope}";
    ...
    [Fact]
    public async Task TryAcquire_CommandForwarding_WhenHandlerThrows_ReceivesRuntimeFailure()
    {
        string scope = Guid.NewGuid().ToString("N");
        string mutexName = $"AudioPilot.Tests.Mutex.{scope}";
        string pipeName = $"AudioPilot.Tests.Pipe.{scope}";
        ...
        first.CommandRequested += (_, _) => throw new InvalidOperationException("handler-failed");
    ...
    public async Task BeginShutdown_StopsForwardingButRetainsOwnershipUntilAsyncDisposeCompletes()
    {
        ...
        first.CommandRequested += static (_, _) => Task.FromResult(new SingleInstanceCommandResult(0));
        Assert.True(first.TryAcquire());
        await first.ActivationListenerReadyForTests;
