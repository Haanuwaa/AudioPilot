...
public sealed class LoggerHealthTests : IDisposable
{
    ...
    [Fact]
    public void QueueCapacityReservation_AllowsOnlyRemainingSlots_UnderConcurrency()
    {
        using Logger logger = Logger.CreateInMemoryForTests();
        logger.SetQueueDepthForTests(logger.MaxQueueCapacity - 1);
        int successfulReservations = 0;

        Parallel.For(0, 64, _ =>
        {
            if (logger.TryReserveQueueSlotForTests())
            {
        ...
        Assert.Equal(logger.MaxQueueCapacity, logger.QueueDepth);
        logger.SetQueueDepthForTests(0);
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Dispose_WhenWriterTimesOut_DefersPrimitiveDisposalUntilWriterCompletes(bool disposeAsync)
    {
        var writerCompletion = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        Logger logger = Logger.CreateWithWriterTaskForTests(writerCompletion.Task, disposeWaitMs: 20);
        ...
        Assert.False(logger.WriterPrimitivesDisposedForTests);

        writerCompletion.TrySetResult();
        await TestExecutionGuards.WaitUntilAsync(
            () => logger.WriterPrimitivesDisposedForTests,
            "Logger writer primitives were not disposed after the timed-out writer completed.",
    ...
    public async Task Dispose_WaitsForInFlightMessageFactory_AndPersistsMessage()
    {
        ...
        await factoryEntered.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);

        Task disposeTask = Task.Run(logger.Dispose, TestContext.Current.CancellationToken);
        allowFactoryToFinish.TrySetResult(true);

        await Task.WhenAll(producerTask, disposeTask).WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    public void WriteBatch_WhenFallbackTriggered_EmitsExceptionTypeWithoutRawMessage()
    {
        ...
        TestPrivateAccess.SetField(logger, "_consoleAttached", true);
    ...
    [Fact]
    public void WriteBatch_WhenFileAndConsoleFail_ContainsFailureAndCanResumeFileWrites()
    {
        const string fileName = "health-fallback-recovery.log";
        using var logger = new Logger(_directory.Root, fileName);
        string logPath = Path.Combine(_directory.Root, fileName);
        TestPrivateAccess.SetField(logger, "_logFilePath", _directory.Root);
        TestPrivateAccess.SetField(logger, "_consoleAttached", true);
        MethodInfo? writeBatch = typeof(Logger).GetMethod("WriteBatch", BindingFlags.Instance | BindingFlags.NonPublic);
        Assert.NotNull(writeBatch);
        var entries = new List<Logger.LogEntry>
        {
            new(DateTime.Now, LogLevel.Info, "Test", "queued-message", null, null)
        };

        TextWriter originalOut = Console.Out;
        using var failingOutput = new FailingTextWriter();
        Console.SetOut(failingOutput);
        try
        {
            for (int index = 0; index < 10; index++)
            {
                writeBatch.Invoke(logger, [entries]);
            }
        }
        finally
        {
        ...
        Assert.InRange(failingOutput.WriteAttempts, 1, 4);
        TestPrivateAccess.SetField(logger, "_logFilePath", logPath);
        logger.Info("Test", "file-writer-recovered");
        logger.Dispose();
        Assert.Contains("file-writer-recovered", File.ReadAllText(logPath), StringComparison.Ordinal);
    }

    [Fact]
    public void Error_WithAggregateException_RecordsInnerFailuresAndHResultsWithoutPaths()
    {
        using var scope = TestLoggerScope.CreateInMemory("inner-exceptions.log");
        var exception = new AggregateException("operation failed",
            new IOException("wrapper", new System.Runtime.InteropServices.COMException("device disconnected", unchecked((int)0x88890004))),
            new UnauthorizedAccessException(@"Cannot open C:\private\settings.json"));

        scope.Logger.Error("Test", "error-details", exception: exception);
        string text = scope.DisposeAndReadLogText();

        Assert.Contains("Inner exception: IOException", text, StringComparison.Ordinal);
        Assert.Contains("Inner exception: COMException (HRESULT=0x88890004)", text, StringComparison.Ordinal);
        Assert.Contains("Inner exception: UnauthorizedAccessException", text, StringComparison.Ordinal);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void Error_WithOversizedExceptionDetails_RemainsBounded(bool aggregate)
    {
        using var scope = TestLoggerScope.CreateInMemory("bounded-exceptions.log");
        Exception exception = aggregate
            ? new AggregateException(Enumerable.Range(0, 100).Select(index => new IOException($"failure-{index}")))
            : new IOException(new string('x', 100_000));

        scope.Logger.Error("Test", "bounded-details", exception: exception);
        string text = scope.DisposeAndReadLogText();

        Assert.Contains("[truncated]", text, StringComparison.Ordinal);
        Assert.True(text.Length < AudioPilot.Constants.AppConstants.Logging.MaxExceptionDetailsChars + 200);
        Assert.True(text.Split("Inner exception:", StringSplitOptions.None).Length <= 8);
    }
