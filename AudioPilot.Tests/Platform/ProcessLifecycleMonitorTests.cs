...
{
    [Fact]
    public void PollingTransition_ReportsOnlyChangesInProcessIdOrder()
    {
        HashSet<int> previous = [10, 3, 7];
        HashSet<int> current = [11, 7, 2];

        ProcessLifecycleMonitorTransition result = PollingProcessLifecycleMonitor.CalculateTransition(previous, current);

        Assert.Equal([2, 11], result.StartedProcessIds);
        Assert.Equal([3, 10], result.StoppedProcessIds);
        Assert.True(previous.SetEquals([10, 3, 7]));
        Assert.True(current.SetEquals([11, 7, 2]));
    }

    [Fact]
    public void PollingTransition_HandlesUnchangedAndEmptyProcessSets()
    {
        HashSet<int> processes = [20, 10];
        HashSet<int> empty = [];

        ProcessLifecycleMonitorTransition unchanged = PollingProcessLifecycleMonitor.CalculateTransition(processes, processes);
        ProcessLifecycleMonitorTransition started = PollingProcessLifecycleMonitor.CalculateTransition(empty, processes);
        ProcessLifecycleMonitorTransition stopped = PollingProcessLifecycleMonitor.CalculateTransition(processes, empty);

        Assert.Empty(unchanged.StartedProcessIds);
        Assert.Empty(unchanged.StoppedProcessIds);
        Assert.Equal([10, 20], started.StartedProcessIds);
        Assert.Empty(started.StoppedProcessIds);
        Assert.Empty(stopped.StartedProcessIds);
        Assert.Equal([10, 20], stopped.StoppedProcessIds);
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void FallbackProcessLifecycleMonitor_UsesFallbackWhenEventSourceCleanupFails(bool stopFails)
    {
        using var loggerScope = TestLoggerScope.CreateFileBacked(nameof(ProcessLifecycleMonitorTests), "cleanup-fallback.log", LogLevel.Debug);
        var source = new ThrowingRawProcessLifecycleEventSource(stopFails, disposeFails: true);
        using var primary = new EventDrivenProcessLifecycleMonitor(loggerScope.Logger, () => source, static () => []);
        var fallback = new FakeMonitor(new ProcessLifecycleMonitorStartResult(true, "active"));
        using var monitor = new FallbackProcessLifecycleMonitor(loggerScope.Logger, () => primary, () => fallback);
        List<int> started = [];
        monitor.ProcessStarted += started.Add;
        ...
        Assert.False(primary.IsRunning);
        Assert.True(monitor.IsRunning);
        Assert.Equal([1234], started);
        Assert.Equal(1, source.StopCallCount);
        Assert.Equal(1, source.DisposeCallCount);

        string logText = loggerScope.DisposeAndReadLogText();
        Assert.Contains("event-driven-process-lifecycle-monitor-unavailable | reason=InvalidOperationException", logText, StringComparison.Ordinal);
        Assert.Contains("event-driven-process-lifecycle-cleanup-failed | stage=dispose", logText, StringComparison.Ordinal);
        Assert.Equal(stopFails, logText.Contains("event-driven-process-lifecycle-cleanup-failed | stage=stop", StringComparison.Ordinal));
    ...
    private sealed class ThrowingRawProcessLifecycleEventSource(bool stopFails = false, bool disposeFails = false) : IRawProcessLifecycleEventSource
    {
        public event Action<int>? ProcessStarted
        {
            add { }
        ...
        {
            StopCallCount++;
            if (stopFails) throw new IOException("stop-failed");
        }

        public void Dispose()
        {
            DisposeCallCount++;
            if (disposeFails) throw new IOException("dispose-failed");
        }
    }
