...
{
    [Fact]
    public async Task SubmittedCommandTimeout_SuppressesRetryWithoutClaimingSuccess()
    {
        using var timeout = new CancellationTokenSource();
        timeout.Cancel();
        var pending = new MediaKeyHelper.MediaCommandSendOutcome(false, MediaKeyHelper.MediaCommandRouteKind.CurrentGsmc, CandidateSourceAppUserModelId: "player");

        MediaKeyHelper.MediaCommandSendOutcome result = await MediaKeyHelper.ResolveSubmittedCommandOutcomeAsync(
            Task.FromCanceled<bool>(timeout.Token), pending, TestContext.Current.CancellationToken);

        Assert.False(result.Sent);
        Assert.True(result.SuppressFallback);
        Assert.Equal("gsmc-command-timeout", result.FailureReason);
        Assert.Equal("player", result.CandidateSourceAppUserModelId);
    }

    [Fact]
    public async Task SubmittedCommandCallerCancellation_Propagates()
    {
        using var cancellation = new CancellationTokenSource();
        cancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => MediaKeyHelper.ResolveSubmittedCommandOutcomeAsync(
            Task.FromCanceled<bool>(cancellation.Token), default, cancellation.Token));
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task SubmittedCommandAcknowledgement_PreservesResultAndAllowsRejectedCommandFallback(bool sent)
    {
        var pending = new MediaKeyHelper.MediaCommandSendOutcome(false, MediaKeyHelper.MediaCommandRouteKind.CurrentGsmc);
        MediaKeyHelper.MediaCommandSendOutcome result = await MediaKeyHelper.ResolveSubmittedCommandOutcomeAsync(
            Task.FromResult(sent), pending, TestContext.Current.CancellationToken);

        Assert.Equal(sent, result.Sent);
        Assert.False(result.SuppressFallback);
    }
