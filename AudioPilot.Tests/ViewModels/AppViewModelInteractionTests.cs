...
public sealed partial class AppViewModelInteractionTests : IDisposable
{
    ...
    public void HasRoutineEdits_TracksScheduleOnlyChangesAndRevertedEdits(string field)
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            cached.Routines.Items = [routine];
            harness.SetCachedSettings(cached);
            AudioRoutine edited = routine.Clone();
            harness.ViewModel.Routines.Add(edited);
            Assert.False(harness.ViewModel.HasUnsavedRoutineChanges);
            Assert.False(harness.ViewModel.HasRoutineEdits());

            switch (field)
            {
                case "reminder": edited.NotifyBeforeScheduledRun = true; break;
                case "time": edited.ScheduleTime = new TimeOnly(22, 0); break;
                case "days": edited.ScheduleDays = [DayOfWeek.Friday]; break;
                case "time-zone": edited.ScheduleTimeZoneId = "Eastern Standard Time"; break;
            }
            Assert.True(harness.ViewModel.HasUnsavedRoutineChanges);
            Assert.True(harness.ViewModel.HasRoutineEdits());
            harness.ViewModel.Routines[0] = routine.Clone();
            Assert.False(harness.ViewModel.HasUnsavedRoutineChanges);
            Assert.False(harness.ViewModel.HasRoutineEdits());
        });
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void AutomaticSaveFailure_NotifiesWithoutDialogUnlessShuttingDown(bool shuttingDown)
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            var tray = Assert.IsType<TestAppTrayIconService>(TestPrivateAccess.GetField(shell, "_trayService"));
            harness.SettingsService.OverrideWriteTargetsForTests(
                Path.Combine(_workspace.Root, "<>blocked-primary", "settings.json"),
                Path.Combine(_workspace.Root, "<>blocked-fallback", "settings.json"));
            if (shuttingDown)
            {
                TestPrivateAccess.GetField<CancellationTokenSource>(harness.ViewModel, "_backgroundWorkCts").Cancel();
            }

            TestPrivateAccess.RunTaskOnDispatcher(TestPrivateAccess.InvokeNonPublicTask(
                harness.ViewModel, "SaveSettingsAsync", true));
            TestPrivateAccess.RunTaskOnDispatcher(TestPrivateAccess.InvokeNonPublicTask(
                harness.ViewModel, "SaveSettingsAsync", true));

            Assert.Equal(shuttingDown ? 0 : 1, tray.BalloonCalls);
    ...
    [InlineData("apply")]
    [InlineData("cli")]
    [InlineData("cli-async")]
    public void SettingsWrite_ExplainsNewerSchemaAndPreservesCurrentState(string action)
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            const string newerJson = """{"SchemaVersion":"99.0.0","DeviceSwitching":{"FutureOption":true}}""";
            File.WriteAllText(settingsPath, newerJson);
            string? error;
            if (action is "cli" or "cli-async")
            {
                (bool Updated, string? Error) result;
                if (action == "cli-async")
                {
                    var task = harness.ViewModel.SetConfigFromCliAsync("theme", "Dark");
                    TestPrivateAccess.RunTaskOnDispatcher(task);
                    result = task.GetAwaiter().GetResult();
                }
                else
                {
                    result = harness.ViewModel.SetConfigFromCli("theme", "Dark");
                }
            ...
            {
                TestPrivateAccess.RunTaskOnDispatcher(action == "save"
                    ? harness.ViewModel.SaveSettingsForTestsAsync()
                    : harness.ViewModel.ApplySettingsForTestsAsync());
                error = Assert.Single(harness.Messages.ErrorMessages).message;
            }

            Assert.Contains("Update AudioPilot", error, StringComparison.Ordinal);
            Assert.Equal(newerJson, File.ReadAllText(settingsPath));
            Assert.Equal(cached.Theme, harness.ViewModel.CurrentSettings!.Theme);
            Assert.False(harness.ViewModel.IsSaving);
    ...
    [InlineData("Pause")]
    public void SaveSettingsAsync_UpdatesPersistedCachedSettings_AndSwitchHotkeys(string outputHotkey)
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            harness.ViewModel.Hotkey.SetEditing(true);
            harness.ViewModel.Hotkey.LoadFromString(outputHotkey);
            ...
            Assert.Equal(outputHotkey, current.DeviceSwitching.Output.SwitchHotkey);
            ...
            Assert.Equal(outputHotkey, persisted.DeviceSwitching.Output.SwitchHotkey);
            ...
            Assert.Contains(registrations, entry => entry.Id == AppConstants.Hotkeys.OutputSwitchHotkeyId && entry.Description.Contains(outputHotkey, StringComparison.OrdinalIgnoreCase));
            ...
            Assert.False(harness.ViewModel.Hotkey.HasWarning);
            Assert.False(harness.ViewModel.IsSaving);
    ...
    public void SaveSettingsAsync_PreservesExistingRoutines()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            Assert.True(preserved.ShowInTrayMenu);
        });
    }
    ...
    [Fact]
    public void AutoSave_QueuesTrailingPass_WhenAnEditArrivesDuringAnActiveSave()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            long revisionAtSaveStart = harness.ViewModel.GetAutoSaveDirtyRevisionForTests();
            harness.ViewModel.SetIsSavingForTests(true);
            harness.ViewModel.Theme = AppTheme.Dark;
            harness.ViewModel.SetIsSavingForTests(false);

            Assert.True(harness.ViewModel.GetAutoSaveDirtyRevisionForTests() > revisionAtSaveStart);
            harness.ViewModel.TryQueueTrailingAutoSaveForTests(revisionAtSaveStart, "test-active-save");
    ...
    [Fact]
    public void SetStartupEnabledFromCli_PersistsSettingsRegistryAndLiveState()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            string registryValueName = $"AudioPilotCliToggle_{Guid.NewGuid():N}";
            var registry = new InMemoryUserRegistryAccessor();
            var startupService = new StartupService(registryPath, registryValueName, Logger.Instance, registry, taskStore: new InMemoryStartupTaskStore());
            ...
            bool succeeded = harness.ViewModel.SetStartupEnabledFromCli(enabled: true);

            Assert.True(succeeded);
            Assert.True(harness.ViewModel.RunAtStartup);
            Assert.True(harness.ViewModel.SettingsRunAtStartupDraft);
            Assert.True(harness.SettingsService.LoadSettings().RunAtStartup);
            Assert.NotNull(registry.GetValue(registryPath, registryValueName));
        });
    }
    ...
    public void SaveSettingsAsync_PersistsRunAtStartup_WhenSavingImmediatelyAfterLiveToggle()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            var registry = new InMemoryUserRegistryAccessor();
            var startupService = new StartupService(registryPath, registryValueName, loggerScope.Logger, registry, taskStore: new InMemoryStartupTaskStore());
            using var harness = CreateHarness(
                Dispatcher.CurrentDispatcher,
            ...
            Settings persisted = harness.SettingsService.LoadSettings();
            string? startupEntry = registry.GetValue(registryPath, registryValueName)?.ToString();
    ...
    public void ImportSettingsAsync_ImportsSettings_AndShowsSuccess()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            SettingsTransferService.ExportSettings(new Settings
            {
                Miscellaneous = new MiscellaneousSettings { CheckForUpdates = true },
                DeviceSwitching = new DeviceSwitchingSettings
                {
                    Output = new DeviceSwitchingOutputSettings
                    {
            ...
            Assert.True(current.Miscellaneous.CheckForUpdates);
            Assert.True(persisted.Miscellaneous.CheckForUpdates);
            Assert.True(harness.ViewModel.SettingsCheckForUpdatesDraft);
    ...
    public void ApplySettingsAsync_UpdatesCachedSettings_AndRegistersRuntimeHotkeys()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            SetKnownActiveDeviceInfos(
                harness.ViewModel,
                [new CycleDevice { Id = "out-1", Name = "Speakers", StableId = "stable-output", DisplayOrder = 1 }],
                [new CycleDevice { Id = "in-1", Name = "Microphone", StableId = "stable-input", DisplayOrder = 1 }]);

            harness.ViewModel.SettingsRunAtStartupDraft = true;
            harness.ViewModel.SettingsUseScheduledStartupDraft = true;
            harness.ViewModel.SettingsCheckForUpdatesDraft = true;
            harness.ViewModel.OutputReverseHotkey.LoadFromString("Ctrl+Alt+Shift+O");
            harness.ViewModel.InputReverseHotkey.LoadFromString("Ctrl+Alt+Shift+I");
            harness.ViewModel.SettingsToggleAppVisibilityHotkeyDraft = "Ctrl+Alt+H";
            ...
            harness.ViewModel.SettingsPlayAppSoundsDraft = false;
            Assert.False(harness.Messages.SoundsEnabled);
            ...
            string plaintextDeviceReference = File.ReadAllText(deviceReferencePath);
            Assert.Contains("  Stable ID: stable-output", plaintextDeviceReference, StringComparison.Ordinal);
            Assert.Contains("  Stable ID: stable-input", plaintextDeviceReference, StringComparison.Ordinal);
            ...
            Assert.True(current.Miscellaneous.CheckForUpdates);
            Assert.False(current.DeviceSwitching.PreserveAudioLevels);
            Assert.False(current.DeviceSwitching.BluetoothReconnectEnabled);
            Assert.True(current.RunAtStartup);
            Assert.True(current.Miscellaneous.UseScheduledStartup);
            Assert.True(startup.IsInStartupWithValidPath(useScheduledTask: true));
            Assert.True(harness.SettingsService.LoadSettings().Miscellaneous.UseScheduledStartup);
            ...
            Assert.False(current.Miscellaneous.PlayAppSounds);
            ...
            Assert.DoesNotContain("stable-output", hashedDeviceReference, StringComparison.Ordinal);
            Assert.DoesNotContain("stable-input", hashedDeviceReference, StringComparison.Ordinal);
            Assert.Equal(2, hashedDeviceReference.Split("  Stable ID: sha256:", StringSplitOptions.None).Length - 1);
    ...
    [Fact]
    public void ApplySettingsAsync_ConvertsSeekMinutesWithoutRewritingInputWhileTyping()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            harness.ViewModel.SettingsSeekStepSecondsDraft = "1m ";
            Assert.Equal("1m ", harness.ViewModel.SettingsSeekStepSecondsDraft);
            harness.ViewModel.SettingsSeekStepSecondsDraft = "1m 30s";
            TestPrivateAccess.RunTaskOnDispatcher(harness.ViewModel.ApplySettingsForTestsAsync());

            Settings current = Assert.IsType<Settings>(harness.ViewModel.CurrentSettings);
            Assert.Equal(90, current.Hotkeys.Media.SeekStepSeconds);
            Assert.Equal(90, harness.SettingsService.LoadSettings().Hotkeys.Media.SeekStepSeconds);
            Assert.Equal("1m 30s", harness.ViewModel.SettingsSeekStepSecondsDraft);
        });
    }
    ...
    public void ApplySettingsAsync_Aborts_WhenStartupRegistryUpdateFails()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            harness.ViewModel.SettingsShowAudioStatusHotkeyDraftCapture.LoadFromString("Scroll");
            ...
            Assert.Equal(HotkeyWarningKind.Standalone, harness.ViewModel.SettingsShowAudioStatusHotkeyDraftCapture.WarningKind);
            Assert.Contains(
                harness.Messages.ErrorMessages,
                entry => string.Equals(entry.message, "Failed to apply settings.", StringComparison.Ordinal));
        });
    }

    [Fact]
    public void RunRoutineFromTrayAsync_ReportsMissingConfiguredApplication()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            Assert.Contains(harness.OverlayMessages, message => message.header == "Application not running");
        });
    }
    ...
    [Fact]
    public void SaveSettingsAsync_AppliesEndpointMuteStateAwayFromDispatcher()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            ...
            int microphoneThreadId = 0;
            int playbackThreadId = 0;
            AudioDeviceService.SetMicrophoneMuteOverrideForTests = _ =>
                Volatile.Write(ref microphoneThreadId, Environment.CurrentManagedThreadId);
            AudioDeviceService.SetPlaybackMuteOverrideForTests = _ =>
                Volatile.Write(ref playbackThreadId, Environment.CurrentManagedThreadId);
            ...
            Assert.NotEqual(0, Volatile.Read(ref microphoneThreadId));
            Assert.NotEqual(0, Volatile.Read(ref playbackThreadId));
            Assert.NotEqual(dispatcherThreadId, Volatile.Read(ref microphoneThreadId));
            Assert.NotEqual(dispatcherThreadId, Volatile.Read(ref playbackThreadId));
        });
    }
    ...
    public void RunAtStartup_RestoresRegistration_WhenSettingsPersistenceFails(bool useScheduledTask, bool initiallyEnabled, bool saveImmediately)
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            using var loggerScope = TestLoggerScope.CreateFileBacked(nameof(AppViewModelInteractionTests), "startup-rollback.log", LogLevel.Trace);
            const string registryPath = "SOFTWARE\\AudioPilot.Tests\\StartupRollback";
            const string registryValueName = "AudioPilotTest";
            var registry = new InMemoryUserRegistryAccessor();
            var taskStore = new InMemoryStartupTaskStore();
            var startup = new StartupService(registryPath, registryValueName, loggerScope.Logger, registry, taskStore: taskStore);
            using var harness = CreateHarness(Dispatcher.CurrentDispatcher, logger: loggerScope.Logger,
                startupService: startup, allowBackgroundWork: true);

            Settings cached = BuildCachedSettings();
            cached.RunAtStartup = initiallyEnabled;
            cached.Miscellaneous.UseScheduledStartup = useScheduledTask;
            harness.SetCachedSettings(cached);
            harness.SettingsService.SaveSettings(cached);
            harness.ViewModel.RunAtStartup = initiallyEnabled;
            if (initiallyEnabled) startup.AddToStartup(useScheduledTask: useScheduledTask);
            string? originalTask = taskStore.Xml;
            object? originalRun = registry.GetValue(registryPath, registryValueName);
            string settingsPath = harness.SettingsService.GetSettingsPath();
            harness.SettingsService.OverrideWriteTargetsForTests(settingsPath, Path.Combine(_workspace.Root, "<>invalid-fallback", "settings.json"));
            using var lockedSettings = File.Open(settingsPath, FileMode.Open, FileAccess.Read, FileShare.Read);
            harness.ViewModel.SetIsInitializingForTests(false);

            harness.ViewModel.RunAtStartup = !initiallyEnabled;
            if (saveImmediately) TestPrivateAccess.RunTaskOnDispatcher(harness.ViewModel.SaveSettingsForTestsAsync());
            TestPrivateAccess.RunTaskOnDispatcher(harness.ViewModel.WaitForQueuedBackgroundTasksForTestsAsync());

            Assert.Equal(originalRun, registry.GetValue(registryPath, registryValueName));
            Assert.Equal(originalTask, taskStore.Xml);
            Assert.Equal(saveImmediately ? !initiallyEnabled : initiallyEnabled, harness.ViewModel.RunAtStartup);
            Assert.Equal(saveImmediately ? !initiallyEnabled : initiallyEnabled, harness.ViewModel.SettingsRunAtStartupDraft);
            Assert.Equal(initiallyEnabled, harness.SettingsService.LoadSettings().RunAtStartup);
            Assert.NotEmpty(harness.Messages.ErrorMessages);

            string logText = loggerScope.DisposeAndReadLogText();
            Match opIdMatch = MyRegex().Match(logText);
            Assert.True(opIdMatch.Success, logText);
            string opId = opIdMatch.Groups[1].Value;
            Assert.Contains($"startup-registration-failed | opId={opId} rollbackSucceeded=True", logText, StringComparison.Ordinal);
            if (!saveImmediately) Assert.Contains($"startup-toggle-failed | opId={opId}", logText, StringComparison.Ordinal);
            Assert.DoesNotContain("startup-toggle-applied", logText, StringComparison.Ordinal);
        });
    }
    ...
    public void RunAtStartup_CommitsRegistration_WithOrWithoutAnExistingSettingsFile(bool useScheduledTask, bool settingsExist)
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            var registry = new InMemoryUserRegistryAccessor();
            var startup = new StartupService("SOFTWARE\\AudioPilot.Tests\\FirstRunStartup", "AudioPilotTest", Logger.Instance,
                registry, taskStore: new InMemoryStartupTaskStore());
            using var harness = CreateHarness(Dispatcher.CurrentDispatcher, startupService: startup, allowBackgroundWork: true);
            Settings cached = BuildCachedSettings();
            cached.RunAtStartup = false;
            cached.Miscellaneous.UseScheduledStartup = useScheduledTask;
            harness.SetCachedSettings(cached);
            if (settingsExist) harness.SettingsService.SaveSettings(cached);
            harness.ViewModel.SetIsInitializingForTests(false);

            harness.ViewModel.RunAtStartup = true;
            TestPrivateAccess.RunTaskOnDispatcher(harness.ViewModel.WaitForQueuedBackgroundTasksForTestsAsync());

            Assert.True(startup.IsInStartupWithValidPath(useScheduledTask: useScheduledTask));
            Assert.True(harness.ViewModel.RunAtStartup);
            Assert.Equal(settingsExist, harness.SettingsService.SettingsFileExists());
            if (settingsExist) Assert.True(harness.SettingsService.LoadSettings().RunAtStartup);
            Assert.Empty(harness.Messages.ErrorMessages);

            if (!settingsExist)
            {
                registry.ThrowOnAccess = true;
                harness.ViewModel.RunAtStartup = false;
                TestPrivateAccess.RunTaskOnDispatcher(harness.ViewModel.WaitForQueuedBackgroundTasksForTestsAsync());
                registry.ThrowOnAccess = false;
    ...
    [Fact]
    public void RunAtStartup_SkipsSupersededToggle_AfterWaitingForSettingsWriteLock()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            EnsureApplication();
            var taskStore = new InMemoryStartupTaskStore();
            var startup = new StartupService("SOFTWARE\\AudioPilot.Tests\\StaleStartup", "AudioPilotTest", Logger.Instance,
            ...
            {
                harness.ViewModel.RunAtStartup = true;
                TestPrivateAccess.RunTaskOnDispatcher(Task.Delay(AppConstants.Timing.StartupDebounceMs + 100));
                harness.ViewModel.RunAtStartup = false;
            }
            finally
            {
            ...
            Assert.Equal(0, taskStore.WriteCount);
            Assert.False(harness.SettingsService.LoadSettings().RunAtStartup);
            Assert.False(harness.ViewModel.RunAtStartup);
            Assert.Empty(harness.Messages.ErrorMessages);
        });
    }
    ...
    private static Settings BuildCachedSettings()
    {
        return new Settings
        {
            ...
            Hotkeys = new HotkeysSettings
            {
                ...
                {
                    ToggleAppVisibility = "Ctrl+Alt+H"
                },
