...
public sealed class AppViewModelRoutineRuntimeStateTests
{
    ...
    [InlineData(0)]
    [InlineData(149)]
    public void BulkLoad_PreservesSelectionPublishesCompleteStateAndDoesNotQueueAutoSave(int selectedIndex)
    {
        TestExecutionGuards.RunSta(() =>
        {
            ...
            AppViewModel model = harness.ViewModel;
            model.ApplyRoutinesFromSettings([new() { Id = "keep", Name = "Old", ShowInTrayMenu = true, MasterVolumePercent = 45 }]);
            AudioRoutine detached = model.Routines[0];
            List<AudioRoutine> replacements = [.. Enumerable.Range(0, 250).Select(index => new AudioRoutine
            {
                Id = index == selectedIndex ? "keep" : $"routine-{index}", Name = $"Desk {index}", DisplayOrder = index + 1,
                MasterVolumePercent = 45, ShowInTrayMenu = true,
            })];
            TestPrivateAccess.SetField(model, "_cachedSettings", new Settings { Routines = new() { Items = replacements } });
            long dirtyRevision = TestPrivateAccess.GetField<long>(model, "_autoSaveDirtyRevision");
            List<int> publishedCounts = [];
            bool selectionRefreshed = false;
            model.PropertyChanged += (_, args) => { if (args.PropertyName == nameof(AppViewModel.SelectedRoutine) && model.Routines.Count == 250) selectionRefreshed = true; };
            model.PropertyChanged += (_, args) => { if (args.PropertyName == nameof(AppViewModel.HasRoutines)) publishedCounts.Add(model.Routines.Count); };
            model.ApplyRoutinesFromSettings(replacements);
            Assert.Equal([250], publishedCounts);
            Assert.Equal("keep", model.SelectedRoutine?.Id);
            Assert.True(selectionRefreshed);
            Assert.Equal(Enumerable.Range(1, 250), model.Routines.Select(routine => routine.DisplayOrder));
            Assert.False(model.HasUnsavedRoutineChanges);
            Assert.Equal(dirtyRevision, TestPrivateAccess.GetField<long>(model, "_autoSaveDirtyRevision"));
            detached.Name = "Detached edit";
            Assert.False(model.HasUnsavedRoutineChanges);
            model.Routines[0].Name = "Current edit";
            Assert.True(model.HasUnsavedRoutineChanges);
            model.ApplyRoutinesFromSettings(model.Routines);
            Assert.Equal(250, model.Routines.Count);
            Assert.Equal("Current edit", model.Routines[0].Name);
        });
    }
