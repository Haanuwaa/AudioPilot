...
public sealed class AppViewModelRoutineSaveTests : IDisposable
{
    ...
    [Fact]
    public void RoutineEdits_DetectGroupOnlyChangeAndIgnoreEquivalentReopenedTimeCondition()
    {
        TestExecutionGuards.RunSta(() =>
        {
            ...
            var saved = new AudioRoutine
            {
                ...
                Conditions = new()
                {
                    TimeWindow = new() { Start = new(22, 0), End = new(2, 0), Days = [DayOfWeek.Friday, DayOfWeek.Monday], TimeZoneId = "UTC" },
                },
            };
            TestPrivateAccess.SetField(viewModel, "_cachedSettings", new Settings { Routines = new() { Items = [saved] } });
            using var editor = new RoutineEditorViewModel([], [], saved);
            viewModel.Routines.Add(editor.BuildRoutine());
            Assert.False(viewModel.HasRoutineEdits());
            viewModel.Routines[0].HotkeyCycleGroup = "Desk setups";
            Assert.True(viewModel.HasRoutineEdits());
            viewModel.Routines[0].HotkeyCycleGroup = "";
            Assert.False(viewModel.HasRoutineEdits());
            editor.ConditionTuesday = true;
            viewModel.Routines[0] = editor.BuildRoutine();
            Assert.True(viewModel.HasRoutineEdits());
        });
    }
    ...
    public void SaveRoutinesAsync_UpdatesCachedAndPersistedSettings_AndRefreshesRoutineHotkeys()
    {
        TestExecutionGuards.RunSta(() =>
        {
            ...
            var native = new RecordingAppDialogService();
            TestPrivateAccess.SetField(viewModel, "_dialogs", native);
            try
            {
    ...
    public void SaveRoutinesAsync_DoesNotPersist_WhenConflictConfirmationIsDeclined()
    {
        TestExecutionGuards.RunSta(() =>
        {
            ...
            var native = new RecordingAppDialogService
            {
                YesNoResponse = AppDialogResult.Declined,
            };
            TestPrivateAccess.SetField(viewModel, "_dialogs", native);
            try
            {
