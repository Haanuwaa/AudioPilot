...
public sealed class MixerViewModelChurnTests
{
    ...
    public async Task Refresh_RenamesExistingRowWithoutReplacingItOrOverwritingPendingVolume(
        AudioMixerMode mode, bool pendingVolume)
    {
        MixerViewModel mixer = CreateMixerForApplyTests(mode);
        var item = new AudioSessionItem("Old browser title", 75f, false, false, processId: 42,
            sessionInstanceId: "old-session", endpointId: "old-endpoint");
        var rows = TestPrivateAccess.GetField<ConcurrentDictionary<string, AudioSessionItem>>(mixer, "_sessionsById");
        rows["pid:42"] = item;
        mixer.Sessions.Add(item);
        ...
        if (pendingVolume)
        {
            ...
            stateType.GetField("HasPending")!.SetValue(state, true);
            states.GetType().GetMethod("TryAdd")!.Invoke(states, ["pid:42", state]);
        }

        List<string?> notifications = [];
        int writes = 0;
        item.PropertyChanged += (_, args) => notifications.Add(args.PropertyName);
        item.VolumeChanged += _ => writes++;
        item.MuteChanged += _ => writes++;
        AudioSessionSnapshot[] snapshot =
        [
            new("New browser title", 25f, "Output", "browser", null, 42,
                true, "new-session", "new-endpoint", 2),
        ];

        Task<bool> applyTask = mixer.ApplyRefreshResultsAsync(1, null, null,
            [("pid:42", 25f, true)], CancellationToken.None, snapshot);
        TestPrivateAccess.RunTaskOnDispatcher(applyTask);
        Assert.True(await applyTask);
        Assert.Same(item, Assert.Single(mixer.Sessions));
        Assert.Equal("New browser title", item.DisplayName);
        Assert.Equal("new-session", item.SessionInstanceId);
        Assert.Equal("new-endpoint", item.EndpointId);
        Assert.Equal(2, item.SessionCount);
        Assert.Equal(pendingVolume ? 75f : 25f, item.Volume);
        Assert.Equal(!pendingVolume, item.IsMuted);
        Assert.Equal(0, writes);
        Assert.Single(notifications, name => name == nameof(AudioSessionItem.DisplayName));

        notifications.Clear();
        applyTask = mixer.ApplyRefreshResultsAsync(1, null, null, null, CancellationToken.None, snapshot);
        TestPrivateAccess.RunTaskOnDispatcher(applyTask);
        Assert.True(await applyTask);
        Assert.Empty(notifications);

        TestPrivateAccess.SetField(mixer, "_refreshGeneration", 2);
        applyTask = mixer.ApplyRefreshResultsAsync(1, null, null, null, CancellationToken.None,
            [snapshot[0] with { DisplayName = "Stale title", SessionInstanceId = "stale-session" }]);
        TestPrivateAccess.RunTaskOnDispatcher(applyTask);
        Assert.False(await applyTask);
        Assert.Equal("New browser title", item.DisplayName);
        Assert.Equal("new-session", item.SessionInstanceId);
        Assert.Empty(notifications);
    }
    ...
    [Theory]
    [InlineData("Master Volume")]
    [InlineData("Microphone Volume Controller")]
    [InlineData("System Sounds")]
    public void ProcessNamedLikeSharedControl_RetainsProcessRouting(string displayName)
    {
        var snapshot = new AudioSessionSnapshot(displayName, 50f, "Output", "app", null, 42);
        object?[] arguments = [snapshot, null];
        MethodInfo identityMethod = typeof(MixerViewModel).GetMethod("GetSessionIdentity", BindingFlags.NonPublic | BindingFlags.Static)!;
        Assert.Equal("pid:42", identityMethod.Invoke(null, arguments));
        Assert.Equal(42u, arguments[1]);
        MethodInfo createMethod = typeof(MixerViewModel).GetMethod("CreateSessionItem", BindingFlags.NonPublic | BindingFlags.Static)!;
        var item = (AudioSessionItem)createMethod.Invoke(null, [snapshot, (uint?)42])!;
        Assert.False(item.IsMaster);
        Assert.False(item.IsMic);
        Assert.False(item.IsSystemSounds);
        Assert.Equal(42u, item.ProcessId);
    }

    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void ContinuousEndpointDrag_KeepsTheScheduledWriteDeadline(bool microphone)
    {
        MixerViewModel mixer = CreateMixerForApplyTests();
        var item = new AudioSessionItem("Endpoint", 75f, isMaster: !microphone, isMic: microphone);
        string rowId = microphone ? "mic:primary" : "master:primary";
        object states = TestPrivateAccess.GetField<object>(mixer, "_throttleStates");
        Type stateType = states.GetType().GenericTypeArguments[1];
        object state = Activator.CreateInstance(stateType, nonPublic: true)!;
        using var scheduled = new CancellationTokenSource();
        stateType.GetField("TrailingCts")!.SetValue(state, scheduled);
        stateType.GetField("HasPending")!.SetValue(state, true);
        stateType.GetField("LastApplied")!.SetValue(state, DateTime.UtcNow);
        states.GetType().GetMethod("TryAdd")!.Invoke(states, [rowId, state]);
        try
        {
            InvokeNonPublic(mixer, "OnItemVolumeChanged", item);
            item.SetVolumeFromSystem(63f);
            InvokeNonPublic(mixer, "OnItemVolumeChanged", item);

            Assert.Same(scheduled, stateType.GetField("TrailingCts")!.GetValue(state));
            Assert.False(scheduled.IsCancellationRequested);
            Assert.Equal(63f, stateType.GetField("PendingVolume")!.GetValue(state));
        }
        finally
        {
            mixer.Cleanup();
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void TrailingWrite_OnlyConsumesOwnedPendingValueAndReleasesTimerBeforeAudio(bool staleCallback)
    {
        MixerViewModel mixer = CreateMixerForApplyTests();
        var item = new AudioSessionItem("Game", 75f, false, false, processId: 42);
        object states = TestPrivateAccess.GetField<object>(mixer, "_throttleStates");
        Type stateType = states.GetType().GenericTypeArguments[1];
        object state = Activator.CreateInstance(stateType, nonPublic: true)!;
        using var scheduled = new CancellationTokenSource();
        ...
        stateType.GetField("TrailingCts")!.SetValue(state, scheduled);
        stateType.GetField("HasPending")!.SetValue(state, true);
        stateType.GetField("PendingVolume")!.SetValue(state, 63f);
        states.GetType().GetMethod("TryAdd")!.Invoke(states, ["pid:42", state]);
        TestPrivateAccess.SetField(mixer, "_disposed", true);

        InvokeNonPublic(mixer, "ApplyTrailingEdge", "pid:42", item, staleCallback ? superseded : scheduled);

        Assert.Same(staleCallback ? scheduled : null, stateType.GetField("TrailingCts")!.GetValue(state));
        Assert.Equal(staleCallback, (bool)stateType.GetField("HasPending")!.GetValue(state)!);
    }

    [Fact]
    public void RepeatedMuteToggles_PublishBothUserIntentsWithoutWaitingForCom()
    {
        MixerViewModel mixer = CreateMixerForApplyTests();
        var item = new AudioSessionItem("Game", 80, false, false, processId: 10);
        List<bool> requests = [];
        item.MuteChanged += changed => requests.Add(changed.IsMuted);

        mixer.ToggleMute(item);
    ...
    [Fact]
    public void GroupedRow_SingleSessionNotificationCannotOverwriteCombinedState()
    {
        MixerViewModel mixer = CreateMixerForApplyTests();
        var rows = TestPrivateAccess.GetField<ConcurrentDictionary<string, AudioSessionItem>>(mixer, "_sessionsById");
        var item = new AudioSessionItem("Game", 80, false, false, processId: 10, sessionCount: 2);
        rows["pid:10"] = item;
        InvokeNonPublic(mixer, "ReplaceSessionInstanceMappings", new Dictionary<string, string>
        {
            ["representative"] = "pid:10",
        });

        Assert.False(mixer.ApplySessionStateFromSystem("representative", 20, true));
        Assert.Equal(80, item.Volume);
        Assert.False(item.IsMuted);

        item.UpdateRoutingMetadataFromSystem(null, "representative", "output", sessionCount: 1);
        Assert.True(mixer.ApplySessionStateFromSystem("representative", 20, true));
        Assert.Equal(20, item.Volume);
        Assert.True(item.IsMuted);
    }
    ...
    public async Task QueuedMutations_AfterCleanup_DoNotWriteOrPublishResults(AudioMixerMode mode)
    {
        ...
        Assert.Equal(0, InvokeNonPublic<MixerViewModel.MixerMutationResult>(mixer, "ApplyVolumeChange", item, item.Volume).Attempted);
