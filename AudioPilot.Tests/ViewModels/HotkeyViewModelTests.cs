...
public class HotkeyViewModelTests
{
    ...
    public void HoldBinding_ExplainsUnsuitableInputsAndAcceptsReplacement(string rejected)
    {
        var vm = new HotkeyViewModel { RequiresHeldInput = true };
        Assert.False(vm.LoadFromString(rejected));
        Assert.Equal(HotkeyWarningKind.HoldRequired, vm.WarningKind);
        Assert.Contains("can be held", vm.HoverText);
        Assert.True(vm.LoadFromString("Mouse4"));
        Assert.Equal(HotkeyWarningKind.Standalone, vm.WarningKind);
    }
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void SavedStandaloneBinding_ClearsHighlightButRetainsTooltip(bool autoSave)
    {
        var vm = new HotkeyViewModel();
        vm.LoadFromString("Mouse4");
        vm.SetEditing(true);
        vm.AcknowledgeSavedBinding("MouseX1", deferWhileEditing: autoSave);
        Assert.Equal(autoSave, vm.HasWarning);
        vm.SetEditing(false);
        Assert.False(vm.HasWarning);
        Assert.Contains("Back/Forward", vm.HoverText);
        vm.SetEditing(true);
        vm.AcknowledgeSavedBinding("MouseX1", deferWhileEditing: true);
        Assert.False(vm.HasWarning);
        vm.LoadFromString("Mouse5");
        Assert.Equal(HotkeyWarningKind.Standalone, vm.WarningKind);
    }

    [Fact]
    public void OlderAutoSaveCompletion_DoesNotAcknowledgeNewerBindingOrHideConflicts()
    {
        var vm = new HotkeyViewModel();
        vm.SetEditing(true);
        vm.LoadFromString("Pause");
        vm.AcknowledgeSavedBinding("Pause", deferWhileEditing: true);
        vm.LoadFromString("Scroll");
        vm.AcknowledgeSavedBinding("Pause", deferWhileEditing: true);
        vm.SetEditing(false);
        Assert.Equal(HotkeyWarningKind.Standalone, vm.WarningKind);
        vm.SetDuplicateWarning(true);
        vm.AcknowledgeSavedBinding("Scroll");
        Assert.Equal(HotkeyWarningKind.Duplicate, vm.WarningKind);
        vm.SetDuplicateWarning(false);
        vm.SetRegistrationWarning(HotkeyWarningKind.ExternalConflict, "Unavailable");
        vm.AcknowledgeSavedBinding("Scroll");
        Assert.Equal(HotkeyWarningKind.ExternalConflict, vm.WarningKind);
    }

    [Fact]
    public void StandaloneWarning_YieldsToConflictsAndUpdatesWhenModifiersChange()
    {
        var vm = new HotkeyViewModel();
        Assert.True(vm.LoadFromString("Scroll"));
        var notifications = new List<string?>();
        vm.PropertyChanged += (_, e) => notifications.Add(e.PropertyName);
        vm.SetDuplicateWarning(true);
        Assert.Equal(HotkeyWarningKind.Duplicate, vm.WarningKind);
        vm.SetDuplicateWarning(false);
        vm.SetRegistrationWarning(HotkeyWarningKind.ExternalConflict, "Already registered");
        Assert.Equal("Already registered", vm.HoverText);
        vm.ClearRegistrationWarning();
        Assert.Equal(HotkeyWarningKind.Standalone, vm.WarningKind);
        vm.AddModifier(Key.LeftCtrl);
        Assert.False(vm.HasWarning);
        vm.RemoveModifier(Key.LeftCtrl);
        Assert.Equal(HotkeyWarningKind.Standalone, vm.WarningKind);
        Assert.Contains(nameof(vm.WarningKind), notifications);
        Assert.Contains(nameof(vm.HasWarning), notifications);
    }
    ...
    public void LoadFromString_ParsesAliases_AndRoundTripsToHotkeyString()
    {
        ...
        Assert.Equal(HotkeyDisplayFormatter.FormatSpaced("Ctrl+Alt+."), vm.DisplayText);
    }
    ...
    public void LoadFromString_ParsesMouseButtonHotkey()
    {
        ...
        Assert.Equal("Ctrl + Mouse 4", vm.DisplayText);
    }
    ...
    public void LoadFromString_ParsesWheelHotkey()
    {
        ...
        Assert.Equal("Alt + Wheel Down", vm.DisplayText);
    }

    [Theory]
    [InlineData("Ctrl+WheelLeft", "Ctrl + Wheel Left")]
    [InlineData("Ctrl+WheelRight", "Ctrl + Wheel Right")]
    public void LoadFromString_ParsesHorizontalWheelHotkeys(string serialized, string expectedDisplay)
    {
        var vm = new HotkeyViewModel();

        Assert.True(vm.LoadFromString(serialized));
        Assert.Equal(serialized, vm.ToHotkeyString());
        Assert.Equal(expectedDisplay, vm.DisplayText);
    }
    ...
    [InlineData("WheelUp")]
    public void LoadFromString_UnsupportedStandaloneInput_PreservesAttemptForCorrection(string hotkey)
    {
        var vm = new HotkeyViewModel();
        ...
        Assert.Equal(hotkey, vm.ToHotkeyString());
        Assert.Equal(HotkeyWarningKind.ModifierRequired, vm.WarningKind);
        Assert.Contains("Hold Ctrl, Alt, Shift, or Win", vm.HoverText);
        vm.AcknowledgeSavedBinding(hotkey);
        Assert.True(vm.HasWarning);
        Assert.True(vm.LoadFromString("Ctrl+" + hotkey));
        Assert.False(vm.HasWarning);
    }
    ...
    [Fact]
    public void RefreshDisplayText_RaisesDisplayTextChangedWithoutChangingSerialization()
    {
        var vm = new HotkeyViewModel();
        Assert.True(vm.LoadFromString("Ctrl+OemPlus"));
        var changedProperties = new List<string?>();
        vm.PropertyChanged += (_, e) => changedProperties.Add(e.PropertyName);

        vm.RefreshDisplayText();

        Assert.Contains(nameof(HotkeyViewModel.DisplayText), changedProperties);
        Assert.Equal("Ctrl++", vm.ToHotkeyString());
    }
