...
public sealed class RoutineEditorViewModelTests
{
    ...
    public void ScheduleEditor_AutomaticModeUsesLiveWindowsLocalePattern()
    {
        ...
        bool expected = RoutineEditorViewModel.Uses24HourClock(
    ...
    [InlineData("h \\H mm", false)]
    public void Uses24HourClock_IgnoresEscapedAndLiteralHourMarkers(string pattern, bool expected)
    {
        Assert.Equal(expected, RoutineEditorViewModel.Uses24HourClock(pattern));
    }
    ...
    [InlineData("h \\HH mm", false)]
    public void UsesPaddedHour_RespectsHourTokenWidthAndLiterals(string pattern, bool expected)
    {
        Assert.Equal(expected, RoutineEditorViewModel.UsesPaddedHour(pattern));
    }
