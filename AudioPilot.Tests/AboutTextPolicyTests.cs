...
public sealed class AboutTextPolicyTests
{
    ...
    public void AboutText_ListsEveryCurrentGlobalHotkeyActionAndDefaultBinding()
    {
        ...
        [
            ...
            $"- Show audio status: {FormatDefaultBinding(defaults.Hotkeys.App.ShowAudioStatus)}",
            ...
            $"- Raise or lower foreground app volume: Unassigned (step defaults to {defaults.Hotkeys.Volume.ForegroundVolumeStepPercent} percentage points)",
            "- Mute or unmute foreground app: Unassigned",
            "- Push-to-talk: Off and unassigned (mute between presses when enabled)",
            "- Hold-to-mute: Unassigned (restore the previous microphone mute state on release)",
            "- Run a routine: Optional individual hotkey, or a shared hotkey for a named cycle group",
        ];
