...
public sealed partial class LocalHeadlessCommandRunnerTests
{
    ...
    [Fact]
    public void ResolveGuiExecutablePath_TargetsAudioPilotBesideTheCliHost()
    {
        string guiExecutablePath = LocalHeadlessCommandRunner.ResolveGuiExecutablePath();

        Assert.Equal("AudioPilot.exe", Path.GetFileName(guiExecutablePath));
        Assert.Equal(
            Path.TrimEndingDirectorySeparator(Path.GetFullPath(AppContext.BaseDirectory)),
            Path.TrimEndingDirectorySeparator(Path.GetFullPath(Path.GetDirectoryName(guiExecutablePath)!)));
    }
    ...
    [Fact]
    public async Task RoutineQueries_UseDetachedSettingsWithoutCreatingAudioServices()
    {
        using var workspace = new TestSettingsWorkspace(nameof(LocalHeadlessCommandRunnerTests));
        ...
        settingsService.SaveSettings(new Settings
        {
            ...
            {
                Items = [new AudioRoutine { Id = "routine-1", Name = "Desk", TriggerKind = RoutineTriggerKind.Scheduled, ScheduleDays = [DayOfWeek.Monday], MasterVolumePercent = 30 }]
            }
        });
        var tracker = new RuntimeServiceCreationTracker(workspace);
        using var runner = tracker.CreateRunner();
        AudioRoutine snapshot = Assert.Single(runner.GetRoutinesSnapshot());
        snapshot.Name = "Changed snapshot";
        snapshot.ScheduleDays.Clear();

        CliExecutionResult result = await runner.ExecuteAsync(new CliCommand
        {
            Action = CliAction.RoutineGet,
            Key = "routine-1",
            JsonOutput = true,
        });
        ...
        Assert.Equal("Desk", data["name"]!.GetValue<string>());
        Assert.Equal("Monday", Assert.Single(data["triggers"]![0]!["days"]!.AsArray())!.GetValue<string>());
        CliExecutionResult next = await runner.ExecuteAsync(new CliCommand { Action = CliAction.RoutineNext, Key = "routine-1", JsonOutput = true });
        Assert.Equal(0, next.ExitCode);
        Assert.Equal("routine-next-occurrence", JsonNode.Parse(next.Output!)!["data"]!["diagCode"]!.GetValue<string>());
        Assert.Equal(1, tracker.SettingsCreated);
        Assert.Equal(0, tracker.StartupCreated);
        Assert.Equal(0, tracker.AudioCreated);
        Assert.Equal(0, tracker.BluetoothCreated);
    ...
    [Fact]
    public void GetMuteStatus_CreatesOnlyAudioService()
    {
        using var workspace = new TestSettingsWorkspace(nameof(LocalHeadlessCommandRunnerTests));
        var tracker = new RuntimeServiceCreationTracker(workspace);
        using var runner = tracker.CreateRunner();

        _ = runner.GetMuteStatus("sound", jsonOutput: false);
    ...
    public async Task ExecuteAsync_StartupOpen_ReturnsExitCodeThree()
    {
        ...
        Assert.Equal("[diag-code:startup-open-failed] Failed to open startup settings.", result.Output);
    }
    ...
    public async Task ExecuteAsync_StartupOpenJson_ReturnsJsonErrorEnvelope()
    {
        ...
        Assert.Equal("startup-open-failed", parsed["data"]?["error"]?["code"]?.GetValue<string>());
