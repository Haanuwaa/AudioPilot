...
{
    private static readonly JsonSerializerOptions StrictSettingsJsonOptions = new(SettingsJson.Options)
    {
        UnmappedMemberHandling = JsonUnmappedMemberHandling.Disallow,
    };

    public static TheoryData<string> GeneratedHelpTopics { get; } = [.. ExtractSupportedHelpTopics()];

    [Fact]
    public void SettingsExample_UsesEveryCurrentPersistedKey()
    {
        string markdown = File.ReadAllText(Path.Combine(ResolveRepoRoot(), "docs", "CLI.md"));
        string section = ExtractMarkdownSection(markdown, "### Settings File Structure");
        int start = section.IndexOf("```json", StringComparison.Ordinal) + "```json".Length;
        int end = section.IndexOf("```", start, StringComparison.Ordinal);
        JsonObject example = JsonNode.Parse(section[start..end])!.AsObject();
        JsonObject defaults = JsonSerializer.SerializeToNode(new Settings())!.AsObject();
        static IEnumerable<string> PropertyPaths(JsonObject value, string prefix = "")
        {
            foreach ((string name, JsonNode? node) in value.OrderBy(static property => property.Key, StringComparer.Ordinal))
            {
                string path = prefix + name;
                yield return path;
                if (node is JsonObject child)
                {
                    foreach (string nested in PropertyPaths(child, path + "."))
                    {
                        yield return nested;
                    }
                }
        ...
        Assert.Equal(PropertyPaths(defaults), PropertyPaths(example));
        Assert.NotNull(JsonSerializer.Deserialize<Settings>(example.ToString(), StrictSettingsJsonOptions));
    }
    ...
    public void CommandsSection_HelpLine_MatchesGeneratedHelpTopicListExactly()
    {
        ...
        string expectedHelpLine = ExtractUsageLines(CliCommand.UsageText)
            .Single(static line => line.StartsWith("AudioPilot.Cli.exe help [", StringComparison.Ordinal));
    ...
    public void CommandsSection_CoversGeneratedTopicHelpUsage(string topic)
    {
        ...
        IReadOnlyList<string> documentedCommands = ExtractCommandsSection(markdown);
        IReadOnlyList<string> expectedCommands = ExtractUsageLines(CliCommand.GetHelpText(topic));
