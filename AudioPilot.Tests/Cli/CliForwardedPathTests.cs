...
public sealed class CliForwardedPathTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void InvalidPath_ReturnsValidationErrorWithoutThrowing(bool allowAnyPath)
    {
        Assert.False(CliPathPolicy.TryResolveConfigPath("bad\0path.json", string.Empty, allowAnyPath,
            out string resolved, out string? error));
        Assert.Empty(resolved);
        Assert.Equal("Path is not valid.", error);
    }
