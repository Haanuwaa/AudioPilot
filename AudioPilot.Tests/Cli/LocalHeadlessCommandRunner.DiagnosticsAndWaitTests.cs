...
public sealed partial class LocalHeadlessCommandRunnerTests
{
    ...
    [Fact]
    public async Task ReconnectReadiness_WaitsPastFailureBudget_WhenRequestWasAcceptedAndNotificationIsMissing()
    {
        int originalSuccess = RuntimeTuningConfig.BluetoothReconnectSuccessStabilizeWindowMs;
        int originalFailure = RuntimeTuningConfig.BluetoothReconnectPostAttemptRecheckDelayMs;
        int originalInterval = RuntimeTuningConfig.BluetoothReconnectSuccessObservedRecheckIntervalMs;
        try
        {
            RuntimeTuningConfig.BluetoothReconnectSuccessStabilizeWindowMs = 2000;
            RuntimeTuningConfig.BluetoothReconnectPostAttemptRecheckDelayMs = 0;
            RuntimeTuningConfig.BluetoothReconnectSuccessObservedRecheckIntervalMs = 10;
            int observations = 0;
            bool unsubscribed = false;
            using var scope = new HeadlessRunnerScope(new Settings(), audioOverrides: new LocalHeadlessCommandRunner.AudioOverrides(
                SubscribeDeviceStateChanged: _ => new CallbackDisposable(() => unsubscribed = true)));

            bool ready = await scope.Runner.WaitForReconnectReadinessAsync(
                () => ++observations >= 6, new BluetoothReconnectAttemptResult(true, true, 1, 0), "fixture");
        ...
        {
            RuntimeTuningConfig.BluetoothReconnectSuccessStabilizeWindowMs = originalSuccess;
            RuntimeTuningConfig.BluetoothReconnectPostAttemptRecheckDelayMs = originalFailure;
            RuntimeTuningConfig.BluetoothReconnectSuccessObservedRecheckIntervalMs = originalInterval;
        }
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task ReconnectReadiness_SkipsUnattemptedRequestsAndBoundsFailedRequests(bool attempted)
    {
        ...
        try
        {
            ...
            using var scope = new HeadlessRunnerScope(new Settings(), audioOverrides: new LocalHeadlessCommandRunner.AudioOverrides(
                SubscribeDeviceStateChanged: _ => { subscribed = true; return new CallbackDisposable(() => { }); }));

            Assert.False(await scope.Runner.WaitForReconnectReadinessAsync(
                () => false, new BluetoothReconnectAttemptResult(attempted, false, attempted ? 1 : 0, 0), "fixture"));
            Assert.Equal(attempted, subscribed);
        }
        finally
        {
    ...
    [Fact]
    public async Task ReconnectReadiness_CoalescesNotificationBurstsBeforeRecheckingDevices()
    {
        int originalInterval = RuntimeTuningConfig.BluetoothReconnectSuccessObservedRecheckIntervalMs;
        try
        {
            RuntimeTuningConfig.BluetoothReconnectSuccessObservedRecheckIntervalMs = 10000;
            int observations = 0;
            bool ready = false;
            Action? notification = null;
            using var scope = new HeadlessRunnerScope(new Settings(), audioOverrides: new LocalHeadlessCommandRunner.AudioOverrides(
                SubscribeDeviceStateChanged: callback =>
                {
                    notification = callback;
                    for (int index = 0; index < 500; index++) callback();
                    return new CallbackDisposable(() => { });
                }));

            Task<bool> wait = scope.Runner.WaitForReconnectReadinessAsync(
                () => { observations++; return ready; }, new BluetoothReconnectAttemptResult(true, true, 1, 0), "fixture");
            Assert.InRange(observations, 1, 4);
            ready = true;
            Assert.NotNull(notification);
            notification();
            Assert.True(await wait.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
        }
        finally
        {
            RuntimeTuningConfig.BluetoothReconnectSuccessObservedRecheckIntervalMs = originalInterval;
    ...
    [Fact]
    public async Task ReconnectReadiness_DisposalCancelsWaitAndToleratesLateNotification()
    {
        Action? notification = null;
        using var scope = new HeadlessRunnerScope(new Settings(), audioOverrides: new LocalHeadlessCommandRunner.AudioOverrides(
            SubscribeDeviceStateChanged: callback => { notification = callback; return new CallbackDisposable(() => { }); }));
        Task<bool> wait = scope.Runner.WaitForReconnectReadinessAsync(
            () => false, new BluetoothReconnectAttemptResult(true, true, 1, 0), "fixture");
        Assert.NotNull(notification);
        scope.Runner.Dispose();

        Assert.False(await wait.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken));
        notification();
    }
}
