...
public sealed class CliRoutineQueryTests
{
    ...
    [Fact]
    public async Task RoutineNext_ChoosesTheEarliestAdditionalSchedule()
    {
        ...
        var routine = new AudioRoutine
        {
            ...
            Triggers = [new RoutineTrigger { Id = "first", Kind = RoutineTriggerKind.Scheduled, TimeZoneId = "UTC", Time = TimeOnly.FromDateTime(now.AddHours(3)) }, new() { Id = "earlier", Kind = RoutineTriggerKind.Scheduled, TimeZoneId = "UTC", Time = TimeOnly.FromDateTime(now.AddHours(1)) }]
        };
        var runtime = new FakeRuntime { Routines = [routine] };
        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(new CliCommand { Action = CliAction.RoutineNext, Key = "multi", JsonOutput = true }, runtime);
        Assert.Equal(0, result.ExitCode);
        Assert.Equal("earlier", JsonNode.Parse(result.Output!)!["data"]!["triggerId"]!.GetValue<string>());
        Assert.Null(runtime.LastRoutineSelector);
    }
    ...
    [InlineData(false, RoutineTriggerKind.DeviceAvailability)]
    [InlineData(true, RoutineTriggerKind.DeviceAvailability)]
    public void RoutineDetails_RedactsSensitiveFieldsWithoutChangingConfiguration(bool jsonOutput, RoutineTriggerKind triggerKind)
    {
        var routine = new AudioRoutine
        {
            ...
            Conditions = new() { Device = new() { Id = "Private required", Name = "Private requirement" }, RunningAppPath = @"C:\Private\required.exe", ConnectedNetwork = "Private required network" },
            Triggers = [new RoutineTrigger { Id = "first", Kind = triggerKind, ApplicationMode = ApplicationTriggerMode.ProcessFocus, Device = new() { Id = "Private trigger", Name = "Private device", StableId = "Private stable" }, AppPath = @"C:\Private\app.exe", TitlePattern = "Private title", NetworkName = "Private network", Days = [DayOfWeek.Friday, DayOfWeek.Monday], Time = new TimeOnly(7, 5), NotifyBeforeRun = triggerKind == RoutineTriggerKind.Scheduled }, new() { Id = "extra", Kind = RoutineTriggerKind.Application, AppPath = @"C:\Private\Extra.exe", ApplicationMode = ApplicationTriggerMode.ProcessFocus, TitlePattern = "Private extra title" },
                new() { Id = "network", Kind = RoutineTriggerKind.Network, NetworkName = "Private other network" }],
        ...
        if (jsonOutput)
        {
            ...
            Assert.Equal(triggerKind == RoutineTriggerKind.Scheduled, data["triggers"]![0]!["notifyBeforeRun"]!.GetValue<bool>());
            Assert.Equal(triggerKind.ToString(), data["triggers"]![0]!["kind"]!.GetValue<string>());
            Assert.Equal("07:05:00", data["triggers"]![0]!["time"]!.GetValue<string>());
            Assert.Equal(["Monday", "Friday"], data["triggers"]![0]!["days"]!.AsArray().Select(static value => value!.GetValue<string>()));
        }
    }
}
