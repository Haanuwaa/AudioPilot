...
public sealed partial class CliOutputFormatterTests
{
    [Fact]
    public void FormatStartupStatus_Text_ReturnsEnabledOrDisabled()
    {
        Assert.Equal("enabled", CliOutputFormatter.FormatStartupStatus(startupEnabled: true, jsonOutput: false));
        Assert.Equal("disabled", CliOutputFormatter.FormatStartupStatus(startupEnabled: false, jsonOutput: false));
    }

    [Fact]
    public void FormatStartupStatus_Json_ReturnsBooleanProperty()
    {
        string json = CliOutputFormatter.FormatStartupStatus(startupEnabled: true, jsonOutput: true);
    ...
    {
        string text = CliOutputFormatter.FormatStatusSnapshot(
            startupEnabled: true,
    ...
    {
        string json = CliOutputFormatter.FormatStatusSnapshot(
    ...
    public void FormatStatusSnapshot_WithBluetoothReconnectSettings_IncludesReconnectFields()
    {
        string text = CliOutputFormatter.FormatStatusSnapshot(
            ...
            bluetoothReconnectMaxAttempts: AppConstants.Bluetooth.ReconnectMaxAttemptsDefault,
        ...
        Assert.Contains($"bluetooth reconnect max attempts: {AppConstants.Bluetooth.ReconnectMaxAttemptsDefault}", text);
    ...
    {
        string text = CliOutputFormatter.FormatDiagnosticsStatus(
    ...
    {
        string json = CliOutputFormatter.FormatDiagnosticsStatus(
            logFilePath: "C:/logs/AudioPilot.log",
            logFileExists: false,
            logFileBytes: 0,
            logBackupDirectory: "C:/logs/backups",
    ...
    {
        string text = CliOutputFormatter.FormatDeviceList("output", [], jsonOutput: false);
    ...
    public void FormatDeviceList_Text_WithDevices_ReturnsNumberedList()
    {
        ...
        string text = CliOutputFormatter.FormatDeviceList("output", devices, jsonOutput: false);
    ...
    public void FormatDeviceList_Json_ReturnsKindAndDevices()
    {
        ...
        string json = CliOutputFormatter.FormatDeviceList("input", devices, jsonOutput: true);
    ...
    [Fact]
    public void FormatDeviceList_Json_Redact_RedactsIdsAndNames()
    {
        var devices = new List<CycleDevice>
        {
            new() { Id = "in-2", Name = "Mic" },
        ...
        string json = CliOutputFormatter.FormatDeviceList("input", devices, jsonOutput: true, redactOutput: true);

        using var doc = JsonDocument.Parse(json);
        JsonElement device = doc.RootElement.GetProperty("data").GetProperty("devices")[0];

        Assert.StartsWith("device-id[", device.GetProperty("id").GetString(), StringComparison.Ordinal);
    ...
    {
        string text = CliOutputFormatter.FormatCycleList("input", [], jsonOutput: false);
    ...
    public void FormatCycleList_Text_WithDevices_ReturnsOrderedList()
    {
        ...
        string text = CliOutputFormatter.FormatCycleList("output", cycle, jsonOutput: false);
    ...
    public void FormatCycleList_Json_ReturnsOrderedDevicePayload()
    {
        ...
        string json = CliOutputFormatter.FormatCycleList("output", cycle, jsonOutput: true);
    ...
    {
        string text = CliOutputFormatter.FormatCycleValidation(
            "output",
            [],
            [],
            jsonOutput: false);
    ...
    {
        string json = CliOutputFormatter.FormatCycleValidation(
            "output",
            ["Speakers"],
            ["Monitor"],
            jsonOutput: true);
    ...
    {
        string text = CliOutputFormatter.FormatCycleTest(
            kind: "output",
            configuredCount: 3,
    ...
    {
        string json = CliOutputFormatter.FormatCycleTest(
            kind: "input",
            configuredCount: 2,
            connectedConfiguredCount: 1,
            hasDefaultInputDevice: true,
    ...
    {
        string json = CliOutputFormatter.SerializeCliJson(new { Value = 7 });
    ...
    {
        string text = CliOutputFormatter.FormatConfigValidation([], jsonOutput: false);
    ...
    {
        string text = CliOutputFormatter.FormatConfigValidation(
    ...
    {
        string json = CliOutputFormatter.FormatConfigValidation(["warning-a"], jsonOutput: true);
