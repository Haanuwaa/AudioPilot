...
public sealed class CliHostProcessTests
{
    ...
    public async Task CliHost_ForwardsRoutineRunJsonPayload_ToRunningInstance()
    {
        ...
        Assert.Equal("Steam Big Picture | Restore on exit", data["triggerSummary"]?.GetValue<string>());
    ...
    public async Task CliHost_ForwardedHandlerFailure_PreservesErrorOutputContract(bool jsonOutput)
    {
        ...
        owner.CommandRequested += static _ => throw new InvalidOperationException("handler failed");
