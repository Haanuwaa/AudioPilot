...
{
    [Fact]
    public void ProcessBoundary_UnexpectedFailure_ReturnsStablePrivacySafeTextError()
    {
        const string sensitiveDetails = @"C:\Users\ExampleUser\secret.json";
        var error = new StringWriter();
        var metadata = new List<string>();

        int exitCode = CliHostProcessBoundary.Execute(
            ["status"],
            error,
            () => throw new InvalidOperationException(sensitiveDetails),
            metadata.Add);

        Assert.Equal(CliHostProcessBoundary.UnexpectedFailureExitCode, exitCode);
        Assert.Contains("[diag-code:internal-error]", error.ToString(), StringComparison.Ordinal);
        Assert.DoesNotContain(sensitiveDetails, error.ToString(), StringComparison.Ordinal);
        string loggedMetadata = Assert.Single(metadata);
        Assert.Contains("exceptionType=InvalidOperationException", loggedMetadata, StringComparison.Ordinal);
        Assert.DoesNotContain(sensitiveDetails, loggedMetadata, StringComparison.Ordinal);
    }

    [Fact]
    public void ProcessBoundary_UnexpectedFailure_PreservesJsonErrorContract()
    {
        var error = new StringWriter();
        ...
        int exitCode = CliHostProcessBoundary.Execute(
            ["status", "--json"],
            error,
            static () => throw new FormatException("sensitive parser input"));

        Assert.Equal(CliHostProcessBoundary.UnexpectedFailureExitCode, exitCode);
        JsonObject parsed = JsonNode.Parse(error.ToString())!.AsObject();
        Assert.Equal("internal-error", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal(CliHostProcessBoundary.UnexpectedFailureExitCode, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
        Assert.DoesNotContain("sensitive parser input", error.ToString(), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task CliHost_HeadlessFailure_FlushesLogBeforeExitWithoutPollutingJsonStreams()
    {
        string logPath = Path.Combine(Path.GetDirectoryName(ResolveCliPath())!, "AudioPilot.log");
        DateTime startedUtc = DateTime.UtcNow;
        ProcessResult result = await RunCliAsync(["status", "--json"], new Dictionary<string, string>
        {
            ["AUDIOPILOT_TEST_FORCE_HEADLESS_FAILURE"] = "1",
            ["AUDIOPILOT_DISABLE_CONSOLE_LOGGING"] = "0",
        });

        Assert.Equal(7, result.ExitCode);
        Assert.True(string.IsNullOrWhiteSpace(result.StdOut));
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal("headless-runtime-failed", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.True(File.GetLastWriteTimeUtc(logPath) >= startedUtc);
        Assert.Contains("headless-command-failed | action=Status", File.ReadAllText(logPath), StringComparison.Ordinal);
    }
    ...
    public async Task CliHost_InvalidUsageJson_WritesJsonErrorEnvelope()
    {
        ...
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, parsed["schemaVersion"]?.GetValue<string>());
        Assert.Equal("invalid-usage", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Contains("Missing devices arguments", parsed["data"]?["error"]?["message"]?.GetValue<string>(), StringComparison.OrdinalIgnoreCase);
        Assert.Equal(2, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    public async Task CliHost_Help_WritesUsageTextToStdOut()
    {
        ...
        Assert.Contains("AudioPilot.Cli.exe status [--json] [--redact]", result.StdOut, StringComparison.Ordinal);
    ...
    public void CliHost_HelpTopic_WritesTopicHelpToStdOut()
    {
        ...
        Assert.Contains("AudioPilot.Cli.exe diagnostics export-logs", result.StdOut, StringComparison.Ordinal);
        Assert.Contains("AudioPilot.Cli.exe diagnostics reset-per-app-audio", result.StdOut, StringComparison.Ordinal);
        Assert.DoesNotContain("AudioPilot.Cli.exe routine list", result.StdOut, StringComparison.Ordinal);
        Assert.True(string.IsNullOrWhiteSpace(result.StdErr));
    }
    ...
    public void CliHost_NewHelpTopicAlias_WritesTopicHelpToStdOut()
    {
        ...
        Assert.Contains("AudioPilot.Cli.exe volume get master|mic", result.StdOut, StringComparison.Ordinal);
        Assert.DoesNotContain("AudioPilot.Cli.exe routine list", result.StdOut, StringComparison.Ordinal);
        Assert.True(string.IsNullOrWhiteSpace(result.StdErr));
    }
    ...
    public async Task CliHost_ForwardsRoutineRunJsonPayload_ToRunningInstance()
    {
        ...
        JsonObject parsed = JsonNode.Parse(result.StdOut)!.AsObject();
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, parsed["schemaVersion"]?.GetValue<string>());
        JsonNode data = Assert.IsType<JsonObject>(parsed["data"]);
        Assert.Equal("routine-discord", data["id"]?.GetValue<string>());
        Assert.Equal("Steam Big Picture", data["triggerMode"]?.GetValue<string>());
        Assert.False(data["usesApplicationTrigger"]?.GetValue<bool>());
        Assert.Equal(string.Empty, data["triggerAppPath"]?.GetValue<string>());
        Assert.True(data["restorePreviousAudioOnDeactivate"]?.GetValue<bool>());
        Assert.False(data["switchOutputPerApp"]?.GetValue<bool>());
        Assert.True(data["showInTrayMenu"]?.GetValue<bool>());
        Assert.Equal("Steam Big Picture | Restore on exit | Tray menu", data["triggerSummary"]?.GetValue<string>());
        AssertRoutineTimingFieldsOmitted((JsonObject)data);
        Assert.Equal("Headset", data["appliedOutputDeviceName"]?.GetValue<string>());
    ...
    public void CliHost_ForwardsDiagnosticsHistoryDetail_ToRunningInstance()
    {
        ...
        JsonObject parsed = JsonNode.Parse(result.StdOut)!.AsObject();
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, parsed["schemaVersion"]?.GetValue<string>());
        Assert.Equal("op-forwarded-1", parsed["data"]?["opId"]?.GetValue<string>());
        Assert.Equal("mute-mic-on", parsed["data"]?["action"]?.GetValue<string>());
    ...
    [Fact]
    public void CliHost_RuntimeSet_RequiresLiveHostAndPreservesForwardedFlags()
    {
        string[] args = ["runtime", "set", "hotplug-refresh-debounce-ms", "400", "--json", "--redact"];
        var runner = new FakeCliHeadlessRunner();
        RuntimeResult unavailable = RunRuntime(args, headlessRunner: runner);
        Assert.Equal(3, unavailable.ExitCode);
        Assert.Null(runner.ReceivedCommand);
        JsonObject error = JsonNode.Parse(unavailable.StdErr)!.AsObject();
        Assert.Equal("ui-host-unavailable", error["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Contains("config set", unavailable.StdErr, StringComparison.Ordinal);

        var host = new FakeCliHostSingleInstance
        {
            TryAcquireResult = false,
            LastSignalExistingSucceededValue = true,
            LastSignalExitCodeValue = 0,
            LastSignalOutputValue = "forwarded-result",
        };
        RuntimeResult forwarded = RunRuntime(args, singleInstance: host, headlessRunner: runner);
        Assert.Equal(0, forwarded.ExitCode);
        Assert.Contains("forwarded-result", forwarded.StdOut, StringComparison.Ordinal);
        Assert.Null(runner.ReceivedCommand);
        Assert.True(CliCommand.TryFromPipePayload(host.ReceivedPayload!, out CliCommand? command));
    ...
    public async Task CliHost_ForwardingFailureJson_WritesJsonErrorEnvelope()
    {
        ...
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal("forwarding-failed", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal(4, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void CliHost_UncertainSubmission_ReportsUnknownOutcomeWithoutHeadlessRetry(bool json)
    {
        var singleInstance = new FakeCliHostSingleInstance
        {
            TryAcquireResult = false,
            LastSignalExistingSucceededValue = false,
            LastSignalFailureKindValue = SingleInstanceSignalFailureKind.OutcomeUnknown,
        };
        var headlessRunner = new FakeCliHeadlessRunner
        {
            ExceptionToThrow = new InvalidOperationException("An uncertain submission must not be retried locally.")
        };
        RuntimeResult result = RunRuntime(json ? ["status", "--json"] : ["status"], singleInstance: singleInstance, headlessRunner: headlessRunner);
        ...
        {
            JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
            Assert.Equal("ui-host-outcome-unknown", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        }
        else
        {
            Assert.Contains("[diag-code:ui-host-outcome-unknown]", result.StdErr, StringComparison.Ordinal);
            Assert.Contains("check current state before retrying", result.StdErr, StringComparison.OrdinalIgnoreCase);
        }
    }
    ...
    public void CliHost_InvalidUiHostForwardingResponse_ReturnsDistinctDiagnostics()
    {
        ...
        Assert.Equal(4, result.ExitCode);
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal("ui-host-invalid-response", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal(4, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    public async Task CliHost_HeadlessRunnerExceptionJson_WritesJsonErrorEnvelope()
    {
        ...
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal("headless-runtime-failed", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal(7, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    public void CliHost_HeadlessRunnerIOExceptionJson_UsesMappedDiagnostics()
    {
        ...
        Assert.Equal(7, result.ExitCode);
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal("headless-io-failed", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal(7, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task CliHost_ForwardedHandlerFailure_PreservesErrorOutputContract(bool jsonOutput)
    {
        ...
        owner.CommandRequested += static (_, _) => throw new InvalidOperationException("handler failed");
        Assert.True(owner.TryAcquire());
        await owner.ActivationListenerReadyForTests;
        using var client = new SingleInstanceHelperAdapter(new SingleInstanceHelper(mutexName, pipeName));

        RuntimeResult result = RunRuntime(jsonOutput ? ["status", "--json"] : ["status"], singleInstance: client);

        Assert.Equal(7, result.ExitCode);
        Assert.True(string.IsNullOrWhiteSpace(result.StdOut));
        if (jsonOutput)
        {
            JsonObject error = JsonNode.Parse(result.StdErr)!.AsObject();
            Assert.Equal("forwarded-runtime-failed", error["data"]?["error"]?["code"]?.GetValue<string>());
            Assert.Equal(7, error["data"]?["error"]?["exitCode"]?.GetValue<int>());
        }
        else
        {
            Assert.Contains("[diag-code:forwarded-runtime-failed]", result.StdErr, StringComparison.Ordinal);
        }
    }
    ...
    public void CliHost_ForwardedRefreshFailure_ReturnsRefreshContract()
    {
        ...
        JsonObject parsed = JsonNode.Parse(result.StdErr)!.AsObject();
        Assert.Equal("refresh-failed", parsed["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal("Refresh command failed.", parsed["data"]?["error"]?["message"]?.GetValue<string>());
        Assert.Equal(7, parsed["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    {
        JsonNode envelope = JsonSerializer.SerializeToNode(new
        {
        ...
        var headlessRunner = new FakeCliHeadlessRunner
        {
            Result = new CliExecutionResult(0, envelope.ToJsonString()),
        };
        ...
        JsonNode routine = Assert.IsType<JsonObject>(Assert.Single(routines));
        Assert.Equal("routine-discord", routine["id"]?.GetValue<string>());
        Assert.Equal("Steam Big Picture", routine["triggerMode"]?.GetValue<string>());
        Assert.Equal("Output: Headset", routine["targetSummary"]?.GetValue<string>());
    }
    ...
    private static RuntimeResult RunRuntime(
        string[] args,
        ICliHostSingleInstance? singleInstance = null,
    ...
    {
        string cliPath = Path.Combine(AppContext.BaseDirectory, "AudioPilot.Cli.exe");
