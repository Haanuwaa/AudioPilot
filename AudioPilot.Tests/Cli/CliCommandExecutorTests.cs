...
public sealed class CliCommandExecutorTests
{
    ...
    public async Task ExecuteAsync_SetJson_ReportsSuccessAndHonorsRedaction(string family, bool redact)
    {
        string[] args = redact
            ? [family, "set", "key", "Private Value", "--json", "--redact"]
            : [family, "set", "key", "Private Value", "--json"];
        Assert.True(CliCommand.TryParse(args, out CliCommand command, out string? error), error);
        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(command, new FakeRuntime());
        Assert.Equal(0, result.ExitCode);
        JsonNode data = JsonNode.Parse(result.Output!)!["data"]!;
        Assert.True(data["updated"]!.GetValue<bool>());
        Assert.Equal("key", data["key"]!.GetValue<string>());
        if (redact) Assert.DoesNotContain("Private Value", result.Output!, StringComparison.Ordinal);
        else Assert.Equal("Private Value", data["value"]!.GetValue<string>());
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task ExecuteAsync_ConfigGet_RedactsValueInTextAndJson(bool json)
    {
        var runtime = new FakeRuntime { ConfigGetResult = (true, "Private Speakers", null) };
        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.ConfigGet, Key = "listen-monitor-output-device-name", JsonOutput = json, RedactOutput = true }, runtime);
        Assert.Equal(0, result.ExitCode);
        Assert.DoesNotContain("Private Speakers", result.Output!, StringComparison.Ordinal);
    ...
    [Fact]
    public async Task ExecuteAsync_MediaStatus_AwaitsAsynchronousRuntimeCompletion()
    {
        var completion = new TaskCompletionSource<string>(TaskCreationOptions.RunContinuationsAsynchronously);
        var runtime = new FakeRuntime
        {
            MediaStatusAsyncOverride = () => completion.Task,
        };

        Task<CliExecutionResult> execution = CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.MediaStatus },
            runtime);

        Assert.False(execution.IsCompleted);
        completion.SetResult("asynchronous media status");

        CliExecutionResult result = await execution;
        Assert.Equal(0, result.ExitCode);
        Assert.Equal("asynchronous media status", result.Output);
    }

    [Fact]
    public async Task ExecuteAsync_NetworkList_AwaitsAsynchronousRuntimeCompletion()
    {
        var completion = new TaskCompletionSource<string>(TaskCreationOptions.RunContinuationsAsynchronously);
        var runtime = new FakeRuntime
        {
            NetworkListAsyncOverride = () => completion.Task,
        };

        Task<CliExecutionResult> execution = CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.NetworkList },
            runtime);

        Assert.False(execution.IsCompleted);
        completion.SetResult("asynchronous network list");

        CliExecutionResult result = await execution;
        Assert.Equal(0, result.ExitCode);
        Assert.Equal("asynchronous network list", result.Output);
    }

    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public async Task ExecuteAsync_NetworkListFailure_ReturnsNonzeroExitCodeAndPrivacySafeError(bool jsonOutput)
    {
        var completion = new TaskCompletionSource<string>(TaskCreationOptions.RunContinuationsAsynchronously);
        var runtime = new FakeRuntime { NetworkListAsyncOverride = () => completion.Task };
        Task<CliExecutionResult> execution = CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.NetworkList, JsonOutput = jsonOutput }, runtime);

        completion.SetException(new InvalidOperationException("Private network name"));
        CliExecutionResult result = await execution;

        Assert.Equal(7, result.ExitCode);
        Assert.DoesNotContain("Private network name", result.Output, StringComparison.Ordinal);
        if (jsonOutput)
        {
            JsonObject payload = JsonNode.Parse(result.Output!)!.AsObject();
            Assert.Equal("network-list-failed", payload["data"]?["error"]?["code"]?.GetValue<string>());
            Assert.Equal(7, payload["data"]?["error"]?["exitCode"]?.GetValue<int>());
        }
        else
        {
            Assert.Equal("[diag-code:network-list-failed] Failed to scan networks.", result.Output);
        }
    }
    ...
    public async Task ExecuteAsync_RefreshWithJson_ReturnsSuccessEnvelope()
    {
        ...
        Assert.Equal("refresh-success", root["data"]?["diagCode"]?.GetValue<string>());
    }
    ...
    [Fact]
    public async Task ExecuteAsync_DiagnosticsExportBundle_ForwardsPathAllowAnyPathDetailAndSensitivity()
    {
        var runtime = new FakeRuntime
        {
            BundleExportResult = (true, "bundle-exported")
        };

        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.DiagnosticsExportBundle, Key = "bundle.zip", AllowAnyPath = true, DiagnosticsExportDetailLevel = CliDiagnosticsExportDetailLevel.Manifest, IncludeSensitive = true, JsonOutput = true },
            runtime);

        Assert.Equal(0, result.ExitCode);
        Assert.Equal("bundle-exported", result.Output);
        Assert.Equal("bundle.zip", runtime.LastExportPath);
        Assert.True(runtime.LastExportAllowAnyPath);
        Assert.Equal(CliDiagnosticsExportDetailLevel.Manifest, runtime.LastExportDetailLevel);
        Assert.True(runtime.LastIncludeSensitive);
    }

    [Fact]
    public async Task ExecuteAsync_DiagnosticsExportBundle_Failure_ReturnsExecutionFailure()
    {
        var runtime = new FakeRuntime
        {
            BundleExportResult = (false, "bundle-export-failed")
        };

        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.DiagnosticsExportBundle, Key = "bundle.zip" },
            runtime);

        Assert.Equal(3, result.ExitCode);
        Assert.Equal("bundle-export-failed", result.Output);
    }

    [Fact]
    public async Task ExecuteAsync_DiagnosticsExportBundle_AwaitsDeferredRuntimeOperation()
    {
        var completion = new TaskCompletionSource<(bool Success, string Output)>(TaskCreationOptions.RunContinuationsAsynchronously);
        var runtime = new FakeRuntime
        {
            BundleExportAsyncOverride = () => completion.Task,
        };
        ...
        Assert.False(execution.IsCompleted);
        completion.TrySetResult((true, "deferred-bundle-exported"));

        CliExecutionResult result = await execution;
        Assert.Equal(0, result.ExitCode);
        Assert.Equal("deferred-bundle-exported", result.Output);
    }
    ...
    public async Task ExecuteAsync_RoutineList_FormatsSnapshot(bool jsonOutput, bool redactOutput)
    {
        var runtime = new FakeRuntime
        {
            Routines = [new AudioRoutine { Id = "routine-1", Name = "Private routine", OutputDeviceId = "private-output" }]
        };

        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.RoutineList, JsonOutput = jsonOutput, RedactOutput = redactOutput },
            runtime);

        Assert.Equal(0, result.ExitCode);
        Assert.Contains("routine-1", result.Output);
        Assert.Equal(!redactOutput, result.Output!.Contains("Private routine", StringComparison.Ordinal));
        Assert.Equal("Private routine", runtime.Routines[0].Name);
        if (jsonOutput)
        {
            Assert.Single(JsonNode.Parse(result.Output)!["data"]!["routines"]!.AsArray());
        }
    }
    ...
    public async Task ExecuteAsync_SwitchOutputFailure_ReturnsPreconditionExitCode()
    {
        ...
        Assert.Equal(5, result.ExitCode);
        Assert.Equal("[diag-code:output-switch-precondition] Output switch precondition failed.", result.Output);
    }
    ...
    public async Task ExecuteAsync_StartupEnableFailure_ReturnsExecutionFailureExitCode()
    {
        ...
        Assert.Equal(3, result.ExitCode);
        Assert.Equal("[diag-code:startup-enable-failed] Failed to enable startup.", result.Output);
    }
    ...
    public async Task ExecuteAsync_RuntimeSetFailure_ReturnsExecutionFailure()
    {
        ...
        Assert.Equal(5, result.ExitCode);
        Assert.Equal("[diag-code:runtime-set-failed] invalid range", result.Output);
    }
    ...
    public async Task ExecuteAsync_ConfigGetUnknownKey_ReturnsPreconditionFailure()
    {
        ...
        Assert.Equal(5, result.ExitCode);
        Assert.Equal("[diag-code:config-key-not-found] Unknown config key 'missing-key'.", result.Output);
    }
    ...
    public async Task ExecuteAsync_ConfigSetFailure_ReturnsPreconditionFailure()
    {
        ...
        Assert.Equal(5, result.ExitCode);
        Assert.Equal("[diag-code:config-set-failed] value must be one of: System, Light, Dark", result.Output);
    }
    ...
    public async Task ExecuteAsync_RuntimeGetUnknownKey_ReturnsPreconditionFailure()
    {
        ...
        Assert.Equal(5, result.ExitCode);
        Assert.Equal("[diag-code:runtime-key-not-found] Unknown runtime key 'missing-key'.", result.Output);
    }
    ...
    public async Task ExecuteAsync_VolumeSetMasterMissingPercent_ReturnsInvalidUsage()
    {
        ...
        Assert.Equal("[diag-code:missing-volume-percent] Missing volume percent.", result.Output);
    }
    ...
    public async Task ExecuteAsync_RoutineDisableMissingSelector_ReturnsInvalidUsage()
    {
        ...
        Assert.Equal(2, result.ExitCode);
        Assert.Equal("[diag-code:missing-routine-selector] Missing routine selector.", result.Output);
    }
    ...
    public void BuildJsonErrorPayload_UsesCliEnvelopeShape()
    {
        ...
        Assert.Equal("precondition-failed", root["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal("Precondition failed.", root["data"]?["error"]?["message"]?.GetValue<string>());
    ...
    public void BuildRuntimeUnavailableResult_JsonOutput_UsesCliEnvelopeShape()
    {
        ...
        var root = JsonNode.Parse(result.Output!)!;
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, root["schemaVersion"]?.GetValue<string>());
        Assert.Equal("app-not-ready", root["data"]?["error"]?["code"]?.GetValue<string>());
    }
    ...
    public async Task ExecuteAsync_MuteMicOnFailureWithJson_ReturnsJsonErrorEnvelope()
    {
        ...
        var root = JsonNode.Parse(result.Output!)!;
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, root["schemaVersion"]?.GetValue<string>());
        Assert.Equal("mute-mic-set-failed", root["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal("Failed to set microphone mute.", root["data"]?["error"]?["message"]?.GetValue<string>());
        Assert.Equal(3, root["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    public async Task ExecuteAsync_MuteMicOnWithJson_ReturnsMuteStatusEnvelope()
    {
        ...
        var root = JsonNode.Parse(result.Output!)!;
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, root["schemaVersion"]?.GetValue<string>());
        Assert.Equal("mic", root["data"]?["target"]?.GetValue<string>());
        Assert.True(root["data"]?["enabled"]?.GetValue<bool>());
        Assert.Equal("mute-mic-status", root["data"]?["diagCode"]?.GetValue<string>());
    }
    ...
    [InlineData("Infinity")]
    [InlineData("-Infinity")]
    public async Task ExecuteAsync_VolumeSetMasterInvalidPercentWithJson_ReturnsJsonErrorEnvelope(string percent)
    {
        var runtime = new FakeRuntime();

        CliExecutionResult result = await CliCommandExecutor.ExecuteAsync(
            new CliCommand { Action = CliAction.VolumeSetMaster, Value = percent, JsonOutput = true },
            runtime);
        ...
        var root = JsonNode.Parse(result.Output!)!;
        Assert.Equal(CliOutputFormatter.JsonSchemaVersion, root["schemaVersion"]?.GetValue<string>());
        Assert.Equal("invalid-volume-percent", root["data"]?["error"]?["code"]?.GetValue<string>());
        Assert.Equal("Invalid volume percent. Use a number between 0 and 100.", root["data"]?["error"]?["message"]?.GetValue<string>());
        Assert.Equal(2, root["data"]?["error"]?["exitCode"]?.GetValue<int>());
    }
    ...
    private sealed class FileBackedConfigRuntime(Settings settings) : ICliCommandRuntime
    {
        ...
        public Task<MediaSeekResult> MediaSeekAsync(bool backward, int? seconds) => Task.FromResult(new MediaSeekResult(true, "media-seek-accepted", "Seek accepted", 20));
        public Task<MediaPlaybackResult> MediaSetPlayingAsync(bool playing) => Task.FromResult(new MediaPlaybackResult(true, "media-playback-confirmed", "Playback confirmed.", playing ? "Playing" : "Paused", Confirmed: true));
        public Task<string> GetMediaStatusAsync(bool jsonOutput, bool redactOutput) => Task.FromResult("media-status");
        ...
        public (bool Success, string Output) GetVolume(bool playback, string? deviceId, bool jsonOutput, bool redactOutput = false) => (true, "volume");
        public (bool Success, string Output) SetVolume(bool playback, string? deviceId, float percent, bool jsonOutput, bool redactOutput = false, bool relative = false) => (true, "volume");
        public Task<CliExecutionResult> SwitchToDeviceAsync(CliCommand command) => Task.FromResult(new CliExecutionResult(0));
        public IReadOnlyList<AudioRoutine> GetRoutinesSnapshot() => [.. (Settings.Routines?.Items ?? []).Select(static routine => routine.Clone())];
        ...
        public (bool IsValid, string Output) GetConfigValidation(bool jsonOutput, bool redactOutput) => (true, "valid");
        ...
        public Task<(bool Success, string Output)> ExportDiagnosticBundleAsync(string path, bool allowAnyPath, CliDiagnosticsExportDetailLevel detailLevel, bool includeSensitive, bool jsonOutput) => Task.FromResult((true, "bundle-exported"));
        ...
        public Task<string> GetNetworkListAsync(bool jsonOutput, bool redactOutput) => Task.FromResult(jsonOutput ? "{\"Success\":true,\"Count\":0,\"Networks\":[]}" : "No networks detected.");
