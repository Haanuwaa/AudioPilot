...
{
    [Theory]
    [InlineData("foreground-volume-up-hotkey", "Ctrl+F8")]
    [InlineData("foreground-volume-down-hotkey", "Ctrl+F9")]
    [InlineData("foreground-mute-hotkey", "F10")]
    [InlineData("foreground-volume-step-percent", "7")]
    [InlineData("push-to-talk-hotkey", "Mouse4")]
    [InlineData("push-to-talk-enabled", "true")]
    [InlineData("hold-to-mute-hotkey", "Scroll")]
    public void ForegroundAndHoldSettings_RoundTripThroughCloneAndJson(string key, string value)
    {
        var settings = new Settings();
        Assert.True(CliConfigManager.TrySet(settings, key, value, out string? error), error);
        var copy = System.Text.Json.JsonSerializer.Deserialize<Settings>(System.Text.Json.JsonSerializer.Serialize(settings.Clone(), SettingsJson.Options), SettingsJson.Options)!;
        Assert.True(CliConfigManager.TryGet(copy, key, out string actual, out error), error);
        Assert.Equal(value, actual);
    }

    [Theory]
    [InlineData("push-to-talk-hotkey", "Ctrl+WheelUp")]
    [InlineData("hold-to-mute-hotkey", "Pause")]
    [InlineData("push-to-talk-hotkey", "PrintScreen")]
    [InlineData("foreground-volume-step-percent", "0")]
    [InlineData("foreground-volume-step-percent", "101")]
    public void ForegroundAndHoldSettings_RejectUnusableValues(string key, string value)
    {
        Assert.False(CliConfigManager.TrySet(new Settings(), key, value, out _));
    }
    ...
    [InlineData("MouseX1")]
    [InlineData("MouseX2")]
    public void StandaloneHotkey_RoundTripsWithoutAdditionalConfiguration(string key)
    {
        var settings = new Settings();
        Assert.True(CliConfigManager.TrySet(settings, "toggle-app-visibility-hotkey", key, out string? error), error);
        Assert.True(CliConfigManager.TryGet(settings, "toggle-app-visibility-hotkey", out string actual, out error), error);
        Assert.True(new AudioPilot.ViewModels.HotkeyViewModel().LoadFromString(actual));
        Assert.DoesNotContain(SettingsValidationService.EvaluateDiagnostics(settings, [], []).Warnings,
            warning => warning.Code == "invalid-hotkey-toggle-app-visibility-hotkey");
    }
    ...
    public void StandaloneHotkey_RejectsTypingNavigationAndMouseInputs(string key)
    {
        var settings = new Settings();
        Assert.False(CliConfigManager.TrySet(settings, "toggle-app-visibility-hotkey", key, out _));
        Assert.Equal("Ctrl+Alt+H", settings.Hotkeys.App.ToggleAppVisibility);
    }

    [Fact]
    public void ShowAudioStatusHotkey_RoundTripsThroughConfigCloneAndJson_AndCanBeCleared()
    {
        const string key = "show-audio-status-hotkey";
        var settings = new Settings();
        Assert.Empty(settings.Hotkeys.App.ShowAudioStatus);
        Assert.True(CliConfigManager.TrySet(settings, key, "Ctrl+Alt+A", out _));
        string json = System.Text.Json.JsonSerializer.Serialize(settings.Clone(), SettingsJson.Options);
        Settings imported = System.Text.Json.JsonSerializer.Deserialize<Settings>(json, SettingsJson.ImportOptions)!;
        Assert.True(CliConfigManager.TryGet(imported, key, out string binding, out _));
        Assert.Equal("Ctrl+Alt+A", binding);
        Assert.False(CliConfigManager.TrySet(imported, key, "NotAKey", out _));
        Assert.Equal("Ctrl+Alt+A", imported.Hotkeys.App.ShowAudioStatus);
        Assert.True(CliConfigManager.TrySet(imported, key, "", out _));
        Assert.Empty(imported.Hotkeys.App.ShowAudioStatus);
    }

    [Fact]
    public void CheckForUpdates_DefaultsOff_AndSurvivesCliJsonAndClone()
    {
        var settings = new Settings();
        Assert.True(CliConfigManager.TryGet(settings, "check-for-updates", out string initial, out _));
        Assert.Equal("false", initial);
        Assert.True(CliConfigManager.TrySet(settings, "check-for-updates", "true", out _));
        string json = System.Text.Json.JsonSerializer.Serialize(settings.Clone(), SettingsJson.Options);
        Settings imported = System.Text.Json.JsonSerializer.Deserialize<Settings>(json, SettingsJson.ImportOptions)!;
        Assert.True(imported.Miscellaneous.CheckForUpdates);
        Assert.False(CliConfigManager.TrySet(imported, "check-for-updates", "maybe", out _));
        Assert.True(imported.Miscellaneous.CheckForUpdates);
        Assert.True(CliConfigManager.TrySet(imported, "check-for-updates", "false", out _));
        Assert.False(imported.Miscellaneous.CheckForUpdates);
    }

    [Theory]
    [InlineData("preserve-audio-levels", "PreserveAudioLevels")]
    [InlineData("bluetooth-reconnect-enabled", "BluetoothReconnectEnabled")]
    public void DeviceSwitchingOptions_CliJsonAndClone_UseDeviceSwitchingSection(string key, string property)
    {
        var settings = new Settings();
        Assert.True(CliConfigManager.TryGet(settings, key, out string initial, out _));
        Assert.Equal("true", initial);
        Assert.True(CliConfigManager.TrySet(settings, key, "false", out _));

        string json = System.Text.Json.JsonSerializer.Serialize(settings.Clone(), SettingsJson.Options);
        using var document = System.Text.Json.JsonDocument.Parse(json);
        Assert.False(document.RootElement.GetProperty("DeviceSwitching").GetProperty(property).GetBoolean());
        Assert.False(document.RootElement.GetProperty("Miscellaneous").TryGetProperty(property, out _));

        Settings imported = System.Text.Json.JsonSerializer.Deserialize<Settings>(json, SettingsJson.ImportOptions)!;
        Assert.True(CliConfigManager.TryGet(imported, key, out string value, out _));
        Assert.Equal("false", value);
        Assert.False(CliConfigManager.TrySet(imported, key, "maybe", out _));
        Assert.True(CliConfigManager.TryGet(imported, key, out value, out _));
        Assert.Equal("false", value);
    }
    ...
    [Fact]
    public void TrySetThenGet_PlayAppSounds_RoundTrips()
    {
        var settings = new Settings();

        bool updated = CliConfigManager.TrySet(settings, "play-app-sounds", "false", out string? setError);
        bool found = CliConfigManager.TryGet(settings, "play-app-sounds", out string value, out string? getError);

        Assert.True(updated);
        Assert.Null(setError);
        Assert.True(found);
        Assert.Null(getError);
        Assert.False(settings.Miscellaneous.PlayAppSounds);
        Assert.Equal("false", value);
    }
    ...
    [Fact]
    public void TrySetThenGet_ScheduleTimeZone_RoundTrips()
    {
        var settings = new Settings();
        string expected = TimeZoneInfo.Local.Id;

        bool updated = CliConfigManager.TrySet(settings, "schedule-timezone", expected, out string? setError);
        bool found = CliConfigManager.TryGet(settings, "schedule-timezone", out string value, out string? getError);
    ...
    [Fact]
    public void TrySet_ScheduleTimeZone_InvalidTimeZone_ReturnsError()
    {
        var settings = new Settings();
        const string invalidTimeZone = "InvalidTimeZone";

        bool updated = CliConfigManager.TrySet(settings, "schedule-timezone", invalidTimeZone, out string? setError);

        Assert.False(updated);
        Assert.NotNull(setError);
        Assert.Contains("Invalid time zone", setError, StringComparison.Ordinal);
    }
    ...
    public void TrySet_Hotkey_ReservedShortcut_ReturnsError()
    {
        ...
        bool updated = CliConfigManager.TrySet(settings, "toggle-app-visibility-hotkey", "Win+S", out string? error);

        Assert.False(updated);
        Assert.Equal("Ctrl+Alt+H", settings.Hotkeys.App.ToggleAppVisibility);
        Assert.Equal("show/hide app hotkey uses reserved Windows shortcut 'Win+S'.", error);
    }
    ...
    public void TrySetThenGet_BluetoothReconnectEnabled_RoundTrips()
    {
        ...
        Assert.Null(getError);
        Assert.Equal("false", value);
        Assert.False(settings.DeviceSwitching.BluetoothReconnectEnabled);
    }

    [Fact]
    public void TrySetThenGet_SuppressDeviceStartupWarnings_RoundTrips()
    {
        var settings = new Settings();

        bool updated = CliConfigManager.TrySet(settings, "suppress-device-startup-warnings", "true", out string? setError);
        bool found = CliConfigManager.TryGet(settings, "suppress-device-startup-warnings", out string value, out string? getError);
        ...
        Assert.Null(getError);
        Assert.Equal("true", value);
        Assert.True(settings.Miscellaneous.SuppressDeviceStartupWarnings);
    }
