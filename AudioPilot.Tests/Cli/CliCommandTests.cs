...
public sealed class CliCommandTests
{
    public static TheoryData<string[], CliAction> RedactCapableCommands => new()
    {
        ...
        { ["routine", "get", "desk", "--redact"], CliAction.RoutineGet },
        { ["routine", "next", "morning", "--redact"], CliAction.RoutineNext },
        ...
        { ["network", "list", "--redact"], CliAction.NetworkList },
        ...
        { ["startup", "enable", "--json", "--redact"], CliAction.StartupEnable },
        { ["startup", "disable", "--json", "--redact"], CliAction.StartupDisable },
    };

    public static TheoryData<string> RedactCapableUsagePrefixes => new()
    {
        { "AudioPilot.Cli.exe media status" },
        { "AudioPilot.Cli.exe switch output" },
        { "AudioPilot.Cli.exe switch input" },
        { "AudioPilot.Cli.exe wait --wait-for-device" },
        { "AudioPilot.Cli.exe diagnostics status" },
        { "AudioPilot.Cli.exe diagnostics history" },
        { "AudioPilot.Cli.exe diagnostics history-detail" },
        { "AudioPilot.Cli.exe diagnostics export-logs" },
        { "AudioPilot.Cli.exe listen toggle|on|off" },
        { "AudioPilot.Cli.exe routine list" },
        { "AudioPilot.Cli.exe routine get" },
        { "AudioPilot.Cli.exe routine next" },
        { "AudioPilot.Cli.exe routine run|enable|disable|delete" },
        { "AudioPilot.Cli.exe routine create" },
        { "AudioPilot.Cli.exe routine update" },
        { "AudioPilot.Cli.exe routine import" },
        { "AudioPilot.Cli.exe routine export" },
        { "AudioPilot.Cli.exe status" },
        { "AudioPilot.Cli.exe config get" },
        { "AudioPilot.Cli.exe runtime get" },
        { "AudioPilot.Cli.exe network list" },
        { "AudioPilot.Cli.exe config export" },
        { "AudioPilot.Cli.exe config import" },
        { "AudioPilot.Cli.exe config validate" },
        { "AudioPilot.Cli.exe devices list output|input" },
        { "AudioPilot.Cli.exe devices get output|input" },
        { "AudioPilot.Cli.exe devices find output|input" },
        { "AudioPilot.Cli.exe cycle show|validate|test output|input" },
        { "AudioPilot.Cli.exe cycle add|remove output|input" },
        { "AudioPilot.Cli.exe cycle reorder output|input" },
        { "AudioPilot.Cli.exe startup enable|disable|status" },
    };
    ...
    [InlineData("Infinity")]
    [InlineData("-Infinity")]
    public void TryParse_VolumeSet_InvalidPercent_Fails(string percent)
    {
        bool ok = CliCommand.TryParse(["volume", "set", "master", percent], out _, out string? error);
    ...
    [Fact]
    public void TryParse_NetworkListJsonRedact_Parses()
    {
        bool ok = CliCommand.TryParse(["network", "list", "--json", "--redact"], out CliCommand command, out string? error);

        Assert.True(ok);
        Assert.Null(error);
        Assert.Equal(CliAction.NetworkList, command.Action);
        Assert.True(command.JsonOutput);
        Assert.True(command.RedactOutput);
    }

    [Fact]
    public void TryParse_NetworkListUnknownFlag_Fails()
    {
        bool ok = CliCommand.TryParse(["network", "list", "--banana"], out _, out string? error);
    ...
    [Theory]
    [InlineData("config", CliAction.ConfigSet)]
    [InlineData("runtime", CliAction.RuntimeSet)]
    public void TryParse_SetOutputFlags_PreservesValueAndPipePayload(string family, CliAction action)
    {
        Assert.True(CliCommand.TryParse([family, "set", "key", "Desk", "Speakers", "--json", "--redact"], out CliCommand command, out string? error), error);
        Assert.Equal(action, command.Action);
        Assert.Equal("Desk Speakers", command.Value);
        Assert.True(command.JsonOutput);
        Assert.True(command.RedactOutput);
    ...
    [Theory]
    [InlineData("config", "--json", "--redact")]
    [InlineData("runtime", "--json", "--redact")]
    [InlineData("config", "Dark", "--unknown")]
    [InlineData("runtime", "400", "--unknown")]
    public void TryParse_SetRejectsMissingValueAndUnknownFlags(string family, string value, string flag)
    {
        Assert.False(CliCommand.TryParse([family, "set", "key", value, flag], out _, out string? error));
    ...
    [InlineData("status")]
    [InlineData("version")]
    public void TryParse_CoreCommandHelpFlag_ParsesCoreHelp(string verb)
    {
        bool ok = CliCommand.TryParse([verb, "--help"], out CliCommand command, out string? error);

        Assert.True(ok);
        Assert.Null(error);
        Assert.Equal(CliAction.Help, command.Action);
        Assert.Equal("core", command.Key);
    }
    ...
    [InlineData("hide")]
    [InlineData("version")]
    public void TryParse_ArgumentlessCoreCommandWithExtraArgument_ReturnsInvalidUsage(string verb)
    {
        bool ok = CliCommand.TryParse([verb, "unexpected"], out _, out string? error);

        Assert.False(ok);
        Assert.Contains("Too many arguments", error, StringComparison.Ordinal);
    }
    ...
    [InlineData("network", "network list")]
