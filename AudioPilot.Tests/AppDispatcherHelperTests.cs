...
{
    [Fact]
    public void InvokeAsync_WhenShutdownAbortsQueuedWork_CompletesWithoutRunningAction()
    {
        TestExecutionGuards.RunIsolatedSta(() =>
        {
            ...
            Task task = AppDispatcherHelper.InvokeAsync(
                ...
                "dispatch-failed",
                nameof(InvokeAsync_WhenShutdownAbortsQueuedWork_CompletesWithoutRunningAction));

            dispatcher.InvokeShutdown();
            task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken).GetAwaiter().GetResult();
    ...
    [Fact]
    public void InvokeAsync_WhenAsynchronousActionFails_LogsOriginalFailureOnce()
    {
        using var scope = TestLoggerScope.CreateInMemory("dispatcher-async-failure.log");
        TestExecutionGuards.RunOnSharedSta(() =>
        {
            Task task = AppDispatcherHelper.InvokeAsync(
                Dispatcher.CurrentDispatcher,
                scope.Logger,
                async () => { await Task.Yield(); throw new IOException("operation failed"); },
                "dispatch-failed",
                nameof(InvokeAsync_WhenAsynchronousActionFails_LogsOriginalFailureOnce));
        ...
        Assert.Equal(1, text.Split("[Error]", StringSplitOptions.None).Length - 1);
    }

    [Fact]
    public void ExecuteAsync_WhenErrorDialogFails_LogsBothFailuresWithoutFaulting()
    {
        using var scope = TestLoggerScope.CreateInMemory("dispatcher-dialog-failure.log");
        var dialogs = new RecordingAppDialogService { ShowException = new IOException("dialog failed") };
        TestExecutionGuards.RunOnSharedSta(() =>
        {
            Task task = AppDispatcherHelper.ExecuteAsync(
                ...
                nameof(ExecuteAsync_WhenErrorDialogFails_LogsBothFailuresWithoutFaulting));
