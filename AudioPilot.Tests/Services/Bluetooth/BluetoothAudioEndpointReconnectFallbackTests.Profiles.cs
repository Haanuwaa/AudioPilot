...
public sealed partial class BluetoothAudioEndpointReconnectFallbackTests
{
    ...
    [Fact]
    public void Reconnect_RejectsSameNameAcrossDifferentPhysicalDevices_WithoutConfiguredIdentity()
    {
        var first = new Profile("first", "Identical headset", "bthenum-first");
        var second = new Profile("second", first.Name, "bthenum-second") { Container = Guid.NewGuid() };
        using var harness = new ProfileHarness(first, second);
        Assert.False(harness.Reconnect(first.Name, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public void Reconnect_ResolvesBothProfilesBeforeRequestsWithCachedAndFullScanSelection(bool cached, string kind)
    {
        ...
        Assert.True(harness.Reconnect(primary.Name, kind, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public void Reconnect_CompanionsRequirePhysicalIdentityAndBluetoothTopology()
    {
        ...
        Assert.True(harness.Reconnect(primary.Name, cancellationToken: TestContext.Current.CancellationToken, audioEndpointId: primary.Id));
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void Reconnect_FailedSharedTopologyOnlyTriesOtherRoutesOnSamePhysicalDevice(bool sameContainer)
    {
        ...
        Assert.Equal(sameContainer, harness.Reconnect(primary.Name, cancellationToken: TestContext.Current.CancellationToken, audioEndpointId: primary.Id));
        Assert.Equal(sameContainer ? [primary.TopologyId, primary.Id, other.Id] : [primary.TopologyId, primary.Id], harness.Factory.Requests);
        harness.AssertDisposed();
    }
    ...
    private sealed class ProfileHarness : IDisposable
    {
        ...
        public bool Reconnect(string name, string kind = "output", string audioEndpointId = "", CancellationToken cancellationToken = default) =>
            _fallback.TryReconnectEndpoints(_endpoints, name, "fixture", kind, cancellationToken, audioEndpointId);
