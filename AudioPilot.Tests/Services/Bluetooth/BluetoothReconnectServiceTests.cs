...
public sealed class BluetoothReconnectServiceTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Reconnect_PhysicalIdentityWinsOverDuplicateName_AndNeverFallsBackToOtherDevice(bool succeeds)
    {
        var source = new FakeBluetoothAssociationEndpointSource();
        source.SnapshotCandidates =
        [
            source.CreateCandidate("wrong", "Headset", true) with { ContainerId = Guid.NewGuid() },
            source.CreateCandidate("intended", "Renamed", succeeds)
        ];
        var service = new BluetoothReconnectService(Logger.Instance, new FakeBluetoothAudioEndpointReconnectFallback(), source, _ => ContainerId);
        Assert.Equal(succeeds, await service.TryReconnectPairedAudioDeviceAsync("Headset", "audio-endpoint", "identity", "output", TestContext.Current.CancellationToken));
        Assert.Equal(["intended"], source.PairedEndpointIds);
    }

    [Fact]
    public async Task Reconnect_UnknownIdentityDoesNotReuseNameCacheToResolveNewAmbiguity()
    {
        var source = new FakeBluetoothAssociationEndpointSource();
        var first = source.CreateCandidate("first", "Headset", true) with { ContainerId = null };
        source.SnapshotCandidates = [first];
        var service = new BluetoothReconnectService(Logger.Instance, new FakeBluetoothAudioEndpointReconnectFallback(), source, _ => null);
        Assert.True(await service.TryReconnectPairedAudioDeviceAsync("Headset", "audio-endpoint", "first", "output", TestContext.Current.CancellationToken));
        source.CandidateById = first;
        source.SnapshotCandidates = [first, source.CreateCandidate("second", "Headset", true) with { ContainerId = null, IsConnected = true }];
        Assert.False(await service.TryReconnectPairedAudioDeviceAsync("Headset", "audio-endpoint", "second", "output", TestContext.Current.CancellationToken));
        Assert.Equal(["first"], source.PairedEndpointIds);
    }
    [Fact]
    public async Task Reconnect_DifferentEndpointsWithSameName_DoNotShareRememberedAssociation()
    {
        var source = new FakeBluetoothAssociationEndpointSource();
        var first = source.CreateCandidate("association-a", "Bluetooth Headset", connected: true);
        var second = source.CreateCandidate("association-b", "Bluetooth Headset", connected: true);
        source.SnapshotCandidates = [first];
        var service = new BluetoothReconnectService(Logger.Instance, new FakeBluetoothAudioEndpointReconnectFallback(), source, _ => ContainerId);

        Assert.True(await service.TryReconnectPairedAudioDeviceAsync("Bluetooth Headset", "audio-a", "op-a", "output", TestContext.Current.CancellationToken));
        source.CandidateById = first;
        source.SnapshotCandidates = [second];
        Assert.True(await service.TryReconnectPairedAudioDeviceAsync("Bluetooth Headset", "audio-b", "op-b", "output", TestContext.Current.CancellationToken));

        Assert.Equal(["association-a", "association-b"], source.PairedEndpointIds);
        Assert.Equal(0, source.TryGetByIdCalls);
    }
    ...
    [Fact]
    public async Task TryReconnectPairedAudioDeviceAsync_RejectsAmbiguousTopRankCandidates()
    {
        var source = new FakeBluetoothAssociationEndpointSource();
        source.SnapshotCandidates =
        [
        ...
        Assert.False(result);
        Assert.Equal(0, source.PairCalls);
        Assert.Empty(source.PairedEndpointIds);
    }

    [Fact]
    public async Task TryReconnectPairedAudioDeviceAsync_RejectsAmbiguousSnapshot_WhenRememberedIdentityCannotBeRevalidated()
    {
        var source = new FakeBluetoothAssociationEndpointSource();
        source.SnapshotCandidates =
        [
        ...
        Assert.True(first);
        Assert.False(second);
        Assert.Equal(["endpoint-2"], source.PairedEndpointIds);
    }
    ...
    [Fact]
    public async Task TryReconnectPairedAudioDeviceAsync_AlreadyPairedAssociationEndpointResultSucceedsWhenEndpointBecomesConnected()
    {
        int originalProbeAttempts = RuntimeTuningConfig.BluetoothReconnectCachedEndpointVisibilityProbeAttempts;
        int originalProbeDelayMs = RuntimeTuningConfig.BluetoothReconnectCachedEndpointVisibilityProbeDelayMs;
        RuntimeTuningConfig.BluetoothReconnectCachedEndpointVisibilityProbeAttempts = 1;
        RuntimeTuningConfig.BluetoothReconnectCachedEndpointVisibilityProbeDelayMs = 25;
        ...
        try
        {
            ...
            [
                source.CreateCandidate(
                    ...
                    "Bluetooth Headset",
                    connected: false,
                    pairStatus: "AlreadyPaired",
                    onPair: () => source.CandidateById = new BluetoothAssociationEndpointCandidate(
                        "endpoint-1",
                        "Bluetooth Headset",
                        IsPaired: true,
                        IsConnected: true,
                        TryPairAsync: _ => Task.FromResult(new BluetoothAssociationEndpointPairAttempt(false, "AlreadyPaired"))))
            ];

            var fallback = new FakeBluetoothAudioEndpointReconnectFallback();
            var service = new BluetoothReconnectService(Logger.Instance, fallback, source);

            bool result = await service.TryReconnectPairedAudioDeviceAsync("Bluetooth Headset", "op-already-paired-observed", "output", CancellationToken.None);
        ...
        {
            RuntimeTuningConfig.BluetoothReconnectCachedEndpointVisibilityProbeAttempts = originalProbeAttempts;
            RuntimeTuningConfig.BluetoothReconnectCachedEndpointVisibilityProbeDelayMs = originalProbeDelayMs;
        }
    }
    ...
    public async Task TryReconnectPairedAudioDeviceAsync_LogsTraceWhenRememberedAssociationEndpointIdMissesAndSnapshotFallbackContinues()
    {
        ...
        var service = new BluetoothReconnectService(loggerScope.Logger, new FakeBluetoothAudioEndpointReconnectFallback(), source, _ => ContainerId);
    ...
    private sealed class FakeBluetoothAssociationEndpointSource : IBluetoothAssociationEndpointSource
    {
        ...
        public BluetoothAssociationEndpointCandidate CreateCandidate(string id, string name, bool connected, string pairStatus = "Paired", Action? onPair = null)
        {
