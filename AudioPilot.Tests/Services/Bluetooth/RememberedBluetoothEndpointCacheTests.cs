...
{
    [Fact]
    public void ClockMovingBackward_DiscardsEntriesAndAllowsFreshDiscovery()
    {
        DateTime nowUtc = new(2026, 1, 1, 12, 0, 0, DateTimeKind.Utc);
        var cache = new RememberedBluetoothEndpointCache(utcNow: () => nowUtc);
        cache.RememberEndpointId("headset", "old-endpoint");
        nowUtc = nowUtc.AddMinutes(-5);

        Assert.False(cache.TryGetEndpointId("headset", out _));
        cache.RememberEndpointId("headset", "new-endpoint");
        Assert.True(cache.TryGetEndpointId("headset", out string endpointId));
        Assert.Equal("new-endpoint", endpointId);
