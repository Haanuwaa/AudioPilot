...
public sealed class MediaSeekServiceTests
{
    ...
    [Theory]
    [InlineData(false, false, "media-playback-already-in-state", 50)]
    [InlineData(true, false, "media-playback-unsupported", 50)]
    [InlineData(true, true, "media-playback-confirmed", 40)]
    public async Task ExplicitPlaybackInvalidatesPendingSeekOnlyWhenARequestIsSent(bool initiallyPlaying, bool canPause, string expectedCode, double expectedPosition)
    {
        var session = new Session();
        session.Timeline = session.Timeline with { IsPlaying = initiallyPlaying };
        var playbackSession = new FakeMediaPlaybackSession { Status = FakeMediaPlaybackSession.Desired(initiallyPlaying), CanPause = canPause };
        ...
        var coordinator = new AppCliOverlayCoordinator(null!, overlay, new MediaOverlayCommandService(), Logger.Instance, () => new Settings(),
            ...
            mediaPlaybackService: playbackSession.CreateService());

        try
        {
            Assert.Equal(40, (await coordinator.MediaSeekAsync(false, 10, cancellationToken: TestContext.Current.CancellationToken)).TargetPositionSeconds);
            MediaPlaybackResult playback = await coordinator.MediaSetPlayingAsync(false, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(expectedCode, playback.Code);
            Assert.Equal(initiallyPlaying && canPause, playback.RequestSent);
            Assert.Equal(expectedPosition, (await coordinator.MediaSeekAsync(false, 10, cancellationToken: TestContext.Current.CancellationToken)).TargetPositionSeconds);
        }
        finally
        {
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task RemovedSessionReleasesPendingSeek_WithoutUsingItsLateResult(bool lateFault)
    {
        var completion = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        using var canceled = new CancellationTokenSource();
        var oldSession = new Session { Send = () => { canceled.Cancel(); return completion.Task; } };
        Session selected = oldSession;
        var service = new MediaSeekService(_ => Task.FromResult<IMediaSeekSession?>(selected), new Clock());
        try
        {
            Assert.Equal("media-seek-unconfirmed", (await service.SeekAsync(10, canceled.Token)).Code);
            selected = new Session();
            Assert.Equal("media-seek-busy", (await service.SeekAsync(10, TestContext.Current.CancellationToken)).Code);
            Assert.Empty(selected.Targets);
            ...
            Assert.Equal(50, (await service.SeekAsync(10, TestContext.Current.CancellationToken)).TargetPositionSeconds);
            Assert.Single(oldSession.Targets);
            Assert.Equal(2, selected.Targets.Count);
        }
        finally
        {
    ...
    [InlineData("navigation")]
    [InlineData("track")]
    public async Task LateAcceptedSeekUsesOnlyCurrentPositionEvidence(string context)
    {
        var completion = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        Task<MediaSeekResult> first = service.SeekAsync(10, cancellation.Token);
        Assert.Single(session.Targets);
        cancellation.Cancel();
        Assert.Equal("media-seek-unconfirmed", (await first).Code);
        ...
        switch (context)
        {
            case "expired": clock.NowUtc = Now.AddSeconds(3); break;
            case "navigation": service.InvalidatePosition(); break;
            case "track": session.Timeline = session.Timeline with { TrackIdentity = new("Other", "Artist", "Album", 2) }; break;
        }
        completion.SetResult(true);
        session.Send = () => Task.FromResult(true);
        MediaSeekResult next = await service.SeekAsync(10, TestContext.Current.CancellationToken);
        if (context == "expired")
        {
            Assert.Equal("media-seek-position-pending", next.Code);
            Assert.Single(session.Targets);
        }
        else
        {
            Assert.Equal(context == "unchanged" ? 50d : 40d, next.TargetPositionSeconds);
            Assert.Equal(2, session.Targets.Count);
        }
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task QueuedSeekFeedbackIsDiscardedWhenAudioStatusSupersedesIt(bool accepted)
    {
        ...
        var coordinator = new AppCliOverlayCoordinator(null!, overlay, new MediaOverlayCommandService(), Logger.Instance,
        ...
        try
        {
            await coordinator.MediaSeekAsync(false, 10, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CoordinatorUsesSavedStepAndSuppressesSupersededOverlay()
    {
        ...
        try
        {
            Task<MediaSeekResult> first = coordinator.MediaSeekAsync(false, source: "hotkey", cancellationToken: TestContext.Current.CancellationToken);
            await sent.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
            Task<MediaSeekResult> second = coordinator.MediaSeekAsync(true, 5, cancellationToken: TestContext.Current.CancellationToken);
            completion.SetResult(true);
    ...
    public async Task ExpiredQueuedSeekDoesNotLetLaterCommandsOvertakeActiveTransport()
    {
        ...
        try
        {
            ...
            Assert.Equal("media-seek-timeout", (await coordinator.MediaSeekAsync(false, cancellationToken: TestContext.Current.CancellationToken)).Code);
            Assert.False(TestPrivateAccess.GetField<Task>(coordinator, "_mediaSendOrderTail").IsCompleted);
            Task<MediaSeekResult> later = coordinator.MediaSeekAsync(false, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Empty(session.Targets);
    ...
    public async Task CoordinatorShutdownCancelsQueuedSeeksAndSuppressesLateFeedback()
    {
        ...
        var coordinator = new AppCliOverlayCoordinator(audio, overlay, new MediaOverlayCommandService(), Logger.Instance,
            () => new Settings(), mediaSeekService: new MediaSeekService(_ => Task.FromResult<IMediaSeekSession?>(session), new Clock()));
        Task<MediaSeekResult> first = coordinator.MediaSeekAsync(false, cancellationToken: TestContext.Current.CancellationToken);
        await sent.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
        Task<MediaSeekResult> queued = coordinator.MediaSeekAsync(false, cancellationToken: TestContext.Current.CancellationToken);
        ...
        Assert.Single(session.Targets);
        Assert.Empty(presenter.Messages);
        Assert.Equal("media-seek-canceled", (await coordinator.MediaSeekAsync(false, cancellationToken: TestContext.Current.CancellationToken)).Code);
    }
