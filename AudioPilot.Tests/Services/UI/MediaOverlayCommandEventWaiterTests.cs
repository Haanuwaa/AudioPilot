...
{
    [Fact]
    public async Task WaitAsync_PreCanceledWaitDoesNotConsumeQueuedEvent()
    {
        using var waiter = new MediaOverlayCommandEventWaiter();
        using var cancellation = new CancellationTokenSource();
        cancellation.Cancel();
        waiter.Signal(new(true, "browser", MediaEventAssistKind.PlaybackInfoChanged));

        Assert.False((await waiter.WaitAsync("browser", 5000, cancellation.Token)).ObservedEvent);
        Assert.Equal(0, waiter.PendingWaiterCountForTests);
        MediaEventAssistOutcome observed = await waiter.WaitAsync("browser", 5000, TestContext.Current.CancellationToken);
        Assert.True(observed.ObservedEvent);
        Assert.Equal("browser", observed.SignaledSourceAppUserModelId);
    }

    [Theory]
    [InlineData("CHROME")]
    [InlineData(null)]
    public async Task WaitAsync_CoalescesSourceBurst_WithoutConsumingAnotherSourcesEvent(string? preferredSource)
    {
        using var waiter = new MediaOverlayCommandEventWaiter();
        waiter.Signal(new(true, "chrome", MediaEventAssistKind.PlaybackInfoChanged));
        waiter.Signal(new(true, "spotify", MediaEventAssistKind.MediaPropertiesChanged));
        waiter.Signal(new(true, "chrome", MediaEventAssistKind.TimelinePropertiesChanged));
        waiter.Signal(new(true, "Chrome", MediaEventAssistKind.MediaPropertiesChanged));

        MediaEventAssistOutcome observed = await waiter.WaitAsync(preferredSource, 5000, TestContext.Current.CancellationToken);
        Assert.Equal(MediaEventAssistKind.MediaPropertiesChanged, observed.EventKind);
        Assert.Equal("Chrome", observed.SignaledSourceAppUserModelId);
        Assert.Equal(1, waiter.ObservedEventCountForTests);

        Task<MediaEventAssistOutcome> next = waiter.WaitAsync("chrome", 5000, TestContext.Current.CancellationToken);
        Assert.Equal(1, waiter.PendingWaiterCountForTests);
        waiter.Signal(new(true, "chrome", MediaEventAssistKind.PlaybackInfoChanged));
        Assert.Equal(MediaEventAssistKind.PlaybackInfoChanged, (await next).EventKind);
        Assert.Equal("spotify", (await waiter.WaitAsync("spotify", 5000, TestContext.Current.CancellationToken)).SignaledSourceAppUserModelId);
    ...
    public async Task WaitAsync_IgnoresDifferentSourceUntilRelevantEventArrives()
    {
        ...
        MediaEventAssistOutcome outcome = await waitTask.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    public async Task WaitAsync_DoesNotReplayEventThatCompletedActiveWaiter()
    {
        ...
        MediaEventAssistOutcome firstOutcome = await firstWait.WaitAsync(TimeSpan.FromSeconds(1), TestContext.Current.CancellationToken);
    ...
    public async Task Dispose_CompletesPendingWaitersAndClearsObservedEvents()
    {
        ...
        waiter.Dispose();
        MediaEventAssistOutcome outcome = await waitTask.WaitAsync(TimeSpan.FromSeconds(1), TestContext.Current.CancellationToken);
