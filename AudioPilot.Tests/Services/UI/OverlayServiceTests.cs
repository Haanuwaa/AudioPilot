...
{
    [Fact]
    public void DelayedPresentationRetainsRequestTraceAndRestoresDispatchContext()
    {
        var queue = new Queue<Action>();
        string? presentationTrace = null;
        using var service = new OverlayService(queue.Enqueue, _ =>
        {
            presentationTrace = AudioPilot.Logging.OperationTrace.CurrentId;
            return new RecordingOverlayPresenter();
        });
        using (AudioPilot.Logging.OperationTrace.Start("request", id: "overlay-request"))
            service.Show("Audio status");
        Assert.Null(AudioPilot.Logging.OperationTrace.CurrentId);
        Assert.Single(queue)();
        Assert.Equal("overlay-request", presentationTrace);
        Assert.Null(AudioPilot.Logging.OperationTrace.CurrentId);
    }

    [Fact]
    public void InitiallyDisabled_DoesNotQueuePreparationOrShow_UntilEnabled()
    {
        var queue = new Queue<Action>();
        var presenter = new RecordingOverlayPresenter();
        int created = 0;
        using var service = new OverlayService(queue.Enqueue, _ => { created++; return presenter; }, initiallyEnabled: false);

        service.PrepareForFirstUse();
        service.ShowMediaTrack("Next track", "Song", "Artist");
    ...
    [Fact]
    public void PrepareForFirstUse_CreatesOnceWithoutShowing_AndFirstHotkeyReusesPresenter()
    {
        int created = 0;
        var presenter = new RecordingOverlayPresenter();
        using var service = new OverlayService(action => action(), _ => { created++; return presenter; });
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void PrepareForFirstUse_QueuedBeforeDisableOrDispose_DoesNotCreatePresenter(bool dispose)
    {
        var queue = new Queue<Action>();
        int created = 0;
        using var service = new OverlayService(queue.Enqueue, _ => { created++; return new RecordingOverlayPresenter(); });
        service.PrepareForFirstUse();
        if (dispose)
        {
            service.Dispose();
    ...
    [Fact]
    public void PrepareForFirstUse_Failure_AllowsFirstShowToRetry()
    {
        int attempts = 0;
        var presenter = new RecordingOverlayPresenter();
        using var service = new OverlayService(action => action(), _ => ++attempts == 1
    ...
    public void UpdateDisplayOptions_NonFiniteDuration_UsesDefaultDuration(double duration)
    {
        var presenter = new RecordingOverlayPresenter();
        using var service = new OverlayService(dispatch: action => action(), presenterFactory: _ => presenter);

        service.UpdateDisplayOptions(OverlayPosition.Center, duration);
        service.Show("Safe duration");

        Assert.Equal(AudioPilot.Constants.AppConstants.Timing.OverlayAutoHideSeconds, presenter.DurationSeconds);
    }
    ...
    public void Show_ReusesExistingPresenter_AcrossCalls()
    {
        ...
        service.Show("First");
        service.Show("Second");
        service.Show(OverlayActionStateKind.Disabled, "Hold-to-mute", "Microphone muted");
        ...
        Assert.Equal("Microphone muted", Assert.Single(presenter.ActionDetails));
