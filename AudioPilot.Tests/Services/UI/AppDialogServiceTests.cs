...
public sealed partial class AppDialogServiceTests
{
    ...
    [Fact]
    public void CustomActions_WithoutCancel_DoNotConfirmOnDismissal()
    {
        var request = new AppDialogRequest(
            "message", "caption", AppDialogKind.Warning,
            [new AppDialogAction("Continue", AppDialogResult.Confirmed, isDefault: true)]);
    ...
    public async Task PresentationSound_RespectsLivePreferenceAndQueuedPresentationBoundaries()
    {
        ...
        Assert.True(((IAppDialogService)service).SoundsEnabled);
        service.SetSoundsEnabled(false);
        Assert.False(((IAppDialogService)service).SoundsEnabled);
