...
{
    [Fact]
    public async Task UnchangedReleaseReusesValidatedMetadata_AndMissingReleaseClearsCache()
    {
        int calls = 0;
        using var client = new HttpClient(new Handler((request, _) =>
        {
            calls++;
            Assert.Equal(calls is 2 or 3 ? "\"release-one\"" : string.Empty, request.Headers.IfNoneMatch.ToString());
            if (calls == 2) return Task.FromResult(new HttpResponseMessage(HttpStatusCode.NotModified));
            if (calls == 3) return Task.FromResult(new HttpResponseMessage(HttpStatusCode.NotFound));
            var response = Json("{\"draft\":false,\"prerelease\":false,\"tag_name\":\"v2.0.0\"}");
        ...
        PublishedRelease? first = await service.GetLatestAsync(TestContext.Current.CancellationToken);
        Assert.NotNull(first);
        Assert.Same(first, await service.GetLatestAsync(TestContext.Current.CancellationToken));
        Assert.Null(await service.GetLatestAsync(TestContext.Current.CancellationToken));
        Assert.NotNull(await service.GetLatestAsync(TestContext.Current.CancellationToken));
    }

    [Fact]
    public async Task InvalidMetadataDoesNotReplacePreviouslyValidatedCache()
    {
        int calls = 0;
        using var client = new HttpClient(new Handler((request, _) =>
        {
            calls++;
            if (calls > 1) Assert.Equal("\"valid\"", request.Headers.IfNoneMatch.ToString());
            if (calls == 3) return Task.FromResult(new HttpResponseMessage(HttpStatusCode.NotModified));
            var response = Json(calls == 1 ? "{\"draft\":false,\"prerelease\":false,\"tag_name\":\"v2.0.0\"}" : "{}");
            response.Headers.ETag = new System.Net.Http.Headers.EntityTagHeaderValue(calls == 1 ? "\"valid\"" : "\"invalid\"");
            return Task.FromResult(response);
        }));
        var service = new GitHubReleaseService(client);
        PublishedRelease? first = await service.GetLatestAsync(TestContext.Current.CancellationToken);
        await Assert.ThrowsAsync<InvalidDataException>(() => service.GetLatestAsync(TestContext.Current.CancellationToken));
        Assert.Same(first, await service.GetLatestAsync(TestContext.Current.CancellationToken));
    }

    [Fact]
    public async Task NotModifiedWithoutCachedMetadataIsRejected()
    {
        using var client = new HttpClient(new Handler((_, _) => Task.FromResult(new HttpResponseMessage(HttpStatusCode.NotModified))));
