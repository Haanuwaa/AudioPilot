...
public sealed class HotkeyServiceTests : IDisposable
{
    ...
    [Theory]
    [InlineData("Ctrl+WheelUp")]
    [InlineData("Pause")]
    [InlineData("PrintScreen")]
    public void RegisterMicrophoneHolds_RejectsInputsWithoutUsableHoldState(string hotkey)
    {
        Assert.False(_service.RegisterMuteHotkeys("", "", "", hotkey, ""));
        Assert.Equal(HotkeyRegistrationOutcomeKind.Unsupported, _service.GetLastRegistrationOutcome(AppConstants.Hotkeys.PushToTalkHotkeyId).Kind);
    }

    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void UnregisterHold_InvalidatesDelayedPressBeforeCancellationNotification(bool unregisterAll)
    {
        FakeKeyboardHotkeyCaptureHost? host = null;
        using var service = new HotkeyService(
            hotkeyParser: null, dispatchCoordinator: null,
            keyboardMessageHostFactory: dispatch => new FakeKeyboardHotkeyMessageHost(dispatch),
            keyboardCaptureHostFactory: dispatch => host = new FakeKeyboardHotkeyCaptureHost(dispatch));
        service.InitializeInfrastructure();
        Assert.True(service.RegisterMuteHotkeys("", "", "", "F24", ""));
        Assert.True(host!.Snapshot.TryMatch(HotkeyMainInput.FromKeyboard(Key.F24), HotkeyModifierMask.None, out var binding));
        var held = Assert.IsType<HotkeyHeldInputState>(binding.HoldState);
        held.Press();
        Action delayed = held.CapturePressCallback();
        int presses = 0;
        service.OnMicrophoneHoldPressed += (_, _) => presses++;
        service.OnMicrophoneHoldCancelled += () =>
        {
            Assert.False(held.IsHeld());
            delayed();
        };
        if (unregisterAll) service.UnregisterAllHotkeys();
        else service.UnregisterHotkey(AppConstants.Hotkeys.PushToTalkHotkeyId);
    ...
    [InlineData("Insert")]
    [InlineData("NumLock")]
    public void RegisterOutputSwitchHotkey_AllowsStandaloneSpecialKeysAndDetectsDuplicates(string key)
    {
        Assert.True(_service.RegisterOutputSwitchHotkey(key));
        Assert.False(_service.RegisterInputSwitchHotkey(key));
    }
    ...
    public void RegisterOutputSwitchHotkey_ReturnsFalse_WhenHotkeyNeedsModifier(string key)
    {
        bool registered = _service.RegisterOutputSwitchHotkey(key);
    ...
    [InlineData(null)]
    [InlineData("")]
    public void RegisterMediaHotkeys_DoesNotAssignShortcutsWhenUnconfigured(string? hotkey)
    {
        Assert.True(_service.RegisterMediaHotkeys(hotkey, hotkey, hotkey, hotkey, seekForward: hotkey, seekBackward: hotkey));
        Assert.Empty(TestPrivateAccess.GetRegisteredHotkeys(_service));
    }
    ...
    public void RegisterMediaHotkeys_UsesHookOnlyFallback_WhenOsRegistrationUnavailable()
    {
        ...
        bool registered = service.RegisterMediaHotkeys("F7", "F8", "F9", "F10", seekForward: "F11", seekBackward: "F12");
        ...
        Assert.Contains(hotkeys, hotkey => hotkey.Id == AppConstants.Hotkeys.MediaSeekForwardId);
        Assert.Contains(hotkeys, hotkey => hotkey.Id == AppConstants.Hotkeys.MediaSeekBackwardId);
    }
    ...
    [Theory]
    [InlineData("Ctrl++", Key.OemPlus, "Ctrl++")]
    [InlineData("Ctrl + +", Key.OemPlus, "Ctrl++")]
    public void ParseHotkeyString_AcceptsLiteralPlusMainKey(string value, Key expectedKey, string expectedCanonical)
    {
        var parser = new HotkeyParsingService();

        var parsed = parser.ParseHotkeyString(value);

        Assert.True(parsed.HasValue);
        Assert.Equal(expectedKey, parsed.Value.mainInput.Key);
        Assert.Equal(expectedCanonical, HotkeyParsingService.BuildComboKey(parsed.Value.modifiers, parsed.Value.mainInput));
    }
    ...
    [Theory]
    [InlineData("Ctrl+WheelLeft", "Ctrl+WheelLeft")]
    [InlineData("Ctrl+ScrollRight", "Ctrl+WheelRight")]
    public void ParseHotkeyString_AcceptsHorizontalWheelAliases(string value, string expectedCanonical)
    {
        var parser = new HotkeyParsingService();
    ...
    [Fact]
    public void RegisterMediaKeyMainInput_UsesHookOnlySoSyntheticFallbackCanPassThrough()
    {
        FakeKeyboardHotkeyMessageHost? messageHost = null;
        FakeKeyboardHotkeyCaptureHost? captureHost = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
            dispatchCoordinator: new HotkeyDispatchCoordinator(Logger.Instance, () => _utcNowTicks),
            keyboardMessageHostFactory: dispatch => messageHost = new FakeKeyboardHotkeyMessageHost(dispatch),
            keyboardCaptureHostFactory: dispatch => captureHost = new FakeKeyboardHotkeyCaptureHost(dispatch));
        service.InitializeInfrastructure();

        bool registered = service.RegisterMediaHotkeys(null, "MediaPlayPause", string.Empty, string.Empty);

        Assert.True(registered);
        Assert.NotNull(captureHost);
        Assert.True(captureHost!.IsRunning);
        Assert.NotNull(messageHost);
        Assert.Equal(0, messageHost!.RegisterCount);
    ...
    [Fact]
    public void RegisterOutputSwitchHotkey_DoesNotStartMouseHook()
    {
        FakeMouseHotkeyCaptureHost? host = null;
        FakeKeyboardHotkeyMessageHost? messageHost = null;
        using var service = new HotkeyService(
            ...
            dispatchCoordinator: new HotkeyDispatchCoordinator(Logger.Instance, () => _utcNowTicks),
            keyboardMessageHostFactory: dispatch => messageHost = new FakeKeyboardHotkeyMessageHost(dispatch),
            keyboardCaptureHostFactory: dispatch => new FakeKeyboardHotkeyCaptureHost(dispatch),
            mouseCaptureHostFactory: dispatch => host = new FakeMouseHotkeyCaptureHost(dispatch));
        service.InitializeInfrastructure();
    ...
    [Fact]
    public void InitializeInfrastructure_DoesNotStartMouseHook_WhenNoMouseHotkeysAreRegistered()
    {
        FakeMouseHotkeyCaptureHost? host = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
    ...
    [Fact]
    public void RegisterDynamicMouseHotkey_DefersHookStartUntilInitialization()
    {
        FakeMouseHotkeyCaptureHost? host = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
    ...
    [Fact]
    public void RegisterDynamicMouseHotkey_StartsHookAfterInitialization()
    {
        FakeMouseHotkeyCaptureHost? host = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
            ...
            mouseCaptureHostFactory: dispatch => host = new FakeMouseHotkeyCaptureHost(dispatch));
        service.InitializeInfrastructure();
        bool registered = service.RegisterDynamicHotkey(10009, "Ctrl+WheelUp", static () => { }, "Wheel Test");
    ...
    [Theory]
    [InlineData("Ctrl+MouseX1", "Alt+MouseX2", 2)]
    [InlineData("Mouse4", "Mouse5", 0)]
    public async Task RegisteringAdditionalMouseHotkey_UpdatesRunningHostSnapshot(string first, string second, int modifiers)
    {
        FakeMouseHotkeyCaptureHost? host = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
        ...
        int callbackCount = 0;
        Assert.True(service.RegisterDynamicHotkey(10014, first, () => Interlocked.Increment(ref callbackCount), "Mouse One"));
        Assert.True(service.RegisterDynamicHotkey(10015, second, () => Interlocked.Add(ref callbackCount, 10), "Mouse Two"));
        Assert.False(service.RegisterDynamicHotkey(10016, second, static () => { }, "Duplicate"));

        Assert.NotNull(host);
        Assert.True(host!.UpdateCount >= 1);
        Assert.False(host.Trigger(HotkeyMainInput.FromMouseButton(MouseButton.XButton2), HotkeyModifierMask.Ctrl));
        Assert.True(host.Trigger(HotkeyMainInput.FromMouseButton(MouseButton.XButton2), (HotkeyModifierMask)modifiers));
        ...
        service.UnregisterHotkey(10015);
        Assert.False(host.Trigger(HotkeyMainInput.FromMouseButton(MouseButton.XButton2), (HotkeyModifierMask)modifiers));
    }
    ...
    public async Task RapidOutputSwitchHotkeyBurst_IsDebounced()
    {
        ...
        await firstActivation.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        Assert.Equal(1, callbackCount);

        _utcNowTicks += AppConstants.Timing.HotkeyDebounceTicks + TimeSpan.TicksPerMillisecond;
        Assert.True(_messageHost.Trigger(AppConstants.Hotkeys.OutputSwitchHotkeyId));
        await secondActivation.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    {
        bool parsed = LowLevelMouseHotkeyThreadHost.TryParseMouseHookInput(
            (IntPtr)0x0201,
            IntPtr.Zero,
            out HotkeyMainInput mainInput);
    ...
    public void TryParseMouseHookInput_ParsesHorizontalWheel(short delta, string expectedToken)
    {
        IntPtr hookData = Marshal.AllocHGlobal(32);
        try
        {
            for (int offset = 0; offset < 32; offset += sizeof(int))
            {
                Marshal.WriteInt32(hookData, offset, 0);
            }

            Marshal.WriteInt32(hookData, 8, unchecked((ushort)delta << 16));

            Assert.True(LowLevelMouseHotkeyThreadHost.TryParseMouseHookInput((IntPtr)0x020E, hookData, out HotkeyMainInput input));
    ...
    public void QueuedHotkey_RemovedOrReplacedBeforeExecution_DoesNotRunOldAction(string route, bool replace)
    {
        var queued = new Queue<Action>();
        FakeKeyboardHotkeyMessageHost? messageHost = null;
        FakeKeyboardHotkeyCaptureHost? keyboardHost = null;
        FakeMouseHotkeyCaptureHost? mouseHost = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
            dispatchCoordinator: new HotkeyDispatchCoordinator(Logger.Instance, () => _utcNowTicks, queued.Enqueue),
            keyboardMessageHostFactory: dispatch => messageHost = new FakeKeyboardHotkeyMessageHost(dispatch),
            keyboardCaptureHostFactory: dispatch => keyboardHost = new FakeKeyboardHotkeyCaptureHost(dispatch),
            mouseCaptureHostFactory: dispatch => mouseHost = new FakeMouseHotkeyCaptureHost(dispatch));
        service.InitializeInfrastructure();
        int oldCalls = 0;
        int newCalls = 0;
        string shortcut = route == "mouse" ? "Alt+WheelUp" : "Ctrl+Alt+K";
        Assert.True(service.RegisterDynamicHotkey(10100, shortcut, () => oldCalls++, "Old action"));
        Assert.True(route switch
        {
            "message" => messageHost!.Trigger(10100),
            "keyboard" => keyboardHost!.Trigger(HotkeyMainInput.FromKeyboard(Key.K), HotkeyModifierMask.Ctrl | HotkeyModifierMask.Alt),
            _ => mouseHost!.Trigger(HotkeyMainInput.WheelUp, HotkeyModifierMask.Alt),
        });
        Assert.Single(queued);

        service.UnregisterHotkey(10100);
        if (replace) Assert.True(service.RegisterDynamicHotkey(10100, shortcut, () => newCalls++, "New action"));
        queued.Dequeue()();
    ...
    [Fact]
    public void VisibilityShortcut_UsesOsDeliveryAndRetainsHookDeliveryAfterRebinding()
    {
        FakeKeyboardHotkeyMessageHost? messageHost = null;
        FakeKeyboardHotkeyCaptureHost? keyboardHost = null;
        using var service = new HotkeyService(
            hotkeyParser: null,
            dispatchCoordinator: new HotkeyDispatchCoordinator(Logger.Instance, () => _utcNowTicks, action => action()),
            keyboardMessageHostFactory: dispatch => messageHost = new FakeKeyboardHotkeyMessageHost(dispatch),
            keyboardCaptureHostFactory: dispatch => keyboardHost = new FakeKeyboardHotkeyCaptureHost(dispatch));
        service.InitializeInfrastructure();
        int calls = 0;
        service.OnToggleAppVisibilityHotkeyPressed += () => calls++;

        Assert.True(service.RegisterToggleAppVisibilityHotkey("Ctrl+Alt+H"));
        Assert.False(keyboardHost!.Trigger(HotkeyMainInput.FromKeyboard(Key.H), HotkeyModifierMask.Ctrl | HotkeyModifierMask.Alt));
        Assert.True(messageHost!.Trigger(AppConstants.Hotkeys.ToggleAppVisibilityHotkeyId));
        Assert.Equal(1, calls);

        Assert.True(service.RegisterToggleAppVisibilityHotkey("MediaPlayPause"));
        _utcNowTicks += TimeSpan.TicksPerSecond;
        Assert.False(messageHost.Trigger(AppConstants.Hotkeys.ToggleAppVisibilityHotkeyId));
        Assert.True(keyboardHost.Trigger(HotkeyMainInput.FromKeyboard(Key.MediaPlayPause), HotkeyModifierMask.None));
    ...
    private sealed class FakeKeyboardHotkeyCaptureHost(Action<KeyboardHotkeyBindingSnapshot, long> dispatchMatch) : IKeyboardHotkeyCaptureHost
    {
        ...
        public bool Trigger(HotkeyMainInput mainInput, HotkeyModifierMask modifiers)
        {
            if (!Snapshot.TryMatch(mainInput, modifiers, out KeyboardHotkeyBindingSnapshot binding) || binding.UseOsDelivery)
            {
                return false;
            }
