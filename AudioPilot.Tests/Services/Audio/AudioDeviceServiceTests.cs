...
{
    [Theory]
    [InlineData(DataFlow.Capture, Role.Console, 1)]
    [InlineData(DataFlow.Capture, Role.Multimedia, 1)]
    [InlineData(DataFlow.Capture, Role.Communications, 1)]
    [InlineData(DataFlow.Render, Role.Console, 0)]
    public void DefaultMicrophoneRolesChanged_ObservesEveryCaptureRoleAndContainsSubscriberFailures(DataFlow flow, Role role, int expected)
    {
        using var service = CreateAudioService();
        int notifications = 0;
        service.DefaultMicrophoneRolesChanged += () => throw new InvalidOperationException("Subscriber failed");
        service.DefaultMicrophoneRolesChanged += () => notifications++;
        var method = typeof(AudioDeviceService).GetMethod("OnDeviceSwitchNotification", BindingFlags.Instance | BindingFlags.NonPublic);

        Assert.NotNull(method);
        method.Invoke(service, [flow, role]);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void MixerInvalidation_OnlyDiscardsCachedNamesWhenRequested(bool refreshProcessMetadata)
    {
        using var service = CreateAudioService();
        SeedRecentMixerSnapshotState(service);
        AudioSessionService sessions = TestPrivateAccess.GetField<AudioSessionService>(service, "_sessionService");
        sessions.AddProcessCacheEntryForTests(42, "browser", "Old tab", "Old tab", 0);

        service.InvalidateRecentMixerSnapshotState(refreshProcessMetadata);

        Assert.Null(sessions.GetRecentSnapshotDataForTests(AudioMixerMode.Output).Snapshot);
        Assert.Null(sessions.GetRecentSnapshotDataForTests(AudioMixerMode.Input).Snapshot);
        Assert.Equal(refreshProcessMetadata ? 0 : 1, sessions.ProcessCacheCountForTests);
    }
