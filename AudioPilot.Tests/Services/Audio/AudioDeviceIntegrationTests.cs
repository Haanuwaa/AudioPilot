...
public sealed class AudioDeviceIntegrationTests
{
    ...
    {
        if (!TestExecutionGuards.RequireAudioHardwareEnabled(nameof(SwitchAudioDeviceAsync_UpdatesAllRenderRoles_WhenIntegrationEnabled)))
        {
            return;
        }
    ...
    {
        if (!TestExecutionGuards.RequireAudioHardwareEnabled(nameof(SwitchInputDeviceToAsync_UpdatesAllCaptureRoles_WhenIntegrationEnabled)))
        {
            return;
        }
    ...
    {
        if (!TestExecutionGuards.RequireAudioHardwareEnabled(nameof(SetMuteMethods_UpdateAndRestoreAllDefaultEndpoints_WhenIntegrationEnabled)))
        {
            return;
        }
    ...
    public async Task SuccessfulSwitchSoak_AlternatesOutputAndInput_AndRestoresEveryOriginalRole()
    {
        if (!TryGetIntegrationDevicePair(OutputDeviceAEnvVar, OutputDeviceBEnvVar, out var outputA, out var outputB) ||
            !TryGetIntegrationDevicePair(InputDeviceAEnvVar, InputDeviceBEnvVar, out var inputA, out var inputB)) return;
        Assert.NotEqual(outputA, outputB);
        Assert.NotEqual(inputA, inputB);
        using var enumerator = new MMDeviceEnumerator();
        Role[] roles = [Role.Console, Role.Multimedia, Role.Communications];
        if (!EnsureDefaultEndpointsAvailable(enumerator, DataFlow.Render, roles) ||
            !EnsureDefaultEndpointsAvailable(enumerator, DataFlow.Capture, roles) ||
            !EnsureConfiguredDevicesAreActive(enumerator, DataFlow.Render, outputA, outputB) ||
            !EnsureConfiguredDevicesAreActive(enumerator, DataFlow.Capture, inputA, inputB)) return;
        var original = new[] { DataFlow.Render, DataFlow.Capture }.SelectMany(flow => roles.Select(role =>
            (Flow: flow, Role: role, Id: TryGetDefaultEndpointId(enumerator, flow, role)!))).ToArray();
        using var service = new AudioDeviceService();
        service.UpdateRoleConfiguration(["Console", "Multimedia", "Communications"], ["Console", "Multimedia", "Communications"]);
        ...
        try
        {
            Assert.True(await PrimeDefaultEndpointsAsync(enumerator, DataFlow.Render, outputA, roles));
            Assert.True(await PrimeDefaultEndpointsAsync(enumerator, DataFlow.Capture, inputA, roles));
            service.RegisterNotificationClient();
            while (elapsed.Elapsed < duration)
            {
                TestContext.Current.CancellationToken.ThrowIfCancellationRequested();
                string output = cycles % 2 == 0 ? outputB : outputA;
                string input = cycles % 2 == 0 ? inputB : inputA;
                Assert.True((await service.SwitchAudioDeviceAsync(output, false, false, false, false)).Success,
                    $"Output switch failed at cycle {cycles}.");
                Assert.True((await service.SwitchInputDeviceToAsync(input, "Soak input", false, null)).Success,
                    $"Input switch failed at cycle {cycles}.");
                await WaitForAllDefaultRolesAsync(enumerator, DataFlow.Render, output);
                await WaitForAllDefaultRolesAsync(enumerator, DataFlow.Capture, input);
                cycles++;
                if (cycles >= 4 && elapsed.Elapsed >= resources.NextSampleAt) resources.Sample(elapsed.Elapsed);
                await Task.Delay(300, TestContext.Current.CancellationToken);
            }
            Assert.True(cycles >= 4, $"Only {cycles} successful switch cycles completed.");
            resources.AssertBounded($"device-switch cycles={cycles}");
        }
        catch (Exception ex) { failures.Add(ex); }
        finally
        {
            try { service.UnregisterNotificationClient(); }
            catch (Exception ex) { failures.Add(ex); }
            foreach (var assignment in original)
            {
                try { AudioPolicyConfig.SetDefaultDevice(assignment.Id, assignment.Role); }
                catch (Exception ex) { failures.Add(ex); }
            }
            foreach (var assignment in original)
            {
                ...
                {
                    await TestExecutionGuards.WaitUntilAsync(
                        () => TryGetDefaultEndpointId(enumerator, assignment.Flow, assignment.Role) == assignment.Id,
                        $"Original {assignment.Flow}/{assignment.Role} default was not restored.", TimeSpan.FromSeconds(3));
                }
                catch (Exception ex) { failures.Add(ex); }
            }
        }
        if (failures.Count > 0) throw new AggregateException("Audio switch soak or original-default restoration failed.", failures);
    }
