...
public sealed class AudioDeviceResumeRecoveryHelperTests
{
    ...
    [Fact]
    public async Task RunBestEffortRecoveryAsync_StopsBeforeMonitorUpdate_WhenShutdownStartsDuringRegistration()
    {
        using var loggerScope = new TestLoggerScope(nameof(AudioDeviceResumeRecoveryHelperTests), "resume-helper-mid-shutdown.log");
        using var cts = new CancellationTokenSource();
        int updateCalls = 0;
        var helper = new AudioDeviceResumeRecoveryHelper(
            loggerScope.Logger, () => false, () => false, cts.Cancel, () => updateCalls++);
