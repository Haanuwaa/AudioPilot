...
public sealed class AudioEndpointTestServiceTests
{
    ...
    [Fact]
    public async Task MonitorCleanupFailure_PublishesActualMonitorState()
    {
        var input = new FakeInputSession(InputA)
        {
            AfterConfigureFailure = new InvalidOperationException("old monitor cleanup failed"),
        };
        ...
        await service.ConfigureInputMonitoringAsync(true, OutputB, 0.7f, TestContext.Current.CancellationToken);

        Assert.Equal(AudioEndpointTestFailureKind.Unexpected, service.CurrentState.FailureKind);
        Assert.True(service.CurrentState.MonitoringEnabled);
        Assert.Equal(OutputB, service.CurrentState.MonitorEndpoint);
        Assert.Equal(0.7f, service.CurrentState.MonitorVolume);
        Assert.Equal(0, input.DisposeCount);
    }

    [Fact]
    public async Task CanceledMonitorChange_PreservesRunningStateWithoutReportingFailure()
    {
        ...
        await using var service = new AudioEndpointTestService(new FakeFactory { InputSession = input });
        await service.StartInputTestAsync(InputA, null, TestContext.Current.CancellationToken);
        AudioEndpointTestState before = service.CurrentState;

        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            service.ConfigureInputMonitoringAsync(true, OutputA, 0.5f, cancellation.Token));

        Assert.Equal(before, service.CurrentState);
        Assert.Equal(0, input.DisposeCount);
    }
    ...
    private sealed class FakeInputSession(AudioEndpointReference endpoint) : IAudioInputTestSession
    {
        ...
        public Exception? AfterConfigureFailure { get; init; }
        public Func<Task>? ConfigureOverride { get; init; }
        ...
        public Task ConfigureMonitoringAsync(bool enabled, AudioEndpointReference? monitorEndpoint, float volume, CancellationToken cancellationToken)
        {
            ...
            if (ConfigureOverride != null) return ConfigureOverride();
            ...
            return AfterConfigureFailure == null ? Task.CompletedTask : Task.FromException(AfterConfigureFailure);
        }
        public ValueTask DisposeAsync()
        {
            DisposeCount++;
