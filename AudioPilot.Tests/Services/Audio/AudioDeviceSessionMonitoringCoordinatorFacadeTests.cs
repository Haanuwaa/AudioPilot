...
{
    [Fact]
    public void OutputUpdate_DoesNotStopInputConsumerAcquiredDuringScheduling()
    {
        using var sessionService = new AudioSessionService(new StubEnumerator());
        AudioDeviceSessionMonitoringCoordinatorFacade? facade = null;
        bool acquireInput = true;
        facade = CreateFacade(sessionService, _ => { }, () =>
        {
            if (!acquireInput) return;
            acquireInput = false;
            facade!.Acquire(AudioMixerMode.Input);
        });
        ...
        try
        {
            ...
            Assert.Equal(1, facade.GetConsumerCountForTests(AudioMixerMode.Input));
            Assert.True(facade.IsMonitoringActiveForTests(AudioMixerMode.Input));
    ...
    private static AudioDeviceSessionMonitoringCoordinatorFacade CreateFacade(
        ...
        Action? onPlaybackWorkScheduled = null)
    {
        ...
        var playbackCoordinator = new SessionMonitorCoordinator(
            ...
            static (_, _, _, _) => { },
            static _ => { },
            (_, _) => onPlaybackWorkScheduled?.Invoke(),
            static () => false);
