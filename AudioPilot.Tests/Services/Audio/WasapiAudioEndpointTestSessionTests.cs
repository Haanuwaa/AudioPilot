...
public sealed class WasapiAudioEndpointTestSessionTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task MonitorPlaybackFailure_CompletesInputTestIncludingDuringStart(bool duringStart)
    {
        using var logger = TestLoggerScope.CreateInMemory("monitor-stopped.log");
        var failure = new InvalidOperationException("render device lost");
        var player = new FakePlayer { StopDuringPlay = duringStart ? failure : null };
        var factory = new FakeMonitorFactory();
        factory.Results.Enqueue(new AudioTestMonitorPlaybackResources(player, null, null));
        await using var session = new WasapiInputTestSession(
            Input, null, new FakeDisposable(), new FakeDisposable(), new FakeRecorder(), factory, logger.Logger);

        await session.ConfigureMonitoringAsync(true, Output, 0.5f, TestContext.Current.CancellationToken);
        if (!duringStart) player.RaiseStopped(failure);

        AudioEndpointTestException exception = await Assert.ThrowsAsync<AudioEndpointTestException>(
            () => session.Completion.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken));
        Assert.Same(failure, exception.InnerException);
        Assert.Contains("monitoring stopped", exception.Message);
    }

    [Fact]
    public async Task MonitorStartFailure_CompletesInputTestAndReleasesResources()
    {
        using var logger = TestLoggerScope.CreateInMemory("monitor-start.log");
        var player = new FakePlayer { PlayFailure = new InvalidOperationException("start failed") };
        var device = new FakeDisposable();
        var factory = new FakeMonitorFactory();
        factory.Results.Enqueue(new AudioTestMonitorPlaybackResources(player, null, device));
        var session = new WasapiInputTestSession(
            Input, null, new FakeDisposable(), new FakeDisposable(), new FakeRecorder(), factory, logger.Logger);

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            session.ConfigureMonitoringAsync(true, Output, 0.5f, TestContext.Current.CancellationToken));
        await Assert.ThrowsAsync<AudioEndpointTestException>(() => session.Completion);
    ...
    [Fact]
    public async Task ReplacedOrDisabledMonitor_LateStoppedCallbackDoesNotStopCapture()
    {
        using var logger = TestLoggerScope.CreateInMemory("monitor-stale-stop.log");
        var first = new FakePlayer();
        var second = new FakePlayer();
        var factory = new FakeMonitorFactory();
        factory.Results.Enqueue(new AudioTestMonitorPlaybackResources(first, null, null));
        factory.Results.Enqueue(new AudioTestMonitorPlaybackResources(second, null, null));
        await using var session = new WasapiInputTestSession(
            Input, null, new FakeDisposable(), new FakeDisposable(), new FakeRecorder(), factory, logger.Logger);

        await session.ConfigureMonitoringAsync(true, Output, 0.5f, TestContext.Current.CancellationToken);
        Action firstStopped = first.CaptureStoppedCallback();
        await session.ConfigureMonitoringAsync(true, new("other", "Other"), 0.5f, TestContext.Current.CancellationToken);
        firstStopped();
        Action secondStopped = second.CaptureStoppedCallback();
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Start_RecordsLowLatencyFallbackReason(bool input)
    {
        using var logger = TestLoggerScope.CreateFileBacked("wasapi-latency", "fallback.log");
        const string reason = "IAudioClient3 is not supported";
        if (input)
        {
            var recorder = new FakeRecorder { LowLatencyUnavailableReason = reason };
            await using var session = new WasapiInputTestSession(
                Input, null, new FakeDisposable(), new FakeDisposable(), recorder, new FakeMonitorFactory(), logger.Logger);
            session.Start();
        }
        else
        {
            var player = new FakePlayer { LowLatencyUnavailableReason = reason };
            await using var session = new WasapiOutputTestSession(
                Output, new FakeDisposable(), new FakeDisposable(), player, logger.Logger);
            session.Start();
        }

        string log = logger.DisposeAndReadLogText();
        Assert.Contains(input ? "input-test-stream-started" : "output-test-stream-started", log);
        Assert.Contains("lowLatency=False", log);
        Assert.Contains($"lowLatencyUnavailableReason={reason}", log);
    }

    [Fact]
    public async Task SilentCapturePacket_ProducesSilenceInMonitorDespiteNonzeroPacketData()
    {
        using var logger = TestLoggerScope.CreateInMemory("silent-capture.log");
        var recorder = new FakeRecorder();
        var factory = new FakeMonitorFactory();
        factory.Results.Enqueue(new AudioTestMonitorPlaybackResources(new FakePlayer(), null, null));
        await using var session = new WasapiInputTestSession(
            Input, null, new FakeDisposable(), new FakeDisposable(), recorder, factory, logger.Logger);
        await session.ConfigureMonitoringAsync(true, Output, 0.5f, TestContext.Current.CancellationToken);

        byte[] packet = [.. Enumerable.Repeat((byte)0x7F, 1024)];
        recorder.RaiseData(packet, AudioClientBufferFlags.Silent);
        byte[] playback = new byte[packet.Length];
        Assert.Equal(playback.Length, factory.Source!.Read(playback));

        Assert.All(playback, value => Assert.Equal(0, value));
        Assert.Equal(0, session.ReadLevel().LevelPercent);
        Assert.False(session.Completion.IsCompleted);
    }
    ...
    private sealed class FakePlayer : IAudioTestPlayerAdapter
    {
        ...
        public string? LowLatencyUnavailableReason { get; init; }
        public int LatencyMilliseconds { get; init; } = 50;
        public float Volume { get; set; }
        public Exception? PlayFailure { get; init; }
        public Exception? StopDuringPlay { get; init; }
        public int SubscriberCount => _playbackStopped?.GetInvocationList().Length ?? 0;
        public void RaiseStopped(Exception? exception) => _playbackStopped?.Invoke(this, new StoppedEventArgs(exception));
        public Action CaptureStoppedCallback()
        {
            EventHandler<StoppedEventArgs>? callback = _playbackStopped;
            return () => callback?.Invoke(this, new StoppedEventArgs(new InvalidOperationException("late stop")));
        }
        ...
        public void Play()
        {
            ...
            if (StopDuringPlay != null) RaiseStopped(StopDuringPlay);
        }
    ...
    private sealed class FakeRecorder : IAudioTestRecorderAdapter
    {
        ...
        public void RaiseData(byte[] data, AudioClientBufferFlags flags = AudioClientBufferFlags.None) => _dataAvailable?.Invoke(data, flags, 0, 0);
        public ValueTask DisposeAsync()
        {
            DisposeCount++;
            return DisposeFailure == null ? ValueTask.CompletedTask : ValueTask.FromException(DisposeFailure);
