...
public sealed class VolumeControlServiceTests
{
    ...
    [Fact]
    public void UpdateSessionVolume_KeepsPlaybackAndRecordingCachesIndependent()
    {
        using var service = CreateService();
        service.UpdateSessionVolume("Discord", "discord.exe", 23f);
        service.UpdateSessionVolume("Discord", "discord.exe", 61f, AudioMixerMode.Input);

        var playback = TestPrivateAccess.GetField<VolumeCacheStore>(service, "_cacheStore");
        var recording = TestPrivateAccess.GetField<VolumeCacheStore>(service, "_recordingCacheStore");
        Assert.Equal(23f, playback.TryGetCachedVolume("discord"));
        Assert.Equal(61f, recording.TryGetCachedVolume("discord"));

        service.UpdateSessionVolume("Discord", "discord.exe", 42f);
        Assert.Equal(42f, playback.TryGetCachedVolume("discord"));
        Assert.Equal(61f, recording.TryGetCachedVolume("discord"));
    }
