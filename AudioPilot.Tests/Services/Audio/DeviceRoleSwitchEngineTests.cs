...
public sealed class DeviceRoleSwitchEngineTests
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task FailedSwitch_RollsBackRemainingRoles_WhenOneRollbackFails(bool output)
    {
        RuntimeTuningConfig.SwitchMaxRetries = 1;
        using var loggerScope = new TestLoggerScope(nameof(DeviceRoleSwitchEngineTests), "switch-rollback-partial.log");
        var rollbackAttempts = new List<NRole>();
        bool applied = false;
        void Apply(string id, NRole role)
        {
            if (id == "target") { applied = true; return; }
            rollbackAttempts.Add(role);
            if (role == NRole.Multimedia) throw new COMException("Endpoint disappeared");
        }
        string GetDefault(NRole role) => !applied ? $"original-{role}" : role == NRole.Communications ? "external-choice" : "target";

        bool success = output
            ? await DeviceRoleSwitchEngine.TrySwitchOutputRolesAsync(
                "target", [NRole.Multimedia, NRole.Console, NRole.Communications], Apply, GetDefault, loggerScope.Logger,
                "rollback-test", "test", TestContext.Current.CancellationToken)
            : await DeviceRoleSwitchEngine.TrySwitchInputRolesAsync(
                "target", "Microphone", [NRole.Multimedia, NRole.Console, NRole.Communications], Apply, GetDefault, loggerScope.Logger,
                "rollback-test", "test", false, false, TestContext.Current.CancellationToken);

        Assert.False(success);
        Assert.Equal([NRole.Multimedia, NRole.Console], rollbackAttempts);
        Assert.Contains("role=Multimedia", loggerScope.DisposeAndReadLogText());
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public async Task Switch_WithMissingPreviousDefault_AssignsValidTarget(bool output)
    {
        string? current = null;
        using var logger = new TestLoggerScope(nameof(DeviceRoleSwitchEngineTests), "missing-default.log");
        string GetDefault(NRole _) => current ?? throw new COMException("No default", unchecked((int)0x80070490));
        void Apply(string id, NRole _) => current = id;
        bool success = output
            ? await DeviceRoleSwitchEngine.TrySwitchOutputRolesAsync("target", [NRole.Console], Apply, GetDefault, logger.Logger, "missing", "test", TestContext.Current.CancellationToken)
            : await DeviceRoleSwitchEngine.TrySwitchInputRolesAsync("target", "Target", [NRole.Console], Apply, GetDefault, logger.Logger, "missing", "test", false, false, TestContext.Current.CancellationToken);
        Assert.True(success);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task Rollback_PreservesExternalChoice_AndUntouchedRoles(bool output)
    {
        RuntimeTuningConfig.SwitchMaxRetries = 1;
        using var logger = new TestLoggerScope(nameof(DeviceRoleSwitchEngineTests), "external-choice.log");
        var defaults = new Dictionary<NRole, string> { [NRole.Console] = "original", [NRole.Multimedia] = "original", [NRole.Communications] = "target" };
        var writes = new List<(string, NRole)>();
        void Apply(string id, NRole role)
        {
            writes.Add((id, role));
            if (role == NRole.Multimedia)
            {
                defaults[NRole.Console] = "external-choice";
                throw new COMException("Disconnected");
            }
            defaults[role] = id;
        }
        Task Switch() => output
            ? DeviceRoleSwitchEngine.TrySwitchOutputRolesAsync("target", [NRole.Console, NRole.Multimedia, NRole.Communications], Apply, role => defaults[role], logger.Logger, "external", "test", TestContext.Current.CancellationToken)
            : DeviceRoleSwitchEngine.TrySwitchInputRolesAsync("target", "Target", [NRole.Console, NRole.Multimedia, NRole.Communications], Apply, role => defaults[role], logger.Logger, "external", "test", false, false, TestContext.Current.CancellationToken);
        await Assert.ThrowsAsync<COMException>(Switch);
        Assert.Equal("external-choice", defaults[NRole.Console]);
        Assert.Equal("original", defaults[NRole.Multimedia]);
        Assert.Equal("target", defaults[NRole.Communications]);
        Assert.Equal(2, writes.Count);
    }

    [Fact]
    public void ReadDefault_DoesNotSuppressOtherComFailures()
    {
        Assert.Throws<COMException>(() => DeviceRoleSwitchEngine.ReadDefaultDeviceId(_ => throw new COMException("Access denied", unchecked((int)0x80070005)), NRole.Console));
    }
