...
public sealed class SettingsServiceTests : IDisposable
{
    ...
    [InlineData("fallback")]
    [InlineData("backup")]
    public void DeleteSettingsFiles_ReportsLockedFiles_AndCanBeRetried(string target)
    {
        var service = new SettingsService(_workspace.PrimaryDir, _workspace.FallbackDir);
        string primary = Path.Combine(_workspace.PrimaryDir, "settings.json");
        string fallback = Path.Combine(_workspace.FallbackDir, "settings.json");
        string backupDirectory = Path.Combine(_workspace.PrimaryDir, AppConstants.Files.BackupFolderName);
        Directory.CreateDirectory(backupDirectory);
        string backup = Path.Combine(backupDirectory, "settings.json.bak");
        File.WriteAllText(primary, "{}");
        File.WriteAllText(fallback, "{}");
        File.WriteAllText(backup, "{}");
        string lockedPath = target switch { "primary" => primary, "fallback" => fallback, _ => backup };
        using (File.Open(lockedPath, FileMode.Open, FileAccess.Read, FileShare.Read))
        {
            Assert.Throws<AggregateException>(service.DeleteSettingsFiles);
