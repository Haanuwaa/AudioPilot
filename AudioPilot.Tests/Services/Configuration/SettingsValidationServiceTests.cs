...
public sealed class SettingsValidationServiceTests
{
    ...
    [InlineData(64, false)]
    [InlineData(65, true)]
    public void RoutineNameLimit_IsValidatedWithoutSilentlyTruncatingImportedNames(int length, bool invalid)
    {
        var routine = new AudioRoutine { Name = new string('N', length), MasterVolumePercent = 40 };
        var settings = new Settings { Routines = new() { Items = [routine] } };
        SettingsValidationService.Normalize(settings);
        Assert.Equal(length, settings.Routines.Items[0].Name.Length);
        var diagnostics = SettingsValidationService.EvaluateDiagnostics(settings, [], []);
        Assert.Equal(invalid, diagnostics.Warnings.Any(warning => warning.Code == "invalid-routine-name-0"));
    }

    [Fact]
    public void NormalizeAndSerialization_PreserveManualAccessOnAutomaticRoutine()
    {
        var routine = new AudioRoutine
        {
            TriggerKind = RoutineTriggerKind.Network,
            TriggerNetworkName = "Office",
        ...
        SettingsValidationService.Normalize(settings);
        var reopened = System.Text.Json.JsonSerializer.Deserialize<AudioRoutine>(System.Text.Json.JsonSerializer.Serialize(Assert.Single(settings.Routines.Items)));
        Assert.NotNull(reopened);
        Assert.Equal("Ctrl+Alt+R", reopened.Hotkey);
        Assert.True(reopened.ShowInTrayMenu);
        Assert.Equal("Office", reopened.TriggerNetworkName);
        Assert.Equal(NetworkTriggerDirection.Disconnect, reopened.NetworkTriggerDirection);
    }

    [Fact]
    public void PushToTalk_RequiresAHotkeyOnlyWhenEnabled()
    {
        var settings = new Settings();
        const string code = "invalid-hotkey-push-to-talk-hotkey";
        Assert.DoesNotContain(SettingsValidationService.EvaluateDiagnostics(settings, [], []).Warnings, warning => warning.Code == code);
        settings.Hotkeys.Mute.PushToTalkEnabled = true;
        Assert.Contains(SettingsValidationService.EvaluateDiagnostics(settings, [], []).Warnings, warning => warning.Code == code);
        settings.Hotkeys.Mute.PushToTalk = "Mouse4";
        Assert.DoesNotContain(SettingsValidationService.EvaluateDiagnostics(settings, [], []).Warnings, warning => warning.Code == code);
    }
    [Theory]
    [InlineData(double.NaN, AppConstants.Timing.OverlayAutoHideSeconds)]
    [InlineData(double.PositiveInfinity, AppConstants.Timing.OverlayAutoHideSeconds)]
    [InlineData(double.NegativeInfinity, AppConstants.Timing.OverlayAutoHideSeconds)]
    [InlineData(0.0, 0.5)]
    [InlineData(11.0, 10.0)]
    [InlineData(3.75, 3.75)]
    public void Normalize_OverlayDuration_IsFiniteAndWithinDisplayLimits(double duration, double expected)
    {
        var settings = new Settings();
        settings.Overlay.DurationSeconds = duration;

        SettingsValidationService.Normalize(settings);

        Assert.Equal(expected, settings.Overlay.DurationSeconds);
    }

    [Fact]
    public void Normalize_ReplacesInvalidScheduledTimeZoneWithLocalTimeZone()
    {
        var settings = new Settings
        {
        ...
        Assert.Equal(TimeZoneInfo.Local.Id, Assert.Single(settings.Routines.Items).ScheduleTimeZoneId);
    }

    [Fact]
    public void Normalize_AssignsUniqueIdsToDuplicateRoutines()
    {
        var settings = new Settings
        {
            Routines = new RoutinesSettings
            {
                Items =
                [
                    new AudioRoutine { Id = "duplicate", Name = "One", OutputDeviceId = "out-1" },
                    new AudioRoutine { Id = "DUPLICATE", Name = "Two", OutputDeviceId = "out-2" },
                ]
            }
        };
        ...
        Assert.Equal(2, settings.Routines.Items.Select(static routine => routine.Id).Distinct(StringComparer.OrdinalIgnoreCase).Count());
        Assert.Equal("duplicate", settings.Routines.Items[0].Id);
        Assert.NotEqual("DUPLICATE", settings.Routines.Items[1].Id);
    }
    ...
    [Fact]
    public void Normalize_PreservesNetworkName_ForDisconnectOnlyRoutine()
    {
        var settings = new Settings
        {
            Routines = new RoutinesSettings
    ...
    public void EvaluateDiagnostics_TreatsMouseHotkeysWithoutModifierAsInvalid()
    {
        ...
        Assert.Contains(diagnostics.Warnings, warning => warning.Code == "invalid-hotkey-toggle-app-visibility-hotkey");
    ...
    public void EvaluateDiagnostics_ReportsReservedWindowsHotkeys()
    {
        ...
        Assert.Contains(diagnostics.Warnings, warning => warning.Code == "reserved-hotkey-toggle-app-visibility-hotkey");
    ...
    [Fact]
    public void EvaluateDiagnostics_AllowsStandaloneSpecialKeysButRejectsNavigationKeys()
    {
        var settings = new Settings
        {
            Hotkeys = new HotkeysSettings
            {
                App = new HotkeysAppSettings
                {
                    ToggleAppVisibility = "PrintScreen"
                },
                Media = new HotkeysMediaSettings
                {
                    PlayPause = "Home"
        ...
        Assert.DoesNotContain(diagnostics.Warnings, warning => warning.Code == "invalid-hotkey-toggle-app-visibility-hotkey");
