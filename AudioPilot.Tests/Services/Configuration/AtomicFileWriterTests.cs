...
public sealed class AtomicFileWriterTests : IDisposable
{
    ...
    [Fact]
    public void Write_WhenDestinationIsLocked_PreservesItAndRemovesCompletedTemporaryFile()
    {
        Directory.CreateDirectory(_directory);
        string destination = Path.Combine(_directory, "export.json");
        File.WriteAllText(destination, "original");
        using var lockedDestination = new FileStream(destination, FileMode.Open, FileAccess.Read, FileShare.Read);
        bool temporaryWriteCompleted = false;

        Assert.ThrowsAny<IOException>(() => AtomicFileWriter.Write(destination, temporaryPath =>
        {
