...
The direct-output check verifies that the Windows multimedia default does not change. The microphone check verifies real packet metering and, only when the optional monitor endpoint is supplied, bounded live-monitor startup and teardown. Use headphones for the monitor endpoint.

The integration suite validates managed endpoint notifications and real session-wrapper callbacks. The hardware soak also validates rapid default-device switching and checks managed memory, private memory, and process-handle growth while alternating session creation across active output endpoints. A physical unplug/replug, Bluetooth reconnect, and real sleep/resume still require suitable hardware and operator control on the runner.
...
- Release messaging must describe this as a technical minimum, not as blanket support for every Windows release at or above that build. Official support is limited to Windows 11 and currently supported Windows 10 Enterprise/LTSC editions that meet the floor, subject to the Microsoft and .NET 10 support lifecycles.
