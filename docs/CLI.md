...
- With Bluetooth reconnect enabled, headless switching and routine device targets wait for the audio endpoint to become available after Windows accepts the reconnect request (up to 12 seconds by default). Device notifications trigger early checks, with periodic checks covering missed notifications. An accepted reconnect request alone does not mean the switch succeeded.
...
Use media status when you need the current media snapshot in text or JSON form. Play/pause and track navigation remain fire-and-forget. Separate play and pause commands await an explicit request, succeed without sending when already in the requested state, and never fall back to a toggle. They report accepted versus confirmed playback; exit code 0 includes accepted requests whose state is not yet confirmed. They do not show overlays. Seeking reports the result; durations accept seconds or minutes (90, 90s, 1.5m, 1m30s, or 1:30). Omit the duration to use media-seek-step-seconds (default 10 seconds, range 1 second to 60 minutes). Seeking requires the selected player to expose a seekable timeline; live streams and some browser sessions may not support it.
...
If a log cannot be read, the bundle still includes the available diagnostics and marks the export as partial. The manifest records the failed entry's exception type and HRESULT without its exception message or source path. A failure to write the archive fails the export and preserves any existing destination file.
...
- Logs rotate around 5 MiB or when their creation date is at least three UTC calendar days old, including across app restarts. Log backups last written more than 21 days ago are removed at startup, on the first log write of each UTC day, and after size-based rotation. Settings backups have no age limit.
...
`use-scheduled-startup` selects a per-user Task Scheduler logon task instead of the usual Run registry entry. It defaults to `false` and takes effect only when `run-at-startup` is enabled. The task uses no configured delay, normal priority, and no elevation. It may start sooner after sign-in; timing depends on Windows startup load. Changing the mode while startup is enabled replaces the registration immediately. For example:
