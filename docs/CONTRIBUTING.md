...
Use the category runner for integration and stress runs; an xUnit trait filter selects tests but does not enable their environment opt-ins. Do not combine `--filter-query` with simple class, method, or trait filters.

The scripts clear inherited desktop and audio permissions for ordinary runs. `full`, `integration`, and `validate-all.ps1 -IncludeIntegration` do not enable tests that mute devices or start real audio sessions.
To run those checks on a dedicated test system, use `-Category integration -AllowHardwareAudio`. The release workflow opts in on its disposable runner.

`-Category visual -AllowDesktopInteraction` enables `AUDIOPILOT_RUN_INTEGRATION=1`, `AUDIOPILOT_RUN_VISUAL_WPF=1`, and `AUDIOPILOT_TEST_SHOW_WINDOWS=1`. These tests can show windows, popups, tray menus, and change focus.
An offscreen parent does not keep a WPF popup offscreen, so popup tests belong in this category too. `-Category hardware-soak -AllowHardwareAudio` enables strict hardware enforcement and requires all four
`AUDIOPILOT_TEST_*_DEVICE_A/B` variables. Broad review or validation requests do not imply permission to run either interactive suite on an occupied desktop.

Plain `dotnet test` is now unit-oriented by default:

- real audio tests discovery-skip unless both `AUDIOPILOT_RUN_INTEGRATION=1` and `AUDIOPILOT_RUN_AUDIO_HARDWARE=1` are set; stress tests require `AUDIOPILOT_RUN_STRESS=1`,
- visible WPF tests require `AUDIOPILOT_RUN_INTEGRATION=1`, `AUDIOPILOT_RUN_VISUAL_WPF=1`, and `AUDIOPILOT_TEST_SHOW_WINDOWS=1`,
...
- `scripts/run-tests.ps1`: run unit, integration, visual, stress, hardware-soak, or full suites, with optional coverage. Visible UI requires `-AllowDesktopInteraction`; real audio requires `-AllowHardwareAudio`.
...
See [scripts/README.md](../scripts/README.md) for the complete script reference. Editor task files under `.vscode/` are local and are not shipped with the repository.
