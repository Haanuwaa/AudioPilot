...
The task uses no configured logon delay, normal process priority, no idle/network/battery requirement, and no execution time limit. `StartupTaskDefinition` compares effective task values because Windows omits schema defaults and converts the trigger SID to an account name. XML boolean values accept both textual and numeric forms. Initialization and path repair preserve a manually disabled task or trigger; an explicit startup-enable action restores it. Startup reconciliation runs off the dispatcher, and a healthy Run registration avoids Task Scheduler COM calls. Tray startup attempts actual registration immediately and retries asynchronously every 250 ms for up to ten seconds, with shutdown cancellation and the existing main-window fallback. Retry timing is logged; individual registration failures use debug logging during this bounded startup window.
...
`UpdateCheckViewModel` serializes opt-in checks, cancels them on disable/shutdown, and retains the daily success or hourly failure cooldown across option toggles. `GitHubReleaseService` requests public metadata with a ten-second timeout, a one-MiB decoded response limit, no cookies or redirects, and a repository-controlled release URL. Its per-instance ETag cache retains only validated metadata, reuses it on HTTP 304, and clears it when the release endpoint returns 404. Conditional requests and compression reduce repeated transfer and parsing; checks remain unauthenticated and are still subject to GitHub's rate limits. No update assets are downloaded.

### List Reordering

`ListBoxReorderBehavior` handles drag thresholds, selection gestures, insertion feedback, and edge scrolling for output, input, and routine lists. Only a drag originating in the same list is accepted, and a change to item order or membership during the drag invalidates the drop. Cancellation does not mutate the collection. `ListReorderRequest` preserves item identity and relative order using collection Move events; view-model handlers maintain display numbering and existing save semantics. The buttons and keyboard shortcuts remain available without a mouse. The behavior releases its timer and adorners on drag completion, unload, or detach.
...
Mixer rows group sessions by process ID. Volume and mute writes enumerate all current sessions for that process in the mixer's playback/capture direction; a row's representative endpoint and session ID must not restrict writes. Snapshots ignore expired sessions, prefer active metadata over idle metadata, and retain every endpoint containing a represented process in selective scans. Displayed volume and mute combine active sessions when present, otherwise idle sessions: volume is their maximum and mute requires all of those sessions to be muted. Idle sessions on previous endpoints must not override active state. Session counts still include all live sessions. Single-session callbacks must not overwrite grouped state: those rows use the coalesced snapshot refresh. Queued writes capture the requested value before entering the COM queue. At Trace level, `mixer-process-volume-write` and `mixer-process-mute-write` report the target endpoint/session, state, requested value, and readback; `mixer-process-*-applied` records the number of sessions updated. A cache-update message alone does not identify which stream received a write.

Master and microphone dragging use bounded trailing writes: later movements update the pending value without restarting its deadline. A trailing callback releases its timer slot before writing audio and only consumes work it still owns, so a superseded callback cannot steal a newer request. Endpoint monitors are registered before notifications are subscribed, and existing and newly created sessions use the same registration path. Failed or interrupted attachment releases the partial monitor and its sessions. System Sounds classification uses Windows' native session flag; shared row names never take precedence over a nonzero process ID.
...
- An accepted KS reconnect request is not proof of a connected device. Cached and full-scan KS paths return after requesting the related audio profiles; the switch coordinator observes endpoint readiness asynchronously for up to `12000 ms`, followed by a `2000 ms` grace period. Late arrivals can complete through the deferred switch flow.
- KS selection uses one endpoint inventory for cached and full-scan requests, revalidating cached names and skipping disabled endpoints. Before requesting the selected profile, it resolves the identities and topology IDs of disconnected Bluetooth companions with the same nonempty physical ContainerID. Reconnect requests can temporarily invalidate endpoint metadata, so companion requests use those resolved IDs while rechecking active/disabled state. Companion names are not used for identity, and shared topology IDs are requested once per attempt. A failed shared topology request still allows each endpoint's direct fallback. Missing properties and stale or failed companions do not discard an accepted request for the device. Primary identity failures are logged at Info with the failing stage, available HRESULT, and property variant details.
- KS reconnect does not block the Core Audio worker waiting for visibility or repeat an accepted request just because the endpoint is still disconnected. The cached-endpoint probe tuning values still control association-endpoint observation after pairing.
- Headless output/input switching and routine targets share an endpoint-readiness wait: accepted requests use the configured success stabilization window (12 seconds by default); other attempted requests use the short post-attempt recheck budget. The CLI wait coalesces notification bursts into one pending recheck, periodically rechecks availability, honors runner disposal, and logs `bluetooth-reconnect-cli-readiness`.
...
- Diagnostic bundles tolerate unavailable source logs and record the exception type and HRESULT in their manifest. Archive write failures must propagate to the atomic writer; they must not be reported as missing source logs or replace an existing export.
...
- Process-monitor cleanup clears the event source before stopping and disposing it. Cleanup failures are logged by stage and must not prevent promotion to the polling fallback.
...
- Snapshot freshness checks reject negative ages after a system-clock correction. The same rule prevents stale PID volume matching and reuse of remembered Bluetooth endpoints; reconnect timing recorded in the future is reset.
...
- Session-monitor start/stop transitions are serialized, and each direction reads its current consumer state before changing subscriptions.
...
- Logs: approximately `5 MiB` per active file, `5` backups, rotation after `3` UTC calendar days based on persisted creation time, and `21` days of backup retention. Age checks run at startup and on the first write of each UTC day; size rotation also prunes expired backups. Settings backups are excluded from log cleanup.
...
The live Wi-Fi scan in `NativeWifiScannerTests` is an explicit integration test (`--explicit on` with a filter selecting that test). Unit runs cover cancellation and native data handling without scanning nearby networks.
