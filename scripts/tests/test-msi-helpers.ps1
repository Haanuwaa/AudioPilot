...
if (-not [string]::IsNullOrWhiteSpace($InstallerPath)) {
    ...
    $taskCommand = @(Invoke-AudioPilotMsiQuery -Path $packagePath -Query 'SELECT `Source`, `Target` FROM `CustomAction`' -Columns @('Source', 'Target') | Where-Object { $_.Source -eq 'WixQuietExecCmdLine' -and $_.Target -like '*schtasks.exe*' })
    ...
    $runCleanup = @(Invoke-AudioPilotMsiQuery -Path $packagePath -Query 'SELECT `Action`, `Type`, `Target` FROM `CustomAction`' -Columns @('Action', 'Type', 'Target') | Where-Object Action -eq 'CleanupStartupRunValue')
    $runCleanupStep = @($executeSteps | Where-Object Action -eq 'CleanupStartupRunValue')
    $runCommandStep = @($executeSteps | Where-Object Action -eq 'SetStartupRunCleanupCommand')
    $runCommand = @(Invoke-AudioPilotMsiQuery -Path $packagePath -Query 'SELECT `Action`, `Source`, `Target` FROM `CustomAction`' -Columns @('Action', 'Source', 'Target') | Where-Object Action -eq 'SetStartupRunCleanupCommand')
    if ($runCleanup.Count -ne 1 -or [int]$runCleanup[0].Type -ne 65 -or $runCleanup[0].Target -cne 'WixQuietExec' -or
        $runCleanupStep.Count -ne 1 -or $runCommandStep.Count -ne 1 -or
        [int]$runCommandStep[0].Sequence -le [int]$taskCleanupStep[0].Sequence -or
        [int]$runCleanupStep[0].Sequence -le [int]$runCommandStep[0].Sequence -or
        $runCleanupStep[0].Condition -cne 'REMOVE="ALL" AND NOT UPGRADINGPRODUCTCODE' -or
        $runCommandStep[0].Condition -cne $runCleanupStep[0].Condition -or
        $runCommand.Count -ne 1 -or $runCommand[0].Source -cne 'WixQuietExecCmdLine' -or
        $runCommand[0].Target -cne '"[System64Folder]reg.exe" delete "HKU\[UserSID]\Software\Microsoft\Windows\CurrentVersion\Run" /v "AudioPilot" /f /reg:64') {
        throw 'Startup Run cleanup must run quietly only after successful uninstall, after task cleanup, and exclude upgrades.'
    }
    Write-Host 'MSI startup registry and task cleanup checks passed.'
    ...
    try {
        ...
        $validationConditions = @(Invoke-AudioPilotMsiQuery -Path $packagePath -Query 'SELECT `Condition`, `Description` FROM `LaunchCondition`' -Columns @('Condition', 'Description') |
            Where-Object Description -like '*must be 0 or 1 when specified.')
        if ($validationConditions.Count -ne $optionNames.Count) { throw 'Installer switch validation is incomplete.' }
        $validationSession = $null
        try {
            $validationSession = $installer.GetType().InvokeMember('OpenPackage', 'InvokeMethod', $null, $installer, @($packagePath, 1))
            foreach ($optionName in $optionNames) {
                $rule = @($validationConditions | Where-Object Description -eq "$optionName must be 0 or 1 when specified.")
                if ($rule.Count -ne 1) { throw "Missing validation for $optionName." }
                foreach ($value in @('', '0', '1', '2', 'true', 'false', 'yes')) {
                    $validationSession.GetType().InvokeMember('Property', 'SetProperty', $null, $validationSession, @($optionName, $value)) | Out-Null
                    $actual = $validationSession.GetType().InvokeMember('EvaluateCondition', 'InvokeMethod', $null, $validationSession, @($rule[0].Condition))
                    $expected = if ($value -in @('', '0', '1')) { 1 } else { 0 }
                    if ($actual -ne $expected) { throw "$optionName incorrectly validated '$value': $actual" }
                }
                $validationSession.GetType().InvokeMember('Property', 'SetProperty', $null, $validationSession, @($optionName, '')) | Out-Null
            }
            Write-Host 'MSI switch validation checks passed.'
        }
        finally {
            if ($null -ne $validationSession) { [void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($validationSession) }
        }
