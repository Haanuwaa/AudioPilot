...
if ($args -contains '--version' -and $env:AUDIOPILOT_FORMAT_TEST_EXIT -eq '0') { Write-Output '10.0.401' }
...
try {
    ...
    Set-Content -LiteralPath 'Directory.Build.targets' -Value '<Project />'
    Set-Content -LiteralPath '.gitignore' -Value "ignored.cs`nartifacts/`ntools/`nscripts/`n*.json`n*.jsonl"
    & git add AudioPilot.Format.slnf tracked.cs deleted.cs .gitignore Directory.Build.targets
    ...
    foreach ($configurationFile in @('.editorconfig', 'Directory.Build.props', 'project.csproj')) {
        Set-Content -LiteralPath $configurationFile -Value 'configuration change'
        & pwsh -NoProfile -File scripts/validate-format.ps1 -ChangedOnly
        if ($LASTEXITCODE -ne 0) { throw 'Configuration-aware formatting failed.' }
        $formatArguments = Get-Content -LiteralPath $env:AUDIOPILOT_FORMAT_TEST_RECORD -Raw | ConvertFrom-Json
        if ('--include' -in $formatArguments) { throw "$configurationFile changes did not trigger full-solution formatting." }
        Remove-Item -LiteralPath $configurationFile
    }
    Remove-Item -LiteralPath 'Directory.Build.targets'
    & pwsh -NoProfile -File scripts/validate-format.ps1 -ChangedOnly
    if ($LASTEXITCODE -ne 0) { throw 'Formatting after configuration deletion failed.' }
    $formatArguments = Get-Content -LiteralPath $env:AUDIOPILOT_FORMAT_TEST_RECORD -Raw | ConvertFrom-Json
    if ('--include' -in $formatArguments) { throw 'Deleted build configuration did not trigger full-solution formatting.' }
    & git restore -- Directory.Build.targets
    if ($LASTEXITCODE -ne 0) { throw 'Failed to restore the fixture configuration.' }
    ...
    $packageAst = [Management.Automation.Language.Parser]::ParseFile(
        (Join-Path $repositoryRoot 'scripts/package-release.ps1'), [ref]$null, [ref]$null)
    $sdkFunction = $packageAst.Find({
        param($node)
        $node -is [Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Get-DotNetSdkVersion'
    }, $true)
    if ($null -eq $sdkFunction) { throw 'The packaging SDK metadata probe was not found.' }
    Invoke-Expression $sdkFunction.Extent.Text
    foreach ($probeExitCode in @(0, 1)) {
        $env:AUDIOPILOT_FORMAT_TEST_EXIT = [string]$probeExitCode
        foreach ($callerExitCode in @(0, 9)) {
            $global:LASTEXITCODE = $callerExitCode
            $sdkVersion = Get-DotNetSdkVersion
            $expectedVersion = if ($probeExitCode -eq 0) { '10.0.401' } else { $null }
            if ($sdkVersion -ne $expectedVersion) { throw 'The optional SDK probe returned incorrect metadata.' }
            if ($global:LASTEXITCODE -ne $callerExitCode) { throw 'The optional SDK probe changed the caller exit code.' }
        }
    }
    Write-Host 'Validation script checks passed: local/PR file selection, formatter/Git failures, unit retention, build reuse, coverage enforcement, and optional SDK probe isolation.'
}
finally {
    Pop-Location
