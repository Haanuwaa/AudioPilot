...
if ($ChangedOnly) {
    if ($env:GITHUB_EVENT_NAME -eq 'pull_request') {
        ...
        $changed = @(& git -c core.quotepath=false diff --name-only --diff-filter=ACDMRT "FETCH_HEAD...HEAD")
        if ($LASTEXITCODE -ne 0) { throw 'Failed to determine pull-request changes.' }
    }
    else {
        $changed = @(& git -c core.quotepath=false diff --name-only --diff-filter=ACDMRT HEAD)
    ...
    $configurationChanged = @($changed | Where-Object {
        $_ -match '(^|/)(\.editorconfig|global\.json|nuget\.config)$|\.(props|targets|csproj|sln|slnf|slnx)$'
    }).Count -gt 0
    if ($configurationChanged) {
        Write-Host 'Build or analyzer configuration changed; checking the full solution.'
    }
    else {
