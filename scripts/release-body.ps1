...
$projectChangelogUrl = "https://github.com/$Repository/blob/v$resolvedVersion/docs/CHANGELOG.md"
...
$outLines += '- **Portable ZIP**: extract the whole archive, then run `AudioPilot.exe`. Keep its companion files together.'
$outLines += '- **Minimum compatible OS**: Windows 10 version 2004 (build 19041) or later.'
$outLines += '- **Official OS support**: Windows 11 and currently supported Windows 10 Enterprise/LTSC editions that meet the minimum build, subject to the Microsoft and .NET 10 support lifecycles.'
$outLines += ""

$outLines += "### Choosing A Package"
$outLines += ""
$outLines += "- **MSI**: per-user Windows installer with the runtime included, Start Menu and desktop shortcuts, and an optional CLI PATH entry."
...
$outLines += ('- `AudioPilot-{0}-provenance.json`: source commit and build provenance for the packaged release.' -f $resolvedVersion)
