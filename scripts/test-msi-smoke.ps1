...
param(
    ...
    [switch]$VerifyUninstallRollback
)
...
function Get-StartupTaskProbeXml {
    param($Folder, [string]$TaskName)
    $task = $null
    try {
        ...
        if ($_.Exception.HResult -eq -2147024894 -or ($null -ne $_.Exception.InnerException -and $_.Exception.InnerException.HResult -eq -2147024894)) {
            return $null
        }
        throw
    }
...
if ($VerifyUninstallRollback) {
    $uninstallProbePath = Join-Path $logRoot "uninstall-rollback-probe-$([Guid]::NewGuid().ToString('N')).msi"
    & (Join-Path $PSScriptRoot 'new-msi-rollback-probe.ps1') -InputMsiPath $resolvedInstallMsiPath -OutputMsiPath $uninstallProbePath -Operation Uninstall
    $resolvedInstallMsiPath = $uninstallProbePath
}
...
if ($VerifyUninstallRollback) {
    $startupTaskName = "$ProductName Startup $currentUserSid"
    $startupScheduler = New-Object -ComObject Schedule.Service
    $startupScheduler.Connect()
    $startupTaskFolder = $startupScheduler.GetFolder('\')
    if (Get-StartupTaskProbeXml -Folder $startupTaskFolder -TaskName $startupTaskName) {
        throw 'A startup task already exists; refusing to overwrite it for the rollback test.'
    }
    $escapedExePath = [Security.SecurityElement]::Escape($exePath)
    $taskDefinition = @"
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <Triggers><LogonTrigger><UserId>$currentUserSid</UserId></LogonTrigger></Triggers>
  <Principals><Principal id="User"><UserId>$currentUserSid</UserId><LogonType>InteractiveToken</LogonType><RunLevel>LeastPrivilege</RunLevel></Principal></Principals>
  <Actions Context="User"><Exec><Command>$escapedExePath</Command></Exec></Actions>
</Task>
"@
    $registeredTask = $startupTaskFolder.RegisterTask($startupTaskName, $taskDefinition, 2, $currentUserSid, $null, 3)
    $startupTaskProbeCreated = $true
    try { $taskXml = [string]$registeredTask.Xml }
    finally { [void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($registeredTask) }
    $rollbackLog = Join-Path $logRoot 'uninstall-rollback.log'
    $arguments = @('/x', $productCode, '/qn', '/norestart', '/l*v', $rollbackLog, 'AUDIOPILOT_FAIL_UNINSTALL=1')
    $failedUninstall = Start-Process -FilePath 'msiexec.exe' -ArgumentList (ConvertTo-ProcessArgumentLine $arguments) -Wait -PassThru -WindowStyle Hidden
    if ($failedUninstall.ExitCode -ne 1603 -or (Get-Content -LiteralPath $rollbackLog -Raw) -notmatch 'AudioPilotRollbackProbe') {
        throw 'Uninstall did not fail at the expected deferred rollback probe.'
    }
    $installationRestored = @(Get-UninstallEntries -DisplayName $ProductName).Count -eq 1 -and
        (Test-Path -LiteralPath $cliPath) -and (Get-FileHash -LiteralPath $cliPath -Algorithm SHA256).Hash -eq $cliHash
    $startupRunRestored = (Test-RegistryValueExists -Path $runRegistryPath -Name $ProductName) -and
        (Get-ItemPropertyValue -LiteralPath $runRegistryPath -Name $ProductName) -ceq $exePath
    $startupTaskRestored = (Get-StartupTaskProbeXml -Folder $startupTaskFolder -TaskName $startupTaskName) -ceq $taskXml
    if (-not $installationRestored -or -not $startupRunRestored -or -not $startupTaskRestored) {
        throw "Failed-uninstall rollback: installation=$installationRestored startupRun=$startupRunRestored startupTask=$startupTaskRestored"
    }
    Assert-PathExists -Path $exePath
    Assert-PathExists -Path $userDataSentinelPath
    Write-Host 'Failed-uninstall rollback preserved the application, user data, Run value, and startup task.'
}
...
if ($VerifyUninstallRollback -and (Get-StartupTaskProbeXml -Folder $startupTaskFolder -TaskName $startupTaskName)) {
    throw 'Successful uninstall did not remove the startup task.'
}
...
Write-Host "MSI smoke test passed."
    ...
    try {
        if ($startupTaskProbeCreated -and (Get-StartupTaskProbeXml -Folder $startupTaskFolder -TaskName $startupTaskName)) {
            $startupTaskFolder.DeleteTask($startupTaskName, 0)
        }
        ...
        if ($null -ne $startupScheduler) { [void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($startupScheduler) }
        Release-MsiSmokeMutex
    }
}
